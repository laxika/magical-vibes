 ...
 * the each-player pair is the APNAP remainder of an "each player searches for a basic land"
 * flow (Field of Ruin, Old-Growth Dryads), advanced after each player's search resolves;
...
public record LibrarySearchFollowUp(boolean basicLandToHand, boolean cardToGraveyard,
    ...
    public static final LibrarySearchFollowUp NONE =
            new LibrarySearchFollowUp(false, false, List.of(), false, null, null);
    ...
    public static LibrarySearchFollowUp forBasicLandToHand() {
        return new LibrarySearchFollowUp(true, false, List.of(), false, null, null);
    }

    public static LibrarySearchFollowUp forCardToGraveyard() {
        return new LibrarySearchFollowUp(false, true, List.of(), false, null, null);
    }

    public static LibrarySearchFollowUp eachPlayerBasicLand(List<UUID> remainingSearchers, boolean tapped) {
        return new LibrarySearchFollowUp(false, false, remainingSearchers, tapped, null, null);
    }

    public static LibrarySearchFollowUp opponentExile(PendingOpponentExileChoice choice) {
        return new LibrarySearchFollowUp(false, false, List.of(), false, choice, null);
    }

    public static LibrarySearchFollowUp imprint(UUID sourcePermanentId) {
        return new LibrarySearchFollowUp(false, false, List.of(), false, null, sourcePermanentId);
    }

    /** The same follow-up with the each-player remainder advanced past the current searcher. */
