 ...
 *
 * @param damage      damage dealt to the casting opponent, evaluated when the trigger resolves
 ...
 */
public record SpellCastDamageToCasterEffect(DynamicAmount damage, CardPredicate spellFilter, Condition intervening,
    ...
    public SpellCastDamageToCasterEffect(int damage, CardPredicate spellFilter) {
        this(new Fixed(damage), spellFilter, null, false);
    }

    /** The dynamic-damage form. */
    public SpellCastDamageToCasterEffect(DynamicAmount damage, CardPredicate spellFilter) {
    ...
    public SpellCastDamageToCasterEffect(int damage, CardPredicate spellFilter, Condition intervening) {
        this(new Fixed(damage), spellFilter, intervening, false);
    }

    /** The dynamic-damage source-relative intervening-if form. */
    public SpellCastDamageToCasterEffect(DynamicAmount damage, CardPredicate spellFilter, Condition intervening) {
    ...
    public static SpellCastDamageToCasterEffect whenCasterIsNotActiveTurn(int damage) {
        return new SpellCastDamageToCasterEffect(new Fixed(damage), null, null, true);
    }
}
