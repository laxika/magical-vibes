...
/**
 * Cost effect that removes one or more allowed counters from any permanent the payer controls
 ...
 * permanent carrying at least one allowed counter is a legal choice. When the chosen permanent
 * carries several allowed kinds of counters, the first kind present is removed (same convention as
...
public record RemoveCounterFromControlledPermanentCost(List<CounterType> allowedCounterTypes,
    ...
    public RemoveCounterFromControlledPermanentCost() {
        this(List.of(CounterType.ANY), 1, null, false);
    }

    public RemoveCounterFromControlledPermanentCost(List<CounterType> allowedCounterTypes) {
        this(allowedCounterTypes, 1, null, false);
    }

    public RemoveCounterFromControlledPermanentCost(CounterType... allowedCounterTypes) {
        this(List.of(allowedCounterTypes), 1, null, false);
    }

    public RemoveCounterFromControlledPermanentCost(CounterType counterType, PermanentPredicate permanentFilter) {
        this(List.of(counterType), 1, permanentFilter, false);
    }

    public RemoveCounterFromControlledPermanentCost(int count, PermanentPredicate permanentPredicate,
                                                    boolean excludeSource) {
        this(List.of(CounterType.ANY), count, permanentPredicate, excludeSource);
    }
