...
public class ManaPool {
    ...
    private int kickedOrInstantSorceryOnlyColorless;
    private final EnumMap<ManaColor, Integer> kickedOrInstantSorceryOnlyColored = new EnumMap<>(ManaColor.class);
    ...
    /** Per-color mana spendable only on creature spells with mana value at least four or with X. */
    private final EnumMap<ManaColor, Integer> creatureSpellManaValueAtLeastFourOrXOnlyMana =
    ...
    /** Per-color mana that may only be spent to cast spells with devoid. */
    private final EnumMap<ManaColor, Integer> devoidSpellOnlyMana = new EnumMap<>(ManaColor.class);
    /**
    ...
    /** Permission context for mana restricted to instants/sorceries or kicked spells. */
    private boolean kickedOrInstantSorceryOnlyManaUsableForKickedSpell;
    private boolean kickedOrInstantSorceryOnlyManaUsableForInstantSorcery;
    ...
    public ManaPool(ManaPool source) {
        ...
        this.kickedOrInstantSorceryOnlyColorless = source.kickedOrInstantSorceryOnlyColorless;
        kickedOrInstantSorceryOnlyColored.putAll(source.kickedOrInstantSorceryOnlyColored);
        ...
        devoidSpellOnlyMana.putAll(source.devoidSpellOnlyMana);
        ...
        this.kickedOrInstantSorceryOnlyManaUsableForKickedSpell =
                source.kickedOrInstantSorceryOnlyManaUsableForKickedSpell;
        this.kickedOrInstantSorceryOnlyManaUsableForInstantSorcery =
                source.kickedOrInstantSorceryOnlyManaUsableForInstantSorcery;
    ...
    public boolean isKickedOrInstantSorceryOnlyManaUsableForKickedSpell() {
        return kickedOrInstantSorceryOnlyManaUsableForKickedSpell;
    }

    public void setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(boolean enabled) {
        kickedOrInstantSorceryOnlyManaUsableForKickedSpell = enabled;
    }

    public boolean isKickedOrInstantSorceryOnlyManaUsableForInstantSorcery() {
        return kickedOrInstantSorceryOnlyManaUsableForInstantSorcery;
    }

    public void setKickedOrInstantSorceryOnlyManaUsableForInstantSorcery(boolean enabled) {
        kickedOrInstantSorceryOnlyManaUsableForInstantSorcery = enabled;
    }
    ...
    public void clear() {
        ...
        kickedOrInstantSorceryOnlyColored.replaceAll((color, amount) -> 0);
    ...
    public int getTotalAllMana() {
        ...
        total += legendarySpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
        total += restrictedRed;
        total += kickedOnlyGreen + kickedOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
        total += kickedOrInstantSorceryOnlyColorless;
        total += kickedOrInstantSorceryOnlyColored.values().stream().mapToInt(Integer::intValue).sum();
        ...
        total += getDevoidSpellOnlyManaTotal();
        return total;
    }
    ...
    public void addDevoidSpellOnlyMana(ManaColor color, int amount) {
        devoidSpellOnlyMana.merge(color, amount, Integer::sum);
    }

    public int getDevoidSpellOnlyMana(ManaColor color) {
        return devoidSpellOnlyMana.getOrDefault(color, 0);
    }

    public int getDevoidSpellOnlyManaTotal() {
        return devoidSpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void removeDevoidSpellOnlyMana(ManaColor color, int amount) {
        int current = devoidSpellOnlyMana.getOrDefault(color, 0);
        devoidSpellOnlyMana.put(color, Math.max(0, current - amount));
    }

    /** Temporarily exposes devoid-only mana to the spell-payment algorithm. */
    public DevoidSpellManaState promoteDevoidSpellOnlyMana() {
        EnumMap<ManaColor, Integer> regularBefore = new EnumMap<>(ManaColor.class);
        EnumMap<ManaColor, Integer> promoted = new EnumMap<>(ManaColor.class);
        for (ManaColor color : ManaColor.values()) {
            regularBefore.put(color, pool.getOrDefault(color, 0));
            int amount = getDevoidSpellOnlyMana(color);
            promoted.put(color, amount);
            if (amount > 0) {
                pool.merge(color, amount, Integer::sum);
                devoidSpellOnlyMana.put(color, 0);
        ...
        return new DevoidSpellManaState(regularBefore, promoted);
    }

    /** Restores the unspent portion of temporarily promoted devoid-only mana. */
    public void restorePromotedDevoidSpellOnlyMana(DevoidSpellManaState state) {
        for (ManaColor color : ManaColor.values()) {
            ...
            if (remaining > 0) {
                ...
                devoidSpellOnlyMana.merge(color, remaining, Integer::sum);
            }
        }
    }

    public record DevoidSpellManaState(Map<ManaColor, Integer> regularBefore,
    ...
    public int getColorlessSpellOrPermanentAbilityMana() {
        return colorlessSubtypeSpellOrAbilityMana.getOrDefault(null, 0);
    }

    public void addColorlessSpellOrPermanentAbilityMana(int amount) {
        colorlessSubtypeSpellOrAbilityMana.merge(null, amount, Integer::sum);
    }

    public void removeColorlessSpellOrPermanentAbilityMana(int amount) {
        removeColorlessSubtypeSpellOrAbilityMana(null, amount);
    }

    /** Temporarily makes this restricted mana regular colorless mana for one eligible payment. */
    public int promoteColorlessSpellOrPermanentAbilityMana() {
        int amount = getColorlessSpellOrPermanentAbilityMana();
        if (amount > 0) {
            colorlessSubtypeSpellOrAbilityMana.remove(null);
            pool.merge(ManaColor.COLORLESS, amount, Integer::sum);
            promotedColorlessSpellOrPermanentAbilityMana += amount;
        }
        return amount;
    }

    /** Returns unspent temporarily promoted mana to its restricted bucket. */
    public void restorePromotedColorlessSpellOrPermanentAbilityMana() {
        if (promotedColorlessSpellOrPermanentAbilityMana <= 0) {
            return;
        }
    ...
    public int getLegendarySpellOnlyMana(ManaColor color) {
        return legendarySpellOnlyMana.getOrDefault(color, 0);
    }

    public int getLegendarySpellOnlyManaTotal() {
        return legendarySpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void addLegendarySpellOnlyMana(ManaColor color, int amount) {
        if (color == ManaColor.COLORLESS) {
            addLegendarySpellOnlyColorless(amount);
        } else {
            legendarySpellOnlyMana.merge(color, amount, Integer::sum);
        }
    }
    ...
    public int getKickedOnlyMana(ManaColor color) {
        int amount = color == ManaColor.GREEN ? kickedOnlyGreen : kickedOnlyMana.getOrDefault(color, 0);
        return amount + (kickedOrInstantSorceryOnlyManaUsableForKickedSpell
                ? getKickedOrInstantSorceryOnlyMana(color) : 0);
    }

    public int getKickedOnlyManaTotal() {
        return kickedOnlyGreen + kickedOnlyMana.values().stream().mapToInt(Integer::intValue).sum()
                + (kickedOrInstantSorceryOnlyManaUsableForKickedSpell
                ? getKickedOrInstantSorceryOnlyManaTotal() : 0);
    }
    ...
    public void removeKickedOnlyMana(ManaColor color, int amount) {
        if (kickedOrInstantSorceryOnlyManaUsableForKickedSpell && amount > 0) {
            int fromUnion = Math.min(amount, getKickedOrInstantSorceryOnlyMana(color));
            removeKickedOrInstantSorceryOnlyMana(color, fromUnion);
            amount -= fromUnion;
        }
        if (amount == 0) {
            return;
    ...
    public int getKickedOrInstantSorceryOnlyMana(ManaColor color) {
        return color == ManaColor.COLORLESS
                ? kickedOrInstantSorceryOnlyColorless
                : kickedOrInstantSorceryOnlyColored.getOrDefault(color, 0);
    }

    public int getKickedOrInstantSorceryOnlyManaTotal() {
        return kickedOrInstantSorceryOnlyColorless
                + kickedOrInstantSorceryOnlyColored.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void addKickedOrInstantSorceryOnlyMana(ManaColor color, int amount) {
        if (color == ManaColor.COLORLESS) {
            kickedOrInstantSorceryOnlyColorless += amount;
        } else {
            kickedOrInstantSorceryOnlyColored.merge(color, amount, Integer::sum);
        }
    }

    public void removeKickedOrInstantSorceryOnlyMana(ManaColor color, int amount) {
        if (color == ManaColor.COLORLESS) {
            kickedOrInstantSorceryOnlyColorless = Math.max(0,
                    kickedOrInstantSorceryOnlyColorless - amount);
        } else {
            int current = kickedOrInstantSorceryOnlyColored.getOrDefault(color, 0);
            kickedOrInstantSorceryOnlyColored.put(color, Math.max(0, current - amount));
        }
    }
    ...
    public int getInstantSorceryOnlyColorless() {
        return instantSorceryOnlyColorless + foretellOrInstantSorceryOnlyColorless
                ...
                + (kickedOrInstantSorceryOnlyManaUsableForInstantSorcery
                ? kickedOrInstantSorceryOnlyColorless : 0);
    }
    ...
    public void removeInstantSorceryOnlyColorless(int amount) {
        if (kickedOrInstantSorceryOnlyManaUsableForInstantSorcery && amount > 0) {
            int fromUnion = Math.min(amount, kickedOrInstantSorceryOnlyColorless);
            kickedOrInstantSorceryOnlyColorless -= fromUnion;
            amount -= fromUnion;
        }
    ...
    public int getInstantSorceryOnlyColored(ManaColor color) {
        return instantSorceryOnlyColored.getOrDefault(color, 0)
                ...
                + (kickedOrInstantSorceryOnlyManaUsableForInstantSorcery
                ? kickedOrInstantSorceryOnlyColored.getOrDefault(color, 0) : 0);
    }
    ...
    public void removeInstantSorceryOnlyColored(ManaColor color, int amount) {
        if (kickedOrInstantSorceryOnlyManaUsableForInstantSorcery && amount > 0) {
            int fromUnion = Math.min(amount, kickedOrInstantSorceryOnlyColored.getOrDefault(color, 0));
            removeKickedOrInstantSorceryOnlyMana(color, fromUnion);
            amount -= fromUnion;
        }
    ...
    public void removeCreatureSpellOnlyMana(ManaColor color, int amount) {
        ...
        creatureSpellOnlyMana.put(color, current - removed);
        int creatureSource = creatureSourceCreatureSpellOnlyMana.getOrDefault(color, 0);
        creatureSourceCreatureSpellOnlyMana.put(color, Math.max(0, creatureSource - removed));
    }

    public int getCreatureSourceCreatureSpellOnlyManaTotal() {
        return creatureSourceCreatureSpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void addNoncreatureSpellOnlyMana(ManaColor color, int amount) {
        noncreatureSpellOnlyMana.merge(color, amount, Integer::sum);
    }

    public int getNoncreatureSpellOnlyMana(ManaColor color) {
        return noncreatureSpellOnlyMana.getOrDefault(color, 0);
    }

    public int getNoncreatureSpellOnlyManaTotal() {
        return noncreatureSpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void removeNoncreatureSpellOnlyMana(ManaColor color, int amount) {
        int current = noncreatureSpellOnlyMana.getOrDefault(color, 0);
        noncreatureSpellOnlyMana.put(color, Math.max(0, current - amount));
    }

    public void addMulticoloredSpellOnlyMana(ManaColor color, int amount) {
        multicoloredSpellOnlyMana.merge(color, amount, Integer::sum);
    }

    public int getMulticoloredSpellOnlyMana(ManaColor color) {
        return multicoloredSpellOnlyMana.getOrDefault(color, 0);
    }

    public int getMulticoloredSpellOnlyManaTotal() {
        return multicoloredSpellOnlyMana.values().stream().mapToInt(Integer::intValue).sum();
    }

    public void removeMulticoloredSpellOnlyMana(ManaColor color, int amount) {
        int current = multicoloredSpellOnlyMana.getOrDefault(color, 0);
    ...
    public MulticoloredSpellManaState promoteMulticoloredSpellOnlyMana() {
        ...
        for (ManaColor color : ManaColor.values()) {
            regularBefore.put(color, pool.getOrDefault(color, 0));
            int amount = getMulticoloredSpellOnlyMana(color);
            promoted.put(color, amount);
            if (amount > 0) {
                pool.merge(color, amount, Integer::sum);
                multicoloredSpellOnlyMana.put(color, 0);
        ...
        return new MulticoloredSpellManaState(regularBefore, promoted);
    }

    /** Restores unspent multicolored-spell-only mana after a spell payment. */
    public void restorePromotedMulticoloredSpellOnlyMana(MulticoloredSpellManaState state) {
        for (ManaColor color : ManaColor.values()) {
            int promoted = state.promoted().getOrDefault(color, 0);
            int spent = Math.max(0, state.regularBefore().getOrDefault(color, 0)
                    + promoted - pool.getOrDefault(color, 0));
            ...
            if (remaining > 0) {
                pool.merge(color, -remaining, Integer::sum);
                multicoloredSpellOnlyMana.merge(color, remaining, Integer::sum);
            }
        }
    }

    public record MulticoloredSpellManaState(Map<ManaColor, Integer> regularBefore,
                                             Map<ManaColor, Integer> promoted) {
    }

    public NoncreatureSpellManaState promoteNoncreatureSpellOnlyMana() {
        EnumMap<ManaColor, Integer> regularBefore = new EnumMap<>(ManaColor.class);
        EnumMap<ManaColor, Integer> promoted = new EnumMap<>(ManaColor.class);
        for (ManaColor color : ManaColor.values()) {
            regularBefore.put(color, pool.getOrDefault(color, 0));
            int amount = getNoncreatureSpellOnlyMana(color);
            promoted.put(color, amount);
            if (amount > 0) {
                pool.merge(color, amount, Integer::sum);
                noncreatureSpellOnlyMana.put(color, 0);
        ...
        return new NoncreatureSpellManaState(regularBefore, promoted);
    }

    public void restorePromotedNoncreatureSpellOnlyMana(NoncreatureSpellManaState state) {
        for (ManaColor color : ManaColor.values()) {
            int promoted = state.promoted().getOrDefault(color, 0);
            int spent = Math.max(0, state.regularBefore().getOrDefault(color, 0)
                    + promoted - pool.getOrDefault(color, 0));
            ...
            if (remaining > 0) {
                pool.merge(color, -remaining, Integer::sum);
                noncreatureSpellOnlyMana.merge(color, remaining, Integer::sum);
            }
        }
    }

    public record NoncreatureSpellManaState(Map<ManaColor, Integer> regularBefore,
                                            Map<ManaColor, Integer> promoted) {
    }
    ...
    public void addCreatureSpellManaValueAtLeastFourOrXOnlyMana(ManaColor color, int amount) {
        creatureSpellManaValueAtLeastFourOrXOnlyMana.merge(color, amount, Integer::sum);
    }

    public int getCreatureSpellManaValueAtLeastFourOrXOnlyMana(ManaColor color) {
        return creatureSpellManaValueAtLeastFourOrXOnlyMana.getOrDefault(color, 0);
    }

    public int getCreatureSpellManaValueAtLeastFourOrXOnlyManaTotal() {
        return creatureSpellManaValueAtLeastFourOrXOnlyMana.values().stream()
                .mapToInt(Integer::intValue).sum();
    }

    public void removeCreatureSpellManaValueAtLeastFourOrXOnlyMana(ManaColor color, int amount) {
        int current = creatureSpellManaValueAtLeastFourOrXOnlyMana.getOrDefault(color, 0);
        creatureSpellManaValueAtLeastFourOrXOnlyMana.put(color, Math.max(0, current - amount));
    }

    public int getFaceDownSpellsOrTurnFaceUpMana(ManaColor color) {
        return faceDownSpellsOrTurnFaceUpMana.getOrDefault(color, 0);
    }

    public int getFaceDownSpellsOrTurnFaceUpManaTotal() {
        int total = 0;
        for (int value : faceDownSpellsOrTurnFaceUpMana.values()) {
            total += value;
        }
        return total;
    }

    public void addFaceDownSpellsOrTurnFaceUpMana(ManaColor color, int amount) {
        faceDownSpellsOrTurnFaceUpMana.merge(color, amount, Integer::sum);
    }
    ...
    /** Temporarily exposes this restricted mana to the ordinary mana payment path. */
    public FaceDownSpellsOrTurnFaceUpManaState promoteFaceDownSpellsOrTurnFaceUpMana() {
        EnumMap<ManaColor, Integer> regularBefore = new EnumMap<>(ManaColor.class);
        EnumMap<ManaColor, Integer> promoted = new EnumMap<>(ManaColor.class);
        for (ManaColor color : ManaColor.values()) {
            regularBefore.put(color, pool.getOrDefault(color, 0));
            int amount = faceDownSpellsOrTurnFaceUpMana.getOrDefault(color, 0);
            promoted.put(color, amount);
            if (amount > 0) {
                pool.merge(color, amount, Integer::sum);
                faceDownSpellsOrTurnFaceUpMana.put(color, 0);
        ...
        return new FaceDownSpellsOrTurnFaceUpManaState(regularBefore, promoted);
    }

    /** Restores the unspent portion of temporarily exposed face-down/face-up mana. */
    public void restorePromotedFaceDownSpellsOrTurnFaceUpMana(FaceDownSpellsOrTurnFaceUpManaState state) {
        for (ManaColor color : ManaColor.values()) {
            int promotedAmount = state.promoted().getOrDefault(color, 0);
            int spent = Math.max(0, state.regularBefore().getOrDefault(color, 0)
                    + promotedAmount - pool.getOrDefault(color, 0));
            ...
            if (remaining > 0) {
                pool.merge(color, -remaining, Integer::sum);
                faceDownSpellsOrTurnFaceUpMana.merge(color, remaining, Integer::sum);
            }
        }
    }
    ...
    public record FaceDownSpellsOrTurnFaceUpManaState(Map<ManaColor, Integer> regularBefore,
                                                       Map<ManaColor, Integer> promoted) {
    }
    ...
    public void convertNonPersistentManaToColorless() {
        ...
        pool.merge(ManaColor.COLORLESS, restrictedRed + kickedOnlyGreen
                + kickedOnlyMana.values().stream().mapToInt(Integer::intValue).sum(), Integer::sum);
        restrictedRed = 0;
        kickedOnlyGreen = 0;
        kickedOnlyMana.clear();
        kickedOrInstantSorceryOnlyColorless += moveColoredManaToColorless(kickedOrInstantSorceryOnlyColored);
    ...
    public void convertNonPersistentManaTo(ManaColor replacementColor) {
        ...
        moveManaToPool(replacementColor, kickedOnlyGreen
                + kickedOnlyMana.values().stream().mapToInt(Integer::intValue).sum());
        kickedOnlyGreen = 0;
        kickedOnlyMana.clear();
        moveManaToPool(replacementColor, kickedOrInstantSorceryOnlyColorless);
        kickedOrInstantSorceryOnlyColorless = 0;
        ...
        moveManaTo(replacementColor, legendarySpellOnlyMana);
        ...
        moveManaTo(replacementColor, kickedOrInstantSorceryOnlyColored);
    ...
    public boolean drainNonPersistent(Set<ManaColor> protectedColors) {
        ...
        drainColorBucket(legendarySpellOnlyMana, protectedColors);
        ...
        drainColorBucket(devoidSpellOnlyMana, protectedColors);
    ...
    public Map<String, Integer> toMap() {
        ...
        for (ManaColor color : ManaColor.values()) {
            ...
            amount += color == ManaColor.GREEN ? kickedOnlyGreen : kickedOnlyMana.getOrDefault(color, 0);
            amount += kickedOrInstantSorceryOnlyColored.getOrDefault(color, 0);
            ...
            amount += devoidSpellOnlyMana.getOrDefault(color, 0);
