...
@CardUsed({RiptideChronologist.class, AphettoAlchemist.class, AvianChangeling.class, NosyGoblin.class})
class RiptideChronologistTest extends BaseCardTest {
    ...
    void sacrificesAndUntapsChosenType() {
        ...
        Permanent ownWizard = addCreatureReady(player1, new AphettoAlchemist());
        Permanent opponentWizard = addCreatureReady(player2, new AphettoAlchemist());
        Permanent goblin = addCreatureReady(player1, new NosyGoblin());
        ownWizard.tap();
        opponentWizard.tap();
        goblin.tap();
        ...
        assertThat(ownWizard.isTapped()).isFalse();
