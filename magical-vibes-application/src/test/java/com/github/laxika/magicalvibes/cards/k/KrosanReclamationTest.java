...
@CardUsed({KrosanReclamation.class, GrizzlyFate.class, GuidedStrike.class})
class KrosanReclamationTest extends BaseCardTest {
    ...
    void shufflesUpToTwoCardsFromTargetPlayersGraveyard() {
        ...
        harness.setGraveyard(player2, List.of(new GrizzlyFate(), new GuidedStrike(), new GrizzlyFate()));
    ...
    void flashbackExilesSpellAfterResolution() {
        ...
        harness.setGraveyard(player2, List.of(new GrizzlyFate()));
        int librarySizeBefore = gd.playerDecks.get(player2.getId()).size();
        addMana();
