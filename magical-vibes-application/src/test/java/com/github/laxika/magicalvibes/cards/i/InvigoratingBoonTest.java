...
@CardUsed({InvigoratingBoon.class, Blackmail.class, ElvishWarrior.class, ForgottenCave.class})
class InvigoratingBoonTest extends BaseCardTest {
    ...
    @Test
    @DisplayName("The creature target is chosen before the may decision")
    void creatureTargetIsChosenBeforeMayDecision() {
        harness.addToBattlefield(player1, new InvigoratingBoon());
    ...
    @Test
    @DisplayName("Cycling with no creature target does not create a may ability")
    void noCreatureTargetDoesNotCreateMayAbility() {
        harness.addToBattlefield(player1, new InvigoratingBoon());
        harness.setHand(player1, List.of(new ForgottenCave()));
        harness.setLibrary(player1, List.of(new ElvishWarrior()));
        harness.addMana(player1, ManaColor.RED, 1);
