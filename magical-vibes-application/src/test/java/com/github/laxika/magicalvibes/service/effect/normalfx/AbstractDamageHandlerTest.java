...
abstract class AbstractDamageHandlerTest {
    ...
    @Mock protected GameLogService gameLogService;
    ...
    void setUpDamageHandlerBase() {
        ...
        lenient().when(damagePreventionService.applyTurnDamageRedirectToCreature(
                        eq(gd), any(), any(), any(), anyInt(), anyBoolean()))
    ...
    protected void stubCreatureSourceRedirects() {
        ...
        when(damagePreventionService.applyChosenSourceNextDamageToAnyTargetShield(eq(gd), any(), anyInt(), any())).thenAnswer(inv -> inv.getArgument(2));
