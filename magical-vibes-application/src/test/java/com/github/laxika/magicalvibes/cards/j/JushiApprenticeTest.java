...
@CardUsed({JushiApprentice.class, TomoyaTheRevealer.class, Island.class})
class JushiApprenticeTest extends BaseCardTest {
    ...
    @Test
    @DisplayName("Tomoya can target its controller and draws a fixed amount based on the initial hand size")
    void tomoyaCanTargetController() {
        Permanent apprentice = addApprentice();
        apprentice.setTransformed(true);
        apprentice.setCard(apprentice.getOriginalCard().getBackFaceCard());
    ...
    private Permanent addApprentice() {
        return addCreatureReady(player1, new JushiApprentice());
    }

    private List<Card> hand(int count) {
        List<Card> cards = new ArrayList<>();
        IntStream.range(0, count).forEach(index -> cards.add(new Island()));
        return cards;
    }
}
