...
@CardUsed({MistvaultBridge.class, StoneRain.class})
class MistvaultBridgeTest extends BaseCardTest {

    @Test
    @DisplayName("Enters tapped")
    void entersTapped() {
        harness.setHand(player1, List.of(new MistvaultBridge()));
    ...
    @Test
    @DisplayName("Mana ability adds blue or black mana")
    void manaAbilityAddsBlueOrBlackMana() {
    ...
    void survivesDestruction() {
        harness.addToBattlefield(player2, new MistvaultBridge());
        harness.setHand(player1, List.of(new StoneRain()));
        harness.addMana(player1, ManaColor.RED, 4);

        UUID targetId = harness.getPermanentId(player2, "Mistvault Bridge");
