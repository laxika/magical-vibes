...
class KjeldoranSkyknightTest extends BaseCardTest {
    ...
    @Test
    @DisplayName("First strike kills a 5/1 blocker before it can deal damage")
    void firstStrikeKillsBlockerBeforeReciprocalDamage() {
        ...
        skyknight.setAttacking(true);
        gd.playerBattlefields.get(player1.getId()).add(skyknight);

        Permanent blocker = new Permanent(creature("Ogre Bruiser", 5, 1));
    ...
    void bandingAttackerLetsActivePlayerDivideBlockerDamage() {
        ...
        // Funnel all 3 damage onto the bears; first strike from Skyknight already resolved.
