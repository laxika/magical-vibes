...
class EchoOfEonsTest extends BaseCardTest {
    ...
    @Test
    @DisplayName("Flashback resolves the reset and exiles Echo of Eons")
    void flashbackResolvesAndExilesTheCard() {
        EchoOfEons echo = new EchoOfEons();
        Card handCard = new GrizzlyBears();
        Card graveyardCard = new GrizzlyBears();
        harness.setGraveyard(player1, List.of(echo, graveyardCard));
        ...
        assertThat(gd.getPlayerExiledCards(player1.getId())).contains(echo);
    }
    ...
    private void setLibraries(int count) {
