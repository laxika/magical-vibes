...
public class LoginWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if (timeoutTask != null) {
            ...
            log.debug("Cancelled timeout task for session: {}", session.getId());
        }
        ...
        try {
            ...
            if (!"LOGIN".equals(loginRequest.getType())) {
                log.warn("Invalid message type received: {}", loginRequest.getType());
    ...
    private void sendResponse(WebSocketSession session, LoginResponse response) throws IOException {
        ...
        log.info("Sent response to session {}: {}", session.getId(), response.getType());

        // Close connection after sending response
        session.close(CloseStatus.NORMAL);
        log.info("Closed connection for session: {}", session.getId());
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        ...
        if (timeoutTask != null) {
            timeoutTask.cancel(false);
            log.debug("Cleaned up timeout task for session: {}", session.getId());
        }
    }
