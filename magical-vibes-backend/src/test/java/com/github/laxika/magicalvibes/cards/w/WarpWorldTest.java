...
class WarpWorldTest {
    ...
    void warpWorldPromptsAuraAttachmentChoice() {
        ...
        assertThat(gd.pendingAuraCard.getName()).isEqualTo("Pacifism");
        assertThat(gd.awaitingPermanentChoiceValidIds).hasSize(2);

        UUID chosenTarget = gd.awaitingPermanentChoiceValidIds.stream().findFirst().orElseThrow();
    ...
    void controlAuraChoiceStealsCreature() {
        ...
        UUID opponentCreatureId = gd.playerBattlefields.get(player2.getId()).stream()
                .map(Permanent::getId)
                .filter(id -> gd.awaitingPermanentChoiceValidIds.contains(id))
                .findFirst()
                .orElseThrow();
    ...
    void auraChoicesFollowApnapOrder() {
        ...
        UUID p2Choice = gd.awaitingPermanentChoiceValidIds.stream().findFirst().orElseThrow();
        harness.handlePermanentChosen(player2, p2Choice);

        assertThat(gd.awaitingInput).isEqualTo(AwaitingInput.PERMANENT_CHOICE);
        assertThat(gd.awaitingPermanentChoicePlayerId).isEqualTo(player1.getId());

        UUID p1Choice = gd.awaitingPermanentChoiceValidIds.stream().findFirst().orElseThrow();
    ...
    void warpWorldBottomOrderChoice() {
        ...
        assertThat(gd.awaitingInput).isEqualTo(AwaitingInput.LIBRARY_REORDER);
        assertThat(gd.awaitingLibraryReorderPlayerId).isEqualTo(player1.getId());
        assertThat(gd.awaitingLibraryReorderToBottom).isTrue();
        assertThat(gd.awaitingLibraryReorderCards).hasSize(2);

        List<UUID> beforeOrderIds = gd.awaitingLibraryReorderCards.stream().map(Card::getId).toList();
    ...
    void bottomReorderFollowsApnapOrder() {
        ...
        assertThat(gd.awaitingInput).isEqualTo(AwaitingInput.LIBRARY_REORDER);
        assertThat(gd.awaitingLibraryReorderToBottom).isTrue();
        assertThat(gd.awaitingLibraryReorderPlayerId).isEqualTo(player2.getId());
