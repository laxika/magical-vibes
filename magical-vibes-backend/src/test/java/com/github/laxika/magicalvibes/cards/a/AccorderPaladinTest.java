...
class AccorderPaladinTest extends BaseCardTest {
    ...
    @Test
    @DisplayName("Accorder Paladin has battle cry ON_ATTACK effect")
    void hasBattleCryEffect() {
        AccorderPaladin card = new AccorderPaladin();

        assertThat(card.getEffects(EffectSlot.ON_ATTACK)).hasSize(1);
        assertThat(card.getEffects(EffectSlot.ON_ATTACK).getFirst()).isInstanceOf(BoostAllOwnCreaturesEffect.class);
        BoostAllOwnCreaturesEffect effect = (BoostAllOwnCreaturesEffect) card.getEffects(EffectSlot.ON_ATTACK).getFirst();
