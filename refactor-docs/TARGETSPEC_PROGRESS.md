...
## Step 11 — Close-out: permanent guard, doc sweep, program complete  (2026-07-15)

**PROGRAM COMPLETE.** All eleven steps are done. Precondition verified first by reading
`CardEffect.java`: it declares ONLY `targetSpec()` and `isPowerToughnessDefining()` — the eleven legacy
targeting methods are gone (step 10). This step retired the shrinking baseline ratchet in favour of a
permanent invariant guard, deleted the consumed scaffolding, and swept `agent-docs/` + the
`implement-card` skill onto the new mechanism.

### End state

- Every effect exposes its targeting through the single derived `TargetSpec targetSpec()`. The eleven
  legacy `canTarget*` / `isSelfTargeting` / `isDamageOrDestruction` / `targetPredicate` /
  `requiredPlayerTargetCount` methods no longer exist on `CardEffect` and are declared by NO effect.
- `isPowerToughnessDefining()` is KEPT (a copy/CDA rule, CR 707.9d — never in scope).
- `@ValidatesTarget` survives ONLY as the escape hatch for non-structural targeting rules; **34**
  validators remain, and every validated effect (bar the two equip/attach ones) also declares a
  non-NONE structural `targetSpec()`.

### The ratchet became a PERMANENT invariant guard (brief item 1)

The old `TargetSpecRatchetTest` counted per-file legacy-override counts against
`targetspec-baseline.txt` and failed on any increase — but the legacy methods are deleted, so it can
never fire again. It was rewritten (same file) into the permanent guard the program was building
toward, asserting two cheap static invariants (two `@Test` methods):

1. **No reintroduction of a legacy targeting method** — no file under `model/effect/` may declare a
   method named `canTargetPlayer` / `canTargetPermanent` / … (the eleven), matched by the same
   `public <boolean|int|PermanentPredicate> <name>() {` override regex the audit always used. The two
   record-component duals (`ChangeColorTextEffect.canTargetSpell`,
   `PutCounterOnTargetPermanentEffect.targetPredicate`) are components, not method bodies, so they do
   NOT match — they keep their accessors.
2. **Every `@ValidatesTarget` effect declares a non-NONE `targetSpec()`** — a validator is now only an
   escape hatch for non-structural rules, so a validated effect must STILL declare its structural spec
   (a `targetSpec()` body using a `benign(`/`harmful(` factory or a `TargetCategory` other than `NONE`;
   a conditional body with one non-NONE branch counts). Prevents a validator-only effect that bypasses
   the declarative interpreter. **Exempt:** `StaticBoostEffect`, `AttachedBoostEffect` — they target
   through the equip/attach mechanism, not the single-target pipeline, so they carry no `targetSpec()`
   category by design (the only two `@ValidatesTarget` effects with a NONE spec).

`scripts/targetspec-audit.py` was rewritten in lockstep from a baseline/matrix GENERATOR into an
invariant CHECKER of the exact same two invariants (same legacy-method set, same override regex, same
brace-matched non-NONE `targetSpec()` detection, same two exempt effects). A lockstep-contract comment
sits in BOTH files. The audit prints a report and exits non-zero on violation. It was run:
`INVARIANT 1 OK`, `@ValidatesTarget effects: 34`, `INVARIANT 2 OK` — both hold.

**Scaffolding deleted:** `targetspec-baseline.txt` (the new test no longer reads it) and
`TARGETSPEC_MATRIX.md` (the audit no longer generates it — leaving a "Generated by…" file it no longer
writes would be dishonest). Both were fully consumed by the migration; this progress log is the
permanent historical record, and git history preserves them. `effect-dispatch-baseline.txt` and
`EFFECT_COUPLING_MATRIX.md` were NOT changed (the OTHER ratchet — re-ran
`python scripts/effect-coupling-audit.py`, diff empty, no instanceof-on-effect counts changed).

### Tests (brief item 2)

- `python scripts/effect-coupling-audit.py` re-run — baseline byte-unchanged.
- `com.github.laxika.magicalvibes.architecture.TargetSpecRatchetTest` — **PASS** (2 tests: both
  invariants).
- `com.github.laxika.magicalvibes.architecture.EffectDispatchRatchetTest` — **PASS** (1 test).
Both via `scripts/run-card-test.ps1`.

### Documentation sweep (brief item 3)

Grepped `agent-docs/` + `.claude/skills/` for `canTarget` / `ValidatesTarget` / `targetPredicate` /
`isDamageOrDestruction` / `EffectResolution` / `isSelfTargeting` / `requiredPlayerTargetCount`. Rewrote
every place that TEACHES the old mechanism (how to declare targeting / when a validator is needed):
- `agent-docs/EFFECTS_INDEX.md` — the primary "new effect" doc: rewrote the add-effect steps, the
  `targetPredicate`/`isSelfTargeting`/`targetSpec` sections, and the whole "Effect targeting
  declarations" section into the `TargetSpec` mechanism with the **`TargetCategory` table**, a
  **worked example (harmful CREATURE)**, and the `harmful`-flag rule; reframed the target-validator map
  as the escape hatch and dropped the already-deleted `BounceTargetValidators` row (added
  `PreventionTargetValidators`).
- `agent-docs/ARCHITECTURE.md` — the "new effect" checklist item (3) now teaches `targetSpec()` +
  escape-hatch-only validators.
- `agent-docs/CARD_IMPLEMENTATION_PLAYBOOK.md` — the targeting checklist, the add-effect block, and the
  validator step.
- `agent-docs/TRIGGER_SLOT_TARGETING.md` — the routing invariant, the "Spell / activated-ability target
  validation" section (the Fireball-burns-a-Plains section), the capability-matrix predicate column, the
  upkeep/self-leaves/enters/block collector rows, the opponent-filter note, the effect-level-predicate
  section, and the pitfalls — all repointed from `canTarget*`/`targetPredicate()` to `targetSpec()`
  accessors (`category().includesPermanents()`/`includesPlayers()`/`isGraveyard()`,
  `EffectResolution.targetPredicateOf`).
- `agent-docs/ACTIVATED_ABILITY_GUIDE.md` — the two "override canTargetX" instructions, the counter-spell
  note, and the `UPKEEP_TRIGGERED` any-target/multi-player row.
- `agent-docs/ORACLE_TEXT_EFFECT_MAP.md` — the upkeep any-target routing note (the one mechanism line).
- `agent-docs/EFFECTS_QUICK_REFERENCE.md` — the "Targeting rules (summary)" block.
- `.claude/skills/implement-card/SKILL.md` — step 3 of the effect-add workflow.

**Scope note (open-debt below):** the per-effect CATALOG rows in `EFFECTS_INDEX.md`,
`EFFECTS_QUICK_REFERENCE.md`, `ORACLE_TEXT_EFFECT_MAP.md`, and `CARD_PATTERNS_*.md` use `canTargetPlayer`
/ `canTargetPermanent` / `canTargetGraveyard` / `canTargetExile` as a DESCRIPTIVE per-effect tag ("this
effect targets a player"), not as an instruction to override a method. These ~100+ annotations were left
as legacy shorthand — the rewritten teaching sections above now make clear the mechanism is
`targetSpec()` and the legacy booleans were deleted, so a reader is not misled into overriding a
non-existent method.

### The 34 permanent escape-hatch validators (brief item 4) — why each stays

Each keeps its `@ValidatesTarget` because it encodes logic BEYOND target-type structure that the
declarative interpreter cannot express; every one also declares its structural `targetSpec()` (invariant
2), except the two equip/attach validators which have no pipeline target.

- **Damage (3)** — `DealDamageToTargetOpponentOrPlaneswalkerEffect` (opponent-relation +
  controller-compare: rejects the controller as a player target), `DealDividedDamageEffect` (null-target
  tolerance — CHOSEN/ETB targets ride `damageAssignments`, not `targetId`), `DealDamageToPlayersEffect`
  (per-recipient `requireTargetPlayer` guard the no-op PLAYER category can't reproduce).
- **Prevention (1)** — `PreventDividedDamageEffect` (null-target tolerance, same CHOSEN pattern).
- **Destruction (2)** — `SacrificePermanentsEffect` (no-op-PLAYER `requireTargetPlayer` guard),
  `DestroyCreatureBlockingThisEffect` (combat-state: `isBlocking()` + blocking-this check).
- **CreatureMod (2)** — `StaticBoostEffect`, `AttachedBoostEffect` (target through equip/attach, NOT the
  single-target pipeline; no `targetSpec()` category — the two invariant-2 exemptions).
- **Exile (1)** — `ReturnTargetCardFromExileToHandEffect` (validates a card in the EXILE zone + the
  effect's own card filter; EXILE_CARD is a no-op interpreter category).
- **PermanentControl (3)** — `GainControlOfTargetAuraEffect` (aura attachment-state: `isAttached()`),
  `TargetPlayerGainsControlOfSourceCreatureEffect` (no-op-PLAYER guard),
  `PutTargetOnTopOfLibraryEffect` (its reader was repointed to `targetSpec()` in step 10; now REDUNDANT
  with its per-scope PERMANENT spec but kept whole per KEPT-VALIDATORS-ARE-KEPT-WHOLE — a redundant
  structural re-check is harmless).
- **Life (5)** — `TargetPlayerLosesLifeAndControllerGainsLifeEffect`, `LoseLifeEffect`,
  `TargetPlayerGainsLifeEffect`, `GivePoisonCountersEffect`, `DrainLifePerControlledPermanentEffect`
  (all no-op-PLAYER `requireTargetPlayer` guards).
- **Library (4)** — `MillEffect`, `RevealTopCardOfLibraryEffect`, `ChooseCardNameAndExileFromZonesEffect`,
  `MillBottomOfTargetLibraryConditionalTokenEffect` (all no-op-PLAYER `requireTargetPlayer` guards).
- **Graveyard (13)** — `ReturnCardFromGraveyardEffect`,
  `PutCreatureFromOpponentGraveyardOntoBattlefieldWithExileEffect`,
  `CastTargetInstantOrSorceryFromGraveyardEffect`,
  `GrantTargetCreatureCardGraveyardCastAndCopyActivatedAbilitiesEffect`,
  `GrantFlashbackToTargetGraveyardCardEffect`, `ExileTargetCardFromGraveyardAndImprintOnSourceEffect`,
  `ExileTargetCardFromGraveyardAndCreateTokenCopyEffect`, `ExileGraveyardCardsEffect`,
  `ExileTargetCardFromGraveyardMayPlayUntilNextTurnEffect`,
  `PlayTargetCardFromGraveyardWithoutPayingManaCostEffect`,
  `ExileTargetInstantOrSorceryFromOpponentGraveyardMayCastEffect`,
  `ExileTargetGraveyardCardAndSameNameFromZonesEffect`, `PutCardFromOpponentGraveyardOntoBattlefieldEffect`
  — the graveyard categories are TOTAL no-ops in the interpreter (a graveyard card is not a battlefield
  permanent, so the predicate/harmful block early-returns), so every graveyard validator's real work
  (zone check + card-type/predicate + own/opponent controller-compare) has no spec correlate and is kept
  whole.
...
- **`effect-coupling-audit.py` validator-coverage-gap = 221** (informational matrix output, not a
  ratchet): expected — the migration replaced per-effect structural validators with the declarative spec
  interpreter, so most targeted types now have no hand validator by design.

### Suggested final verification (run yourself — I did not run the full suite)

Fixed regression + guard set (via `scripts/run-card-test.ps1 <FQN>`):
`com.github.laxika.magicalvibes.architecture.TargetSpecRatchetTest`,
`com.github.laxika.magicalvibes.architecture.EffectDispatchRatchetTest`,
`com.github.laxika.magicalvibes.service.effect.TargetValidationServiceSpecTest`,
`com.github.laxika.magicalvibes.service.target.ValidTargetServiceTest`,
`com.github.laxika.magicalvibes.service.target.TargetLegalityServiceTest`,
`com.github.laxika.magicalvibes.architecture.CardEffectTargetingConsistencyTest`,
`FireballTest`, `WrackWithMadnessTest`, `DarkNourishmentTest`, and a spell/dual/graveyard spread —
`GlamerdyeTest`, `MindBendTest`, `AncientAnimusTest`, `SnapcasterMageTest`, `SurgicalExtractionTest`,
`AxisOfMortalityTest`, `AethertowTest`, `HarmsWayTest`, `RemedyTest`.
Then the **AI simulator + calibrated AI** tests PROGRESS.md flags as rollout-cost-sensitive
(see the `mcts-iteration-budget-sensitivity` note — measure before trusting a flip):
`com.github.laxika.magicalvibes.ai.GameSimulatorTest`, `AiTargetSelectorTest`, `AiDecisionEngineTest`,
and the calibrated hard-AI tests. Full suite: ask the user to run it (per CLAUDE.md, never run it here).
