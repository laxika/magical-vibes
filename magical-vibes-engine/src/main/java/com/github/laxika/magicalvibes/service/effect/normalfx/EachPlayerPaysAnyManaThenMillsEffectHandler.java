...
/** Resolves the controller-first mana-payment and shared-mill sequence used by Shared Trauma. */
@Component
@RequiredArgsConstructor
public class EachPlayerPaysAnyManaThenMillsEffectHandler implements NormalEffectHandlerBean {
    ...
    public Class<? extends CardEffect> handledEffect() {
        return EachPlayerPaysAnyManaThenMillsEffect.class;
    }
    ...
    private void promptPlayer(GameData gameData, UUID playerId, String cardName) {
        ...
        interactionHandlerRegistry.begin(gameData, new PendingInteraction.XValueChoice(
                ...
                "Pay any amount of mana for " + cardName
                        + ". Each player will mill cards equal to the total mana paid.",
                cardName, true));
    }
    ...
    private void finish(GameData gameData) {
        ...
        if (totalManaPaid > 0) {
            for (UUID playerId : state.order) {
                graveyardService.resolveMillPlayer(gameData, playerId, totalManaPaid);
            }
        }
        state.reset();
    }
