...
public class PayXManaRevealCardsChooseOneToDiscardEffectHandler implements NormalEffectHandlerBean {
    ...
    public Class<? extends CardEffect> handledEffect() {
        return PayXManaRevealCardsChooseOneToDiscardEffect.class;
    }
    ...
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        ...
        if (gameData.chosenXValue != null) {
            ...
            playerInteractionSupport.beginRevealCardsChooseDiscard(gameData, entry, chosenValue, 1);
            return;
        }
        ...
        if (maxPotentialX(gameData, controllerId) <= 0) {
            ...
            log.info("Game {} - {} has no mana for {}'s pay-X discard ability",
    ...
    private void beginXPrompt(GameData gameData, UUID controllerId, String cardName) {
        ...
        interactionHandlerRegistry.begin(gameData,
                new PendingInteraction.XValueChoice(
                        controllerId,
                        maxX,
                        "You may pay {X} for " + cardName
                                + ". Choose X (0 = don't pay). The damaged player reveals X cards.",
                        cardName,
                        true,
                        "{X}"));
    }
