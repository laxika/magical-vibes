 ...
 * them onto the battlefield untapped, then shuffle. The per-player picks are queued on the shared
...
public class EachPlayerMaySearchLibraryForBasicLandsToBattlefieldEffectHandler implements NormalEffectHandlerBean {
    ...
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        int count = ((EachPlayerMaySearchLibraryForBasicLandsToBattlefieldEffect) effect).count();
        if (count <= 0) {
            return;
        }

        List<LibrarySearchFollowUp.BasicLandsPick> picks = basicLandSearchQueueSupport.apnapOrder(gameData).stream()
                .map(playerId -> new LibrarySearchFollowUp.BasicLandsPick(playerId, count))
                .toList();
