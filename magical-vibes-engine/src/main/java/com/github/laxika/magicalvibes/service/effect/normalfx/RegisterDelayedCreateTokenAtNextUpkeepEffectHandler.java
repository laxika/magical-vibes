...
public class RegisterDelayedCreateTokenAtNextUpkeepEffectHandler implements NormalEffectHandlerBean {
    ...
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        ...
        var tokenEffect = e.tokenEffect().amount() instanceof EventValue
                ? e.tokenEffect().withAmount(entry.getEventValue())
                : e.tokenEffect();
        gameData.queueDelayedAction(new DelayedCreateTokenAtNextUpkeep(
                entry.getControllerId(), tokenEffect, entry.getCard(), e.anyPlayerNextUpkeep()));
