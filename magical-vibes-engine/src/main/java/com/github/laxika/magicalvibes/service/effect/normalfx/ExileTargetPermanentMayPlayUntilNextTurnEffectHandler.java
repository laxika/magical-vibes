    ...
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        ...
        if (target == null) {
            gameBroadcastService.logAndBroadcast(gameData,
                    entry.getCard().getName() + " fizzles (target no longer on the battlefield).");
            return;
        }
        ...
        if (ownerId != null) {
            ...
            gameBroadcastService.logAndBroadcast(gameData,
                    exiledCard.getName() + " is exiled — " + ownerName
                            + " may play it until the end of their next turn.");
        } else {
            gameBroadcastService.logAndBroadcast(gameData, exiledCard.getName() + " is exiled.");
        }
