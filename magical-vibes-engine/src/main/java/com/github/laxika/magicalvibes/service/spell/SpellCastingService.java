...
public class SpellCastingService {
    ...
    private record BeholdPowerSnapshot(UUID permanentId, int power) {}
    ...
    private void validateCasualtyPayments(GameData gameData, Player player, Card card,
                                          List<Integer> casualtyValues, List<UUID> casualtyCreatureIds,
                                          UUID sacrificePermanentId, List<UUID> convokeCreatureIds,
        ...
        if (casualtyCreatureIds.stream().anyMatch(Objects::isNull)
                || casualtyCreatureIds.stream().distinct().count() != casualtyCreatureIds.size()) {
            throw new IllegalStateException("Casualty requires different creatures");
        }
        Set<UUID> alreadyUsed = new HashSet<>();
        if (sacrificePermanentId != null) alreadyUsed.add(sacrificePermanentId);
        alreadyUsed.addAll(convokeCreatureIds);
        alreadyUsed.addAll(conspireCreatureIds);
        alreadyUsed.addAll(alternateCostSacrificePermanentIds);
        alreadyUsed.addAll(imposedSacrificePermanentIds);
        alreadyUsed.addAll(additionalCostSacrificePermanentIds);
        if (casualtyCreatureIds.stream().anyMatch(alreadyUsed::contains)) {
            throw new IllegalStateException("A casualty creature cannot pay another cast cost");
        }
        if (!gameQueryService.canSacrificeCreaturesForCosts(gameData)) {
            throw new IllegalStateException("You can't sacrifice creatures to pay costs");
        }
        for (int i = 0; i < casualtyCreatureIds.size(); i++) {
            int requiredPower = casualtyValues.get(i);
            UUID creatureId = casualtyCreatureIds.get(i);
            additionalSpellCostService.validateSingleSacrificeCost(gameData, player, card, creatureId,
                    "a creature with power " + requiredPower,
                    permanent -> gameQueryService.isCreature(gameData, permanent)
                            && gameQueryService.getEffectivePower(gameData, permanent) >= requiredPower);
        }
    }

    private void payCasualtyCosts(GameData gameData, Player player, Card card,
                                  List<Integer> casualtyValues, List<UUID> casualtyCreatureIds) {
        for (int i = 0; i < casualtyCreatureIds.size(); i++) {
            int requiredPower = casualtyValues.get(i);
            paySingleSacrificeCost(gameData, player, card, casualtyCreatureIds.get(i),
                    "a creature with power " + requiredPower,
                    permanent -> gameQueryService.isCreature(gameData, permanent)
                            && gameQueryService.getEffectivePower(gameData, permanent) >= requiredPower);
        }
    ...
    /** Pays the cost by choosing a controlled creature or revealing a creature card from hand. */
    private int payChooseCreatureOrRevealCreatureCardCost(
            GameData gameData, Player player, Card card,
            ChooseCreatureOrRevealCreatureCardCost cost,
            AdditionalSpellCostService.CostSelection selection) {
        if (cost == null) return 0;
        AdditionalSpellCostService.CreatureOrRevealedCardChoice choice =
                additionalSpellCostService.validateChooseCreatureOrRevealCreatureCardCost(
                        gameData, player, card, selection);
        if (choice.permanentId() != null) {
            Permanent chosen = gameQueryService.findPermanentById(gameData, choice.permanentId());
            gameLogService.append(gameData, GameLog.builder()
                    .text(player.getUsername() + " chooses ")
                    .card(chosen.getCard())
                    .text(" to cast ")
                    .card(card)
                    .text(".")
        ...
        gameLogService.append(gameData, GameLog.builder()
                ...
                .text(".")
                .build());
        return choice.card().getPower() == null ? 0 : Math.max(0, choice.card().getPower());
    }
    ...
    /** Pays the optional additional cost that exiles selected matching cards from hand. */
    private void payExileAnyNumberOfCardsFromHandCost(
        ...
        if (cost == null) return;
        List<Integer> effectiveIndices = new ArrayList<>(
                additionalSpellCostService.validateExileAnyNumberOfCardsFromHandCost(
                        gameData, player, card, cost, exileHandCardIndices, spellCardIndex));
        effectiveIndices.sort(java.util.Collections.reverseOrder());
        UUID playerId = player.getId();
        List<Card> hand = gameData.playerHands.get(playerId);
        for (int effectiveIndex : effectiveIndices) {
            Card toExile = hand.remove(effectiveIndex);
            gameData.addToExile(playerId, toExile);
            gameLogService.append(gameData, GameLog.builder()
                    .text(player.getUsername() + " exiles ")
                    .card(toExile)
                    .text(" from hand to cast ")
                    .card(card)
                    .text(".")
                    .build());
        }
    ...
    private int resolveCastTimeXValue(GameData gameData, Card card, UUID controllerId, int announcedXValue) {
        ...
        int effectiveXValue = announcedXValue;
        if (declaredCastTimeAmount != null) {
            effectiveXValue = amountEvaluationService.evaluate(gameData, declaredCastTimeAmount,
            ...
            if (card.getParsedManaCost() != null && card.getParsedManaCost().hasX()
                ...
                effectiveXValue = amountEvaluationService.evaluate(gameData, distribution.total(),
                        com.github.laxika.magicalvibes.service.effect.AmountContext.forCasting(
                                controllerId, announcedXValue, card));
            }
        }
    ...
    private int unwrapChooseOneEffect(GameData gameData, UUID controllerId, Card card,
        ...
        return unwrapChooseOneEffect(gameData, controllerId, card, effects, effectiveXValue,
                false, false, false);
    }

    private int unwrapChooseOneEffect(GameData gameData, UUID controllerId, Card card,
                                      List<CardEffect> effects, int effectiveXValue,
                                      boolean kicked, boolean collectEvidenceCostPaid, boolean teamworkCostPaid) {
        for (int i = 0; i < effects.size(); i++) {
            if (effects.get(i) instanceof ChooseOneEffect coe) {
                boolean additionalModesAllowed = coe.additionalModesCondition() == null
                        ...
                        com.github.laxika.magicalvibes.service.effect.ConditionContext.forCasting(
                                controllerId, kicked, collectEvidenceCostPaid).withTeamworkCostPaid(teamworkCostPaid));
                ...
                effects.remove(i);
                int insertAt = i;
                for (List<CardEffect> modeEffects : chosenModeEffects) {
                    effects.addAll(insertAt, modeEffects);
                    insertAt += modeEffects.size();
                }

                if (card.getRoomDoorManaCosts().size() == 2 && chosenModeIndices.size() == 1) {
                    card.setSelectedRoomDoor(chosenModeIndices.getFirst());
                }
                ...
                if (!useTargetSlots && chosenModes.size() == 1) {
                    ...
                    List<CardEffect> chosenEffects = chosenModeEffects.getFirst();
                    if (chosen.targetFilters() != null) {
                        for (int t = 0; t < chosen.targetFilters().size(); t++) {
                            ...
                            if (t < chosenEffects.size()) {
                                card.registerEffectTargetIndex(chosenEffects.get(t), spellTarget.getIndex());
                            }
                        }
                    ...
                    for (int modeIndex = 0; modeIndex < chosenModes.size(); modeIndex++) {
                        ChooseOneEffect.ChooseOneOption chosen = chosenModes.get(modeIndex);
                        List<CardEffect> modeEffects = chosenModeEffects.get(modeIndex);
                        if (chosen.targetFilters() != null) {
                            for (int t = 0; t < chosen.targetFilters().size(); t++) {
                                SpellTarget spellTarget = card.target(chosen.targetFilters().get(t));
                                if (t < modeEffects.size()) {
                                    card.registerEffectTargetIndex(modeEffects.get(t), spellTarget.getIndex());
                                }
                            }
                        } else if (chosen.targetFilter() != null
                                || needsFlatModalTargetGroup(chosen.effects())) {
                            SpellTarget spellTarget = declareModeTarget(card, chosen);
                            for (CardEffect modeEffect : modeEffects) {
    ...
    private static void validateOptionalCostModalSelection(
        ...
        if (additionalCosts.putCounterCost() != null && additionalCosts.putCounterCost().optional()) {
            optionalCostPaid = costSelection.sacrificePermanentId() != null;
        } else if (additionalCosts.teamworkCost() != null) {
            optionalCostPaid = costSelection.sacrificePermanentIds() != null
                    && !costSelection.sacrificePermanentIds().isEmpty();
        } else {
            return;
        }
    ...
    private void validateGraveyardReturnTargets(
            GameData gameData, UUID playerId, Card card,
            ReturnTargetCardsFromGraveyardToHandEffect effect, boolean kicked, boolean giftPromised) {
        int requiredTargets = effect.minTargets();
        ...
        if (targetGroupIndex >= 0 && targetGroupIndex < card.getSpellTargets().size()) {
            ...
            if (targetGroup.getFilter() instanceof GraveyardCardPredicateTargetFilter) {
                int groupMinTargets = giftPromised
                        ? targetGroup.getGiftPromisedMinTargets()
                        : kicked ? targetGroup.getKickedMinTargets() : targetGroup.getMinTargets();
                requiredTargets = Math.max(requiredTargets, groupMinTargets);
            }
        }
        if (requiredTargets <= 0 && !effect.requireSharedCreatureType()) {
            return;
        }
        List<Card> matchingCards = gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream()
                .filter(graveyardCard -> predicateEvaluationService.matchesCardPredicate(
                        graveyardCard, effect.filter(), null))
                .toList();
        if (matchingCards.size() < requiredTargets) {
            throw new IllegalStateException("The chosen mode requires " + requiredTargets
    ...
    private StackEntryType cardTypeToStackEntryType(Card card) {
        if (!card.getType().isPermanentType()) {
            return card.getAdditionalTypes().stream()
                    .filter(CardType::isPermanentType)
                    .findFirst()
                    .map(this::cardTypeToStackEntryType)
                    .orElseGet(() -> cardTypeToStackEntryType(card.getType()));
        }
        return cardTypeToStackEntryType(card.getType());
    }
    ...
    public void playCardWithGift(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        try {
            playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments,
                    ...
                    null, false, null, false, giftPromised, null);
        } catch (IllegalArgumentException | IllegalStateException e) {
            restoreAttemptedCardAfterFailedCast(gameData, hand, attempted, cardIndex);
            throw e;
        }
    ...
    public void playCard(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId,
        ...
        playCard(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds, convokeCreatureIds,
                ...
                additionalCostSacrificePermanentIds, repeatedAdditionalCosts, buyback, beholdPermanentId,
                beholdHandCardIndex, beholdPermanentIds, beholdHandCardIndices, beholdChosenSubtype,
                chosenCreatureType, waterbendPaid, null, null);
    }
    ...
    public void playCard(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId, Map<UUID, Integer> damageAssignments,
        ...
        try {
            playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds,
                    ...
                    beholdChosenSubtype, chosenCreatureType, waterbendPaid, payLifeForAdditionalCost,
                    false, false, chosenAdditionalCostObjectId);
        } catch (IllegalArgumentException | IllegalStateException e) {
            restoreAttemptedCardAfterFailedCast(gameData, hand, attempted, cardIndex);
            throw e;
        }
    ...
    public void playCardWithAlternateCost(GameData gameData, Player player, int cardIndex, Integer xValue,
                                          UUID targetId, Map<UUID, Integer> damageAssignments, List<UUID> targetIds,
                                          Integer handCardIndex, UUID sacrificePermanentId) {
        List<Card> hand = gameData.castingSourceCards(player.getId());
        Card attempted = hand != null && cardIndex >= 0 && cardIndex < hand.size() ? hand.get(cardIndex) : null;
        try {
            if (attempted != null && attempted.getCastingOption(AdventureCast.class).isPresent()) {
                if (player.getId().equals(gameData.commandCastPlayerId)) {
                    playCardWithAdventure(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds, sacrificePermanentId);
                    return;
                }
                playAdventureCardInternal(gameData, player, cardIndex, xValue, targetId, targetIds, null,
            ...
            if (attempted != null && attempted.getKeywords().contains(Keyword.PLOT)) {
                if (player.getId().equals(gameData.commandCastPlayerId)) throw new IllegalStateException("Plot is only available from hand");
    ...
    public void playCardWithAdventure(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        try {
            ...
            playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments,
                    targetIds != null ? targetIds : List.of(), List.of(), false, sacrificePermanentId, null, List.of(),
    ...
    private void playOmenCard(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        entry.setSourceZone(gameData.castingSourceZone(playerId));
        gameData.stack.add(entry);
        finishSpellCast(gameData, playerId, player, hand, physicalCard);
    }
    ...
    public void playAdventureCard(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        try {
            playAdventureCardInternal(gameData, player, cardIndex, xValue, targetId, targetIds,
                    damageAssignments, sacrificePermanentId);
        } catch (IllegalArgumentException | IllegalStateException e) {
            restoreAttemptedCardAfterFailedCast(gameData, hand, attempted, cardIndex);
            throw e;
        }
    ...
    private void playAdventureCardInternal(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        List<UUID> declaredTargetIds = targetIds != null ? targetIds : List.of();
        List<CardEffect> effects = new ArrayList<>(adventureCard.getEffects(EffectSlot.SPELL));
        AdditionalSpellCostService.ExtractedCosts additionalCosts =
                additionalSpellCostService.extractAndRemove(gameData, playerId, adventureCard, effects);
        AdditionalSpellCostService.CostSelection costSelection = new AdditionalSpellCostService.CostSelection(
                sacrificePermanentId, null, null, null, null, 0, cardIndex, List.of());
        additionalSpellCostService.validateAll(
                gameData, player, adventureCard, additionalCosts, costSelection, effectiveXValue);
        DistributeCountersAmongTargetsEffect counterDistribution = findChosenCounterDistribution(effects);
        Map<UUID, Integer> counterAssignments = counterDistribution == null
                ? Map.of()
                : validateCounterAssignments(gameData, adventureCard, playerId, effectiveXValue,
                        counterDistribution, damageAssignments);
        ...
        if (counterDistribution != null && (targetId != null || !declaredTargetIds.isEmpty())) {
            throw new IllegalStateException("Counter assignments must be supplied as damage assignments");
        } else if (counterDistribution != null) {
            declaredTargetIds = List.of();
        } else if (!declaredTargetIds.isEmpty()) {
        ...
        if (counterDistribution == null && targetId == null && declaredTargetIds.isEmpty()
                && adventureCard.getEffectiveMinTargets(effectiveXValue) > 0) {
            throw new IllegalStateException("Spell requires a target");
        }
        ...
        AdditionalCostPayment additionalCostPayment = payAdditionalCosts(
                gameData, player, adventureCard, additionalCosts, costSelection, effectiveXValue);
        effectiveXValue = additionalCostPayment.resolvedXValue();
        int manaSpent = gameData.getSpellCastManaSpent(adventureCard.getId());
        gameData.clearSpellCastManaSpent(adventureCard.getId());
        gameData.addSpellCastManaSpent(physicalCard.getId(), manaSpent);
        hand.remove(cardIndex);
        ...
        if (counterDistribution != null) {
            entry = new StackEntry(entryType, physicalCard, playerId, adventureCard.getName(), effects,
                    effectiveXValue, null, counterAssignments);
        } else if (!declaredTargetIds.isEmpty()) {
            entry = new StackEntry(entryType, physicalCard, playerId, adventureCard.getName(), effects,
    ...
    public void playAdventureCardFromGraveyard(GameData gameData, Player player, int graveyardCardIndex,
    ...
    private void playAdventureCardFromGraveyardInternal(GameData gameData, Player player, List<Card> graveyard,
                                                        int graveyardCardIndex, Integer xValue, UUID targetId,
                                                        List<UUID> targetIds,
                                                        Map<UUID, Integer> damageAssignments) {
        if (gameData.status != GameStatus.RUNNING) {
            throw new IllegalStateException("Game is not running");
        ...
        if (castingPermissionService.isSpellCastingRestrictedByMostRecentSpell(gameData, adventureCard)
                ...
                || !gameQueryService.canCastSpellFromZone(gameData, adventureCard, Zone.GRAVEYARD, playerId)) {
            throw new IllegalStateException("Card is not playable");
        }
        if (!castingPermissionService.canCastWithSpellTimingRestriction(gameData, playerId, adventureCard)) {
            throw new IllegalStateException("Card is not playable");
        }
        boolean isActivePlayer = playerId.equals(gameData.activePlayerId);
        boolean isMainPhase = gameData.currentStep == TurnStep.PRECOMBAT_MAIN
        ...
        paySpellManaCostFromNonHandZone(gameData, playerId, adventureCard, effectiveXValue, Zone.GRAVEYARD);
        int manaSpent = gameData.getSpellCastManaSpent(adventureCard.getId());
        gameData.clearSpellCastManaSpent(adventureCard.getId());
        gameData.addSpellCastManaSpent(physicalCard.getId(), manaSpent);
        gameData.graveyardAdventureCastPermissions.remove(physicalCard.getId());
        permanentRemovalService.removeCardFromGraveyardById(gameData, physicalCard.getId());
    ...
    /** Casts a face-down spellmorph card from the battlefield for its spellmorph cost. */
    public void playCardWithSpellmorph(GameData gameData, Player player, Permanent permanent,
                                       Integer xValue, UUID targetId, List<UUID> targetIds) {
        if (permanent == null || !permanent.isFaceDown()) {
            throw new IllegalStateException("Permanent is not face down");
        }
        Card card = permanent.getCard();
        if (!card.isSpellmorph() || (!card.hasType(CardType.INSTANT) && !card.hasType(CardType.SORCERY))) {
        ...
        for (int i = gameData.stack.size() - 1; i >= 0; i--) {
            ...
            if (candidate.getCard() != null && card.getId().equals(candidate.getCard().getId())
                    && (candidate.getEntryType() == StackEntryType.INSTANT_SPELL
                    || candidate.getEntryType() == StackEntryType.SORCERY_SPELL)) {
                spellEntry = candidate;
                break;
            }
        }
        if (spellEntry != null) {
            spellEntry.setSourceZone(Zone.BATTLEFIELD);
        }
        if (!permanentRemovalService.removePermanentToStack(gameData, permanent)) {
            throw new IllegalStateException("Spellmorph permanent is no longer on the battlefield");
        }
    }
    ...
    private static void restoreAttemptedCardAfterFailedCast(GameData gameData, List<Card> hand,
        ...
        gameData.pendingCommandCasts.remove(attempted.getId());
        UUID previousMove = gameData.completingCommanderZoneMove;
        gameData.completingCommanderZoneMove = attempted.getId();
        try { hand.add(Math.min(cardIndex, hand.size()), attempted); }
        finally { gameData.completingCommanderZoneMove = previousMove; }
    }
    ...
    private void playCardInternal(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId,
        ...
        playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds,
                ...
                waterbendPaid, payLifeForAdditionalCost, adventure, false, null);
    }
    ...
    private void playCardInternal(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId, Map<UUID, Integer> damageAssignments,
                  ...
                  UUID beholdPermanentId, Integer beholdHandCardIndex,
                  List<UUID> beholdPermanentIds, List<Integer> beholdHandCardIndices,
                  CardSubtype beholdChosenSubtype, CardSubtype chosenCreatureType, boolean waterbendPaid, Boolean payLifeForAdditionalCost, boolean adventure, boolean giftPromised, UUID chosenAdditionalCostObjectId) {
        playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds, convokeCreatureIds, fromGraveyard, sacrificePermanentId, phyrexianLifeCount, alternateCostSacrificePermanentIds, exileGraveyardCardIndex, exileGraveyardCardIndices, kicked, discardHandCardIndex, discardHandCardIndices, forceAlternateCost, conspireCreatureIds, modalXValue, spliceHandCardIndices, imposedSacrificePermanentIds, additionalCostSacrificePermanentIds, repeatedAdditionalCosts, buyback, overloaded, spliceCostPermanentIds, sharedColorDiscardHandCardIndex, beholdPermanentId, beholdHandCardIndex, beholdPermanentIds, beholdHandCardIndices, beholdChosenSubtype, chosenCreatureType, waterbendPaid, payLifeForAdditionalCost, adventure, giftPromised, chosenAdditionalCostObjectId, List.of());
    }
    ...
    private void playCardInternal(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId, Map<UUID, Integer> damageAssignments,
                  ...
                  UUID beholdPermanentId, Integer beholdHandCardIndex,
                  List<UUID> beholdPermanentIds, List<Integer> beholdHandCardIndices,
                  CardSubtype beholdChosenSubtype, CardSubtype chosenCreatureType, boolean waterbendPaid, Boolean payLifeForAdditionalCost, boolean adventure, boolean giftPromised, UUID chosenAdditionalCostObjectId, List<UUID> casualtyCreatureIds) {
        ManaPool casterPool = gameData.playerManaPools.get(player.getId());
        boolean previousPermission = casterPool != null
                && casterPool.isInstantSorceryOrClassLevelManaUsableForInstantSorcery();
        boolean previousUnionKickedPermission = casterPool != null
                && casterPool.isKickedOrInstantSorceryOnlyManaUsableForKickedSpell();
        boolean previousUnionInstantPermission = casterPool != null
                && casterPool.isKickedOrInstantSorceryOnlyManaUsableForInstantSorcery();
        boolean instantOrSorcery = false;
        if (!fromGraveyard) {
            List<Card> hand = gameData.castingSourceCards(player.getId());
            if (hand != null && cardIndex >= 0 && cardIndex < hand.size()) {
                Card candidate = hand.get(cardIndex);
                if (candidate.isModalDoubleFaced() && candidate.getBackFaceCard() != null
                        && xValue != null && xValue == 1) {
                    candidate = candidate.getBackFaceCard();
                }
        ...
        if (casterPool != null && (kicked || instantOrSorcery)) {
            if (kicked) {
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForInstantSorcery(false);
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(true);
            } else {
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(false);
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForInstantSorcery(true);
            }
        }
        if (casterPool != null && instantOrSorcery) {
            casterPool.setInstantSorceryOrClassLevelManaUsableForInstantSorcery(true);
        }
        try {
            playCardInternalImpl(gameData, player, cardIndex, xValue, targetId, damageAssignments,
                    targetIds, convokeCreatureIds, fromGraveyard, sacrificePermanentId, phyrexianLifeCount,
                    alternateCostSacrificePermanentIds, exileGraveyardCardIndex, exileGraveyardCardIndices,
                    kicked, discardHandCardIndex, discardHandCardIndices, forceAlternateCost,
                    conspireCreatureIds, modalXValue, spliceHandCardIndices, imposedSacrificePermanentIds,
                    additionalCostSacrificePermanentIds, repeatedAdditionalCosts, buyback, overloaded,
                    spliceCostPermanentIds, sharedColorDiscardHandCardIndex, beholdPermanentId,
                    beholdHandCardIndex, beholdPermanentIds, beholdHandCardIndices, beholdChosenSubtype,
                    chosenCreatureType, waterbendPaid, payLifeForAdditionalCost, adventure, giftPromised, chosenAdditionalCostObjectId, casualtyCreatureIds);
        } finally {
            gameData.currentCastSacrificedPermanentIds.clear();
            if (casterPool != null) {
                casterPool.setInstantSorceryOrClassLevelManaUsableForInstantSorcery(previousPermission);
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(previousUnionKickedPermission);
                casterPool.setKickedOrInstantSorceryOnlyManaUsableForInstantSorcery(previousUnionInstantPermission);
            }
        }
    }

    private void playCardInternalImpl(GameData gameData, Player player, int cardIndex, Integer xValue, UUID targetId, Map<UUID, Integer> damageAssignments,
                  List<UUID> targetIds, List<UUID> convokeCreatureIds, boolean fromGraveyard, UUID sacrificePermanentId,
                  Integer phyrexianLifeCount, List<UUID> alternateCostSacrificePermanentIds, Integer exileGraveyardCardIndex,
                  List<Integer> exileGraveyardCardIndices, boolean kicked, Integer discardHandCardIndex,
                  List<Integer> discardHandCardIndices, boolean forceAlternateCost,
        ...
        if (hasSacrificeForCostReduction) {
            gameData.playerBattlefields.getOrDefault(playerId, List.of()).stream()
                    ...
                    .forEach(gameData.currentCastSacrificedPermanentIds::add);
        }
        ...
        if (!commandCast && (handCardForTiming.hasType(CardType.LAND) && !castingFaceDown
        ...
        if (commandCast && (handCardForTiming.hasType(CardType.LAND)
                || !castingPermissionService.isSpellCastingAllowed(gameData, playerId, handCardForTiming)
                || !gameQueryService.canCastSpellFromZone(gameData, handCardForTiming, Zone.COMMAND, playerId)
                || !castingPermissionService.canCastWithTiming(gameData, playerId, handCardForTiming,
                    playerId.equals(gameData.activePlayerId),
                    gameData.currentStep == TurnStep.PRECOMBAT_MAIN || gameData.currentStep == TurnStep.POSTCOMBAT_MAIN,
                    gameData.stack.isEmpty()))) throw new IllegalStateException("Commander cannot be cast now");
        List<Integer> playable = commandCast
                ? (actionAvailabilityService.isCardPlayable(gameData, playerId, handCardForTiming,
                    gameData.playerManaPools.get(playerId), 0) ? List.of(cardIndex) : List.of())
                : actionAvailabilityService.getPlayableCardIndices(gameData, playerId);
        if (!playable.contains(cardIndex)) {
            ...
            boolean suppliedExileFromHandAdditionalCost = discardHandCardIndices != null
            ...
            if (suppliedGiftTarget) {
            ...
            } else if (cardCheck.isRequiresNoMana()
                    && cardCheck.getKeywords().contains(Keyword.CONVOKE)
                    && !convokeCreatureIds.isEmpty()) {
                // Hogaak-style spells validate the selected Convoke sources and Delve cards below.
                ...
                List<UUID> suppliedTargetIds = !targetIds.isEmpty()
                        ? targetIds : targetId != null ? List.of(targetId) : List.of();
                boolean suppliedTeamworkCost = !additionalCostSacrificePermanentIds.isEmpty()
                        && cardCheck.getEffects(EffectSlot.SPELL).stream().anyMatch(TeamworkCost.class::isInstance);
                ...
                if (targetId != null
                    ...
                    targetLegalityService.validateSpellTargeting(
                            ...
                            effectiveXValue, false, giftPromised, suppliedTeamworkCost);
                }
                if (!suppliedTargetIds.isEmpty()) {
                    int targetingCostModifier = castingCostService.getTargetingSpellCostModifier(
                            gameData, playerId, cardCheck, targetId, targetIds)
                            - castingCostService.computeTargetBasedCostReduction(
                            gameData, playerId, cardCheck, suppliedTargetIds);
        ...
        filteredSpellEffects.stream()
         ...
         int modalModeCount = filteredSpellEffects.stream()
                 ...
                 .orElse(0);
         if (additionalCosts.entwineManaCost() != null) {
             ChooseOneEffect entwineModal = filteredSpellEffects.stream()
                     .filter(ChooseOneEffect.class::isInstance)
                     .map(ChooseOneEffect.class::cast)
                     .findFirst()
                     .orElseThrow(() -> new IllegalStateException(
                             "Entwine requires a modal spell: " + card.getName()));
             escalateManaSuffix += additionalSpellCostService.entwineManaSuffix(
                     card, additionalCosts.entwineManaCost(), entwineModal.choicesRequired(),
        ...
        filteredSpellEffects = EffectResolution.resolveAlternateCostEffects(filteredSpellEffects, usingAlternateCost);
        ...
        if (!usingAlternateCost && selectedModeManaCost != null) {
            ManaPool modePool = new ManaPool(gameData.playerManaPools.get(playerId));
            if (card.hasType(CardType.ENCHANTMENT)) {
                modePool.promoteEnchantmentOrRoomUnlockOrTurnFaceUpMana();
            }
            if (card.hasType(CardType.ENCHANTMENT)
                    && card.getSubtypes().contains(CardSubtype.ROOM)) {
                modePool.promoteRoomSpellsOrUnlocksMana();
            }
            ...
            ManaPool.ColoredSpellWithoutXManaState coloredSpellWithoutXMana =
                    isColoredSpellWithoutX(gameData, card)
                            ? modePool.promoteColoredSpellWithoutXOnlyMana() : null;
            try {
                ...
                if (coloredSpellWithoutXMana != null) {
                    modePool.restorePromotedColoredSpellWithoutXOnlyMana(coloredSpellWithoutXMana);
                }
            }
        }
        ...
        List<CardEffect> targetingSpellEffects = EffectResolution.resolveEffects(
                filteredSpellEffects, kicked, overloaded, null, teamworkCostPaid);
        ...
        boolean firstDeclaredGroupTargetsSpell = card.getSpellTargets().stream()
                ...
                .map(group -> targetingSpellEffects.stream()
                        ...
                        .anyMatch(EffectResolution::targetsSpellOnStack))
                .orElse(false);
        boolean variableMixedTargetGroups = firstDeclaredGroupTargetsSpell
                && card.getSpellTargets().size() == 2
                && card.getSpellTargets().getFirst().getMaxTargets() > 1
                && card.getSpellTargets().get(1).getMaxTargets() > 1
                && card.getSpellTargets().getFirst().getFilter() instanceof StackEntryPredicateTargetFilter
                && card.getSpellTargets().get(1).getFilter() instanceof PermanentPredicateTargetFilter;
        int variableMixedStackTargetCount = 0;
        if (variableMixedTargetGroups) {
            List<UUID> allTargetIds = new ArrayList<>();
            if (targetId != null) {
                allTargetIds.add(targetId);
            }
            allTargetIds.addAll(targetIds);
            List<UUID> stackTargetIds = allTargetIds.stream()
                    .filter(candidateId -> targetLegalityService.isStackEntryOnStack(gameData, candidateId))
                    .toList();
            List<UUID> permanentTargetIds = allTargetIds.stream()
                    .filter(candidateId -> !targetLegalityService.isStackEntryOnStack(gameData, candidateId))
                    .toList();
            targetIds = new ArrayList<>(stackTargetIds);
            targetIds.addAll(permanentTargetIds);
            variableMixedStackTargetCount = stackTargetIds.size();
            variableMixedTargetGroupSizes = List.of(stackTargetIds.size(), permanentTargetIds.size());
            targetId = null;
        }
        ...
        filteredSpellEffects.stream()
                ...
                .findFirst()
                .ifPresent(effect -> validateGraveyardReturnTargets(
                        gameData, playerId, card, effect, kicked && findKickerEffect(card) != null, giftPromised));
        ...
        if (usingAlternateCost) {
            if (usingWebSlingingCost) {
                ...
                castingCostService.validateSharedColorDiscardAlternativeCost(
                        gameData, playerId, card, sharedColorDiscardHandCardIndex, handCostIndex);
            ...
            if (sacCost.isPresent()) {
                if (new HashSet<>(alternateCostSacrificePermanentIds).size() != alternateCostSacrificePermanentIds.size()) {
                ...
                for (UUID sacId : alternateCostSacrificePermanentIds) {
                    ...
                    if (!gameQueryService.canSacrificePermanentForCosts(gameData, toSacrifice)) {
                        throw new IllegalStateException("Players can't sacrifice this permanent to cast the alternate cost");
                    }
            ...
            if (altCast.sneak()) {
                sneakAttackTargetId = validateSneakCast(
                        gameData, playerId, altCast, alternateCostSacrificePermanentIds);
            }
            ...
            if (revealHandCost.isPresent()) {
                validateRevealFromHandAlternateCost(gameData, playerId, card, revealHandCost.get(),
                        discardHandCardIndex, handCostIndex);
            }
            ...
            if (manaCost.isPresent()) {
                ...
                try {
                    canPay = altCast.reduceManaBySacrificedManaCost()
                            ? manaCost.get().treasureManaOnly()
                                    ? pool.canPayWithTreasureManaAfterReduction(cost, sacrificedManaCost)
                                    : cost.canPayAfterReduction(pool, sacrificedManaCost)
                            : manaCost.get().treasureManaOnly()
                                    ? pool.canPayWithTreasureMana(cost, alternateCostXArgument(cost, effectiveXValue,
                    ...
                    if (exactlyThreeColorMana != null) {
                        pool.restorePromotedExactlyThreeColorSpellOnlyMana(exactlyThreeColorMana);
                    }
        ...
        int exileFromHandCostReduction = additionalCosts.exileAnyNumberOfCardsFromHandCost() == null
                ? 0
            ...
            try {
                ...
                if (!manaCostString.isEmpty()) {
                    ...
                    if (!normalCostPayable) {
                        CastingCostService.AlternativeCostSelection alternativeCost =
        ...
        if (unwrappedNeedsSpellTarget && targetingSpellOnStack) {
            if (variableMixedTargetGroups) {
                targetLegalityService.validateVariableMixedSpellAndPermanentTargets(
                        gameData, card,
                        targetIds.subList(0, variableMixedStackTargetCount),
                        targetIds.subList(variableMixedStackTargetCount, targetIds.size()),
                        playerId, effectiveXValue, kicked, giftPromised);
            } else if (multipleSpellTargets) {
        ...
        ReturnCardFromGraveyardEffect graveyardReturnEffect = (ReturnCardFromGraveyardEffect) graveyardTargetingSource.stream()
                ...
                .findFirst().orElse(null);
        boolean hasBoundGraveyardTargetGroup = graveyardTargetingSource.stream()
                .map(SpellCastingService::unwrapConditional)
        ...
        boolean targetIsGraveyardCard = targetId != null
                && gameQueryService.findGraveyardOwnerById(gameData, targetId) != null;
        ...
        if (!castingFaceDown && targetId == null && targetIds.isEmpty()
        ...
        if (targetId != null && !targetingSpellOnStack && !castingFaceDown) {
            if (needsGraveyardOrExileTargeting) {
            ...
            } else if (needsGraveyardEffectTargeting && targetIsGraveyardCard) {
                boolean inControllersGraveyard = gameData.playerGraveyards
                        .getOrDefault(playerId, List.of())
                        .stream()
                        .anyMatch(c -> c.getId().equals(validatedTargetId));
                ...
                targetLegalityService.validateSpellTargeting(gameData, card, primaryTargetEffects,
                        ...
                        giftPromised, teamworkCostPaid);
            }
        } else if (!castingFaceDown && unwrappedNeedsTarget && needsGraveyardOrExileTargeting) {
        ...
        } else if (!castingFaceDown && unwrappedNeedsTarget && needsExileTargeting && !hasExileTargetInTargetIds) {
        ...
        } else if (!castingFaceDown && unwrappedNeedsTarget && needsSingleGraveyardTargeting && !hasGraveyardTargetInTargetIds) {
        ...
        } else if (!castingFaceDown && unwrappedNeedsTarget && needsImmediateGraveyardEffectTargeting
        ...
        if (unwrappedNeedsSpellTarget && unwrappedNeedsTarget && !targetIds.isEmpty()
                && !mixedSpellAndPermanentTargets && !variableMixedTargetGroups) {
        ...
        ReturnTargetCardsFromGraveyardToHandEffect xScaledToHandEffect =
                card.getEffects(EffectSlot.SPELL).stream()
                        ...
                        .findFirst().orElse(null);
        // Targets are chosen from the zone as it existed before paying additional costs.
        List<UUID> graveyardIdsBeforeCosts = xScaledToHandEffect == null ? List.of()
                : gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream().map(Card::getId).toList();
        if (xScaledToHandEffect != null && effectiveXValue > 0) {
            var matchingCards = gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream()
                    .filter(c -> predicateEvaluationService.matchesCardPredicate(
                            c, xScaledToHandEffect.filter(), card.getId()))
                    .toList();
            long matchingCount = card.getMultiTargetConstraint() == MultiTargetConstraint.DIFFERENT_MANA_VALUES
                    ? matchingCards.stream().map(Card::getManaValue).distinct().count()
                    : matchingCards.size();
            if (effectiveXValue > matchingCount) {
        ...
        if (buyback && buybackEffect != null && buybackEffect.hasDiscardCost()) {
            if (buybackEffect.hasRandomDiscardCost()) {
                ...
                if (gameData.playerHands.get(playerId).size() < buybackEffect.discardCount()) {
                ...
                additionalSpellCostService.validateDiscardXCardsCost(
                        ...
                        discardHandCardIndices, handCostIndex);
            }
        }
        ...
        if (card.hasType(CardType.LAND) && !castingFaceDown) {
            ...
            if (landCopyOnEnterService.prepare(gameData, playerId, card, landFace,
                    true, false, null, castSourceZone)) {
                return;
            }
            Permanent permanent = new Permanent(playingModalBackLand ? preparedCard : card);
            ...
            battlefieldEntryService.putLandOntoBattlefield(gameData, playerId, permanent, castSourceZone);
            ...
            StackEntryType permanentEntryType = castingFaceDown
                    ...
                    : cardTypeToStackEntryType(castCharacteristics);
            ...
            AdditionalSpellCostService.ChosenCreatureOrWarpedCard chosenObject =
                    additionalCosts.chosenCreatureOrWarpedCardCost() == null ? null
                            : additionalSpellCostService.validateChosenCreatureOrWarpedCard(
                                    gameData, player, card, costSelection.chosenObjectId());
            ...
            if (buyback && buybackEffect != null) {
                payBuybackCost(gameData, player, card, buybackEffect, sacrificePermanentId,
                        additionalCostSacrificePermanentIds, discardHandCardIndices, handCostIndex, preManaPaymentPool);
            }
            AdditionalCostPayment additionalCostPayment = payAdditionalCosts(
            ...
            entry.setCastWithWarp(usingWarpAlternativeCost
                    || (usingAlternateCost && card.getKeywords().contains(Keyword.WARP)));
            entry.setWebSlingingReturnedCreatureManaValue(webSlingingReturnedCreatureManaValue);
            if (sneakAttackTargetId != null) {
                entry.setSneak(true);
                entry.setEntersTapped(true);
                entry.setAttackedTargetId(sneakAttackTargetId);
            }
            ...
            entry.setSourceZone(castSourceZone);
            ...
            stampChosenObject(entry, chosenObject);
            ...
            int maximumDelveReduction = castingCostService.maximumDelveReduction(
                    ...
                    castingCostService.getCastCostModifier(
                            gameData, playerId, card, resolvedXValue, null, kicked, collectEvidenceCostPaid)
                            - targetSubtypeCostReduction - tapCreaturesCostReduction + targetingTax);
            if (additionalCosts.delveCost() != null) {
                additionalSpellCostService.validateDelveCost(gameData, player, card, additionalCosts.delveCost(),
                        costSelection.exileGraveyardCardIndices(), maximumDelveReduction);
            }
            ...
            if (additionalCosts.discardXCardsCost() != null) {
                additionalSpellCostService.validateDiscardXCardsCost(gameData, player, card,
                        additionalCosts.discardXCardsCost(), resolvedXValue, discardHandCardIndices, handCostIndex);
            }
            validateImposedSacrificeTax(gameData, player, card, imposedSacrificePermanentIds);
            if (kicked && kickerEffect != null && kickerEffect.hasLifeCost()) {
                additionalSpellCostService.validatePayLifeCost(gameData, player, card, kickerEffect.lifeCost());
            ...
            if (graveyardToHandEffect != null && shuffleGraveyardCardsEffect != null) {
                ...
                List<UUID> permanentTargetIds = new ArrayList<>(targetIds);
                if (targetId != null && gameQueryService.findPermanentById(gameData, targetId) != null
                        && !permanentTargetIds.contains(targetId)) {
                    permanentTargetIds.addFirst(targetId);
                }
                long matchingCount = gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream()
                        .filter(c -> predicateEvaluationService.matchesCardPredicate(
            ...
            } else if (graveyardToHandEffect != null && graveyardToHandEffect.upToXTargets()) {
                // "Return up to X target cards from your graveyard to your hand": X is locked
                // in during casting, while the caster may choose any number from zero through X.
                if (resolvedXValue > 0) {
                    long matchingCount = gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream()
                            .filter(c -> predicateEvaluationService.matchesCardPredicate(
                    ...
                    if (matchingCount > 0) {
                        graveyardTargetingService.handleUpToNGraveyardSpellTargeting(
                                gameData, playerId, card, entryType, graveyardToHandEffect,
                                resolvedXValue, filteredSpellEffects);
                        gameData.graveyardTargetOperation.xValue = resolvedXValue;
                        return; // finishSpellCast handled in handleMultipleCardsChosen
                    }
                }
                // X=0 or no matching cards: put the spell on the stack with zero targets.
                gameData.stack.add(new StackEntry(
                        entryType, card, playerId, card.getName(),
                ...
                if (resolvedXValue > 0) {
                    graveyardTargetingService.handleExactNGraveyardSpellTargeting(
                            ...
                            GraveyardSearchScope.CONTROLLERS_GRAVEYARD, graveyardIdsBeforeCosts);
                    return; // finishSpellCast handled in handleMultipleCardsChosen
                }
                ...
                if (targetGroupIndex >= 0 && targetGroupIndex < card.getSpellTargets().size()) {
                    SpellTarget targetGroup = card.getSpellTargets().get(targetGroupIndex);
                    if (targetGroup.getFilter() instanceof GraveyardCardPredicateTargetFilter) {
                        boolean kickedTargetGroup = kicked && kickerEffect != null;
                        graveyardMinTargets = Math.max(graveyardMinTargets,
                                giftPromised
                                        ? targetGroup.getGiftPromisedMinTargets()
                                        : kickedTargetGroup
                                        ? targetGroup.getKickedMinTargets()
                        ...
                        graveyardMaxTargets = Math.min(graveyardMaxTargets,
                                targetLegalityService.getEffectiveMaxTargetsForGroup(
                                        gameData, card, playerId, null, targetGroup,
                                        kickedTargetGroup, giftPromised));
                    }
                }
                ...
                for (Map.Entry<UUID, Integer> assignment : counterAssignments.entrySet()) {
                    ...
                    if (target == null || distributeCounters.targetRestriction() == null
                ...
                boolean hasGraveyardTarget = targetIsGraveyardCard || hasGraveyardTargetInTargetIds;
                gameData.stack.add(new StackEntry(
                        entryType, card, playerId, card.getName(),
                        filteredSpellEffects, resolvedXValue, targetId,
                        null, counterAssignments,
                        hasGraveyardTarget ? Zone.GRAVEYARD : null,
                        hasGraveyardTarget ? graveyardTargetIds(gameData, targetId, targetIds) : List.of(),
                        targetIds
                ));
                ...
                if (preventDivided != null) {
                    ...
                    DealDividedDamageEffect dividedEffect =
                            findChosenDividedDamageEffectForCast(
                                    gameData, playerId, filteredSpellEffects, resolvedXValue, kicked);
                    ...
                    if (dividedEffect != null && dividedEffect.totalDamage() instanceof Fixed fixedTotal) {
                        ...
                        for (Map.Entry<UUID, Integer> assignment : damageAssignments.entrySet()) {
                            ...
                            if (isPlayer) {
                                ...
                                Permanent target = gameQueryService.findPermanentById(gameData, assignment.getKey());
                                if (target == null || !isValidDividedDamagePermanentTarget(gameData, target,
                                        dividedEffect)) {
                                    throw new IllegalStateException("All targets must be creatures");
                                }
                                if (card.getTargetFilter() != null
                        ...
                        if (dividedEffect.maxTargets() > 0
                                && damageAssignments.size() > dividedEffect.maxTargets()) {
                            throw new IllegalStateException("Too many targets");
                        }
                        for (Map.Entry<UUID, Integer> assignment : damageAssignments.entrySet()) {
                            ...
                            if (isPlayer) {
                                ...
                                if (!isValidDividedDamagePermanentTarget(gameData, target, dividedEffect)) {
                                    throw new IllegalStateException("All targets must be creatures");
                                }
                                if (dividedEffect.targetRestriction() != null
                                        && !predicateEvaluationService.matchesPermanentPredicate(
                ...
                if (variableMixedTargetGroups) {
                    ...
                    entry.setTargetGroupSizes(variableMixedTargetGroupSizes);
                    gameData.stack.add(entry);
                } else if (multipleSpellTargets) {
                ...
                StackEntry entry = new StackEntry(
                        entryType, card, playerId, card.getName(), filteredSpellEffects, resolvedXValue,
                        null, null, Map.of(), Zone.GRAVEYARD,
                ...
                gameData.stack.add(new StackEntry(
                        entryType, card, playerId, card.getName(),
                        filteredSpellEffects, resolvedXValue, targetId, null,
                        Map.of(), targetIsGraveyardCard ? Zone.GRAVEYARD : null, List.of(), List.of()
                ));
            } else {
                gameData.stack.add(new StackEntry(
                        entryType, card, playerId, card.getName(),
            ...
            if (additionalCosts.chooseCreatureOrRevealCreatureCardCost() != null
                    && !gameData.stack.isEmpty()) {
                UUID chosenPermanentId = paymentCostSelection.beholdPermanentId();
                if (chosenPermanentId == null
                        && paymentCostSelection.beholdPermanentIds() != null
                        && paymentCostSelection.beholdPermanentIds().size() == 1) {
                    chosenPermanentId = paymentCostSelection.beholdPermanentIds().getFirst();
                }
                gameData.stack.getLast().setChosenPermanentId(chosenPermanentId);
                if (chosenPermanentId != null) {
                    gameData.stack.getLast().setTriggeringPermanentPowerAtTrigger(
                            additionalCostPayment.resolvedXValue());
                }
            }
            ...
            if (!gameData.stack.isEmpty()) {
                ...
                if (beholdPowerSnapshot != null) {
                    gameData.stack.getLast().setBeholdPower(beholdPowerSnapshot.power());
                    gameData.stack.getLast().setBeholdPermanentId(beholdPowerSnapshot.permanentId());
                }
                ...
                gameData.stack.getLast().setTeamworkCostPaid(
                        isTeamworkCostPaid(additionalCosts, paymentCostSelection));
            }
    ...
    private AdditionalSpellCostService.CostSelection adjustCostSelectionAfterSharedColorDiscard(
        ...
        return new AdditionalSpellCostService.CostSelection(
                ...
                selection.chosenObjectId(), selection.payLifeForAdditionalCost());
    }
    ...
    private BeholdPowerSnapshot snapshotBeholdPower(GameData gameData, Player player,
                                                    AdditionalSpellCostService.ExtractedCosts costs,
                                                    AdditionalSpellCostService.CostSelection selection) {
        if (costs.beholdSelectionCost() == null || !isBeholdCostPaid(costs, selection)) {
            return null;
        }
        List<UUID> permanentIds = selection.beholdPermanentIds() != null
                ? selection.beholdPermanentIds() : List.of();
        if (!permanentIds.isEmpty()) {
            Permanent permanent = gameQueryService.findPermanentById(gameData, permanentIds.getFirst());
            if (permanent != null) {
                return new BeholdPowerSnapshot(permanent.getId(), Math.max(0,
        ...
        if (!handIndices.isEmpty()) {
            int selectedIndex = handIndices.getFirst();
            int effectiveIndex = selectedIndex;
            List<Card> hand = gameData.playerHands.get(player.getId());
            if (hand != null && effectiveIndex >= 0 && effectiveIndex < hand.size()) {
                Card beheldCard = hand.get(effectiveIndex);
                return new BeholdPowerSnapshot(null,
                        beheldCard.getPower() == null ? 0 : Math.max(0, beheldCard.getPower()));
            }
        }
        return null;
    }
    ...
    private boolean controlsModifiedCreatureAsCast(GameData gameData, UUID playerId) {
        return gameData.playerBattlefields.getOrDefault(playerId, List.of()).stream()
                .anyMatch(permanent -> gameQueryService.isCreature(gameData, permanent)
                        && predicateEvaluationService.matchesPermanentPredicate(
                        gameData, permanent, new PermanentIsModifiedPredicate()));
    }
    ...
    private boolean isTeamworkCostPaid(AdditionalSpellCostService.ExtractedCosts costs,
                                       AdditionalSpellCostService.CostSelection selection) {
        return costs.teamworkCost() != null
                && selection.sacrificePermanentIds() != null
    ...
    private AdditionalCostPayment payAdditionalCosts(GameData gameData, Player player, Card card,
        ...
        paySacrificeCreatureOrDiscardCardOrPayLifeCost(gameData, player, card,
        ...
        paySacrificeFractionRoundedUpCost(gameData, player, card, costs.sacrificeFractionRoundedUpCost(),
                selection.sacrificePermanentIds());
        ...
        if (costs.tapCreaturesForManaCost() != null) {
            payTapCreaturesForManaCost(gameData, player, card, selection.sacrificePermanentIds());
        }
        ...
        if (costs.tapPermanentOrPayManaCost() != null) {
            ...
            if (selectedTapPermanentId != null) {
                tappedPermanentId = selectedTapPermanentId;
            }
        }
        if (costs.teamworkCost() != null && selection.sacrificePermanentIds() != null
                && !selection.sacrificePermanentIds().isEmpty()) {
            payTeamworkCost(gameData, player, card, costs.teamworkCost(), selection.sacrificePermanentIds());
        }
        ...
        if (costs.chooseCreatureOrRevealCreatureCardCost() != null) {
            resolvedXValue = payChooseCreatureOrRevealCreatureCardCost(
                    gameData, player, card, costs.chooseCreatureOrRevealCreatureCardCost(), selection);
        }
        ...
        return new AdditionalCostPayment(resolvedXValue, effectiveSacrificePayment.sacrificedCardId(),
                ...
                effectiveSacrificePayment.sacrificedPermanentSnapshot(),
    ...
    private ExiledCostPayment payExileCreatureCost(GameData gameData, Player player, Card card,
        ...
        Permanent toExile = additionalSpellCostService.validateSinglePermanentCost(gameData, player, card,
    ...
    private void stampChosenObject(StackEntry entry,
        ...
        entry.setChosenPermanentId(chosenObject.permanentId());
        entry.setChosenPermanentPowerAtLastKnown(chosenObject.power());
        entry.setChosenObjectCard(chosenObject.card());
    }
    ...
    private void payXLifeCost(GameData gameData, Player player, Card card, int announcedX) {
        ...
        lifeSupport.applyLifePayment(gameData, player.getId(), announcedX, card.getName());
    }
    ...
    private void payPerTargetLifeCost(GameData gameData, Player player, Card card, int amount) {
        ...
        lifeSupport.applyLifePayment(gameData, player.getId(), amount, card.getName());
    }
    ...
    private SacrificeCostPayment payAllSacrificeCosts(GameData gameData, Player player, Card card,
                                                      UUID sacrificePermanentId,
                                                      AdditionalSpellCostService.ExtractedCosts costs,
                                                      int resolvedXValue, int announcedXValue) {
        UUID sacrificedCardId = null;
        Card sacrificedCardSnapshot = null;
        Permanent sacrificedPermanentSnapshot = null;
        int sacrificedPower = 0;
        int sacrificedToughness = 0;
        if (costs.casualtyCost() != null && sacrificePermanentId != null) {
            CasualtyCost casualtyCost = costs.casualtyCost();
            int chosenXValue = casualtyCost.powerMustEqualChosenX() ? announcedXValue : casualtyCost.minimumPower();
            SacrificedCreatureStats stats = paySingleSacrificeCost(gameData, player, card,
                    sacrificePermanentId,
                    casualtyCost.powerMustEqualChosenX()
                            ? "a creature with power exactly " + chosenXValue
                            : "a creature with power " + casualtyCost.minimumPower() + " or greater",
                    p -> casualtyCost.powerMustEqualChosenX()
                            ? gameQueryService.isCreature(gameData, p)
                                    && gameQueryService.getEffectivePower(gameData, p) == chosenXValue
                            : predicateEvaluationService.matchesPermanentPredicate(
                                    gameData, p, casualtyCost.consumedPermanentFilter()));
        ...
        if (costs.sacrificeCreature()) {
            ...
            sacrificedCardId = stats.cardId();
            sacrificedCardSnapshot = stats.card();
            SacrificeCreatureCost sacCreatureCost = costs.sacrificeCreatureCost();
            ...
            if (sacCreatureCost.recordSacrificedPermanentSnapshot()) {
                sacrificedPermanentSnapshot = stats.permanentSnapshot();
            }
        }
        SacrificeCreatureOrDiscardCardOrPayLifeCost sacrificeOrDiscardOrPayLife =
                costs.sacrificeCreatureOrDiscardCardOrPayLifeCost();
        if (sacrificeOrDiscardOrPayLife != null && sacrificePermanentId != null) {
            SacrificedCreatureStats stats = paySingleSacrificeCost(gameData, player, card,
                    sacrificePermanentId, "a creature", p -> gameQueryService.isCreature(gameData, p));
        ...
        if (costs.sacrificePermanentCost() != null
                && (!costs.sacrificePermanentCost().optional() || sacrificePermanentId != null)) {
            ...
            if (sacrificedCardId == null) {
                sacrificedCardId = stats.cardId();
                sacrificedCardSnapshot = stats.card();
                if (sacPermCost.recordsSacrificedPermanentSnapshot()) {
                    sacrificedPermanentSnapshot = stats.permanentSnapshot();
                }
            }
        ...
        return new SacrificeCostPayment(resolvedXValue, sacrificedCardId, sacrificedCardSnapshot,
                sacrificedPermanentSnapshot, sacrificedPower, sacrificedToughness);
    }
    ...
    private SacrificeCostPayment payDiscardCardOrSacrificePermanentCost(
        ...
        if (cost == null) {
            return new SacrificeCostPayment(0, null, null, null, 0, 0);
        }
        if (sacrificePermanentId != null) {
            SacrificedCreatureStats stats = paySingleSacrificeCost(gameData, player, card,
                    sacrificePermanentId, cost.description(),
                    p -> predicateEvaluationService.matchesPermanentPredicate(gameData, p, cost.filter()));
            return new SacrificeCostPayment(0, stats.cardId(), stats.card(), null, 0, 0);
        }
    ...
    private void paySacrificeFractionRoundedUpCost(
            GameData gameData, Player player, Card card, SacrificeFractionRoundedUpCost cost,
            List<UUID> sacrificePermanentIds) {
        if (cost == null) {
            return;
        }
        List<Permanent> toSacrifice = additionalSpellCostService.validateSacrificeFractionRoundedUpCost(
                gameData, player, card, cost, sacrificePermanentIds);
    ...
    private void payTapCreaturesForManaCost(GameData gameData, Player player, Card card,
                                            List<UUID> tapPermanentIds) {
        List<Permanent> toTap = additionalSpellCostService.validateTapCreaturesForManaCost(
                gameData, player, card, tapPermanentIds);
        triggerCollectionService.beginPermanentTapTriggerBatch(gameData);
        try {
            for (Permanent permanent : toTap) {
                permanent.tap();
    ...
    private UUID payTapMultiplePermanentsCost(GameData gameData, Player player, Card card,
        ...
        return toTap.isEmpty() ? null : toTap.getFirst().getId();
    }

    private UUID payTapPermanentOrPayManaCost(GameData gameData, Player player, Card card,
                                              TapPermanentOrPayManaCost cost, List<UUID> tapPermanentIds,
                                              ManaPool preManaPaymentPool) {
        List<UUID> ids = tapPermanentIds != null ? tapPermanentIds : List.of();
        if (!ids.isEmpty()) {
            return payTapMultiplePermanentsCost(gameData, player, card,
                    new TapMultiplePermanentsCost(1, cost.filter()), ids, 0);
        }
        try {
            ManaCost extra = new ManaCost(cost.manaCost());
            ManaPool pool = gameData.playerManaPools.get(player.getId());
            int before = pool.getTotalAllMana();
            if (!extra.canPay(pool)) {
                throw new IllegalStateException("Not enough mana to pay " + cost.manaCost()
                        + " for " + card.getName());
            }
            extra.pay(pool);
            gameData.addSpellCastManaSpent(card.getId(), before - pool.getTotalAllMana());
    ...
    private void payTeamworkCost(GameData gameData, Player player, Card card,
                                 TeamworkCost cost, List<UUID> teamworkPermanentIds) {
        List<Permanent> toTap = additionalSpellCostService.validateTeamworkCost(
                gameData, player, card, cost, teamworkPermanentIds);
        triggerCollectionService.beginPermanentTapTriggerBatch(gameData);
        try {
            for (Permanent permanent : toTap) {
                permanent.tap();
                triggerCollectionService.checkEnchantedPermanentTapTriggers(gameData, permanent);
                triggerCollectionService.checkSelfTappedToPayTeamworkCostTriggers(gameData, permanent);
                gameLogService.append(gameData, GameLog.builder()
                        .text(player.getUsername() + " taps ")
                        .card(permanent.getCard())
                        .text(" to cast ")
    ...
    private void stampSacrificedCostSnapshot(StackEntry entry, AdditionalCostPayment payment) {
        ...
        entry.setSacrificedPermanentSnapshot(payment.sacrificedPermanentSnapshot());
    ...
    private void payExileNCardsFromGraveyardOrPayManaCost(
            GameData gameData, Player player, Card card,
            ExileNCardsFromGraveyardOrPayManaCost cost,
            List<Integer> exileGraveyardCardIndices, ManaPool preManaPaymentPool) {
        if (cost == null) {
            return;
        }
    ...
    /** Pays the selected branch of a sacrifice, discard, or life-payment additional cost. */
    private void paySacrificeCreatureOrDiscardCardOrPayLifeCost(
            GameData gameData, Player player, Card card,
            SacrificeCreatureOrDiscardCardOrPayLifeCost cost,
            AdditionalSpellCostService.CostSelection selection) {
        if (cost == null || selection.sacrificePermanentId() != null) {
            return;
        }
        additionalSpellCostService.validateSacrificeCreatureOrDiscardCardOrPayLifeCost(
                gameData, player, card, cost, selection);
        ...
        for (CastingCost castingCost : flashbackOpt.get().costs()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (!gameQueryService.canSacrificePermanentForCosts(gameData, permanent)) {
                    throw new IllegalStateException("Players can't sacrifice this permanent to cast from the graveyard");
                }
    ...
    private int maximumDelveReductionForGraveyardCast(GameData gameData, UUID playerId, Card card,
                                                       ManaCost cost, int additionalCost) {
        if (castingCostService.maximumDelveReduction(gameData, playerId, card, 0, additionalCost) == 0) {
            return 0;
        }
        int genericDemand = cost.getGenericCost() + additionalCost;
        long availableCards = gameData.playerGraveyards.getOrDefault(playerId, List.of()).stream()
                .filter(graveyardCard -> !graveyardCard.getId().equals(card.getId()))
                .count();
        return Math.min((int) availableCards, Math.max(0, genericDemand));
    }
    ...
    private void playFlashbackSpellFromLocation(GameData gameData, Player player, List<Card> graveyard,
        ...
        playFlashbackSpellFromLocation(gameData, player, graveyard, graveyardCardIndex, xValue, targetId, targetIds,
                ...
                beholdHandCardIndices, List.of(), List.of());
    }
    ...
    private void playFlashbackSpellFromLocation(GameData gameData, Player player, List<Card> graveyard,
        ...
        boolean isRetrace = !graveyardAbilitiesSuppressed
                ...
                || hasSpellCastingAbilityGrantForCard(gameData, playerId, card, Keyword.RETRACE, Zone.GRAVEYARD)
        ...
        boolean castWithWarp = isGraveyardCast && card.hasKeyword(Keyword.WARP);
        ...
        boolean hasUnsupportedAdditionalCost = additionalCosts.sacrificeAllCreatures()
                ...
                || additionalCosts.sacrificeCreatureOrDiscardCardOrPayLifeCost() != null
                ...
                || additionalCosts.exileAnyNumberOfCardsFromHandCost() != null
                ...
                || additionalCosts.sacrificeFractionRoundedUpCost() != null
                || additionalCosts.tieredManaCost() != null
                || additionalCosts.chooseCreatureTypeCost() != null
                || additionalCosts.tapCreaturesForManaCost() != null
                || additionalCosts.waterbendCost() != null
                || additionalCosts.teamworkCost() != null;
        ...
        additionalSpellCostService.validateCasualtyCost(
                gameData, player, castHalf, additionalCosts.casualtyCost(), sacrificePermanentId, effectiveXValue);
        if (additionalCosts.sacrificePermanentCost() != null
                && (!additionalCosts.sacrificePermanentCost().optional() || sacrificePermanentId != null)) {
        ...
        if (isGrantedCyclingGraveyardCast) {
            validateGraveyardCastAdditionalCosts(gameData, player, filteredGraveyardAdditionalCosts,
                    retraceDiscardHandCardIndex, discardHandCardIndices,
                    sacrificePermanentId, additionalCostSacrificePermanentIds);
        }
        int delveReduction = 0;
        if (additionalCosts.delveCost() != null) {
            List<Integer> delveIndices = exileGraveyardCardIndices == null
                    ? List.of() : exileGraveyardCardIndices;
            if (graveyard == gameData.playerGraveyards.get(playerId)
                    && delveIndices.contains(graveyardCardIndex)) {
                throw new IllegalStateException("The card being cast cannot be exiled for delve");
            }
            ManaCost delveCost = castingCostService.applyColoredManaCostReductions(
                    gameData, playerId, card, new ManaCost(card.getManaCost()));
            int maximumDelveReduction = maximumDelveReductionForGraveyardCast(
                    gameData, playerId, card, delveCost, additionalCost);
            additionalSpellCostService.validateDelveCost(gameData, player, card, additionalCosts.delveCost(),
                    delveIndices, maximumDelveReduction);
            delveReduction = additionalSpellCostService.delveReduction(additionalCosts, delveIndices);
        }
        List<ManaColor> convokeContributions = planConvokeContributions(
                gameData, playerId, card, convokeCreatureIds);
        boolean withoutPayingManaCost = gameData.cardsGrantedFlashbackWithoutPayingManaCostUntilEndOfTurn
                .contains(card.getId())
                || grantedGraveyardCardCast
                && gameData.graveyardCardCastPermissionsUntilEndOfTurn.get(card.getId())
        ...
        if (card.isRequiresNoMana() && !withoutPayingManaCost) {
            ManaCost noManaCost = castingCostService.applyColoredManaCostReductions(
                    gameData, playerId, card, new ManaCost(card.getManaCost()));
            int manaXValue = noManaCost.hasX() ? effectiveXValue : 0;
            if (!noManaCost.canPayWithConvoke(new ManaPool(), additionalCost - delveReduction + manaXValue,
                    convokeContributions)) {
                throw new IllegalStateException("This spell must be cast using convoke and delve");
            }
        }
        if (!convokeCreatureIds.isEmpty()) {
            convokeContributions = applyConvokeOrImprovise(
                    gameData, playerId, card, convokeCreatureIds, Zone.GRAVEYARD);
        }
        ...
        if (additionalCosts.sacrificeCreature() || additionalCosts.sacrificePermanentCost() != null) {
            payAllSacrificeCosts(gameData, player, castHalf, sacrificePermanentId, additionalCosts,
                    effectiveXValue, effectiveXValue);
        } else if (additionalCosts.casualtyCost() != null && sacrificePermanentId != null) {
            payAllSacrificeCosts(gameData, player, castHalf, sacrificePermanentId, additionalCosts,
                    effectiveXValue, effectiveXValue);
        }
        if (additionalCosts.sacrificeAnyNumberCost() != null) {
        ...
        if ((isGrantedCyclingGraveyardCast && filteredGraveyardPermission.isPresent()
                && filteredGraveyardPermission.get().permission().exileAfterResolution())
                || isMayCastTopInstantOrSorcery
    ...
    private void playGraveyardLandFromLocation(GameData gameData, Player player, List<Card> graveyard,
        ...
        if (graveyardLandPermission.isPresent()
            ...
            permanent.addPersistentTriggeredEffect(grant.slot(), grant.grantedEffect());
        }
        if (entersTapped) {
            permanent.tap();
        }
    ...
    private void playCardFromExileInternal(GameData gameData, Player player, UUID exileCardId, Integer xValue,
        ...
        Zone sourceZone = sourceZoneOverride != null
        ...
        if (fromOutsideGame) {
            ...
            if (!gameData.outsideGamePlayPermissions.contains(exileCardId)) {
                throw new IllegalStateException("No permission to play this card from outside the game");
            }
        } else {
        ...
        if (card.hasType(CardType.LAND)
                ...
                : castingPermissionService.getForbiddenCardNames(gameData, playerId).contains(card.getName())) {
            throw new IllegalStateException("Card is not playable");
        }
        ...
        boolean hasPermission = resolutionCast
                ...
                || collectionCounterPermission
                || waterbendCast
                || castForForetell;
        boolean anyManaType = !fromOutsideGame && castingPermissionService.hasAnyManaTypePermission(
                gameData, playerId, exileCardId);
        int additionalCounterCost = directExilePlayPermission
                ? 0 : sourceCounterCost.orElse(0);
        if (sourceManaValueLifeAlternative
            ...
            throw new IllegalStateException("Not enough life to cast the exiled card");
        }
        effectiveXValue = sourceManaValueLifeAlternative
        ...
        if (manaValueLifeAlternative && (!gameQueryService.canPayLifeForCosts(gameData)
                || !gameQueryService.canPlayerLifeChange(gameData, playerId)
                || gameData.getLife(playerId) < card.getManaValue())) {
            throw new IllegalStateException("Not enough life to cast the exiled card");
        }
        ...
        if (card.hasType(CardType.LAND)) {
            ...
            gameLogService.append(gameData,
                    GameLog.playerPlays(player.getUsername(), landFace,
                            fromOutsideGame ? " from outside the game." : " from exile."));
            log.info("Game {} - {} plays {} from {}", gameData.id, player.getUsername(), landFace.getName(),
                    fromOutsideGame ? "outside the game" : "exile");
            if (cloneService.prepareCloneReplacementEffect(gameData, playerId, landFace, null, effectiveXValue, true)) {
                return;
            }
        ...
        List<ManaColor> convokeContributions = waterbendCast ? List.of() : applyConvokeOrImprovise(
                gameData, playerId, card, convokeCreatureIds, sourceZone);
        int phyrexianManaPaidWithLife = 0;
        ManaPool pool = gameData.playerManaPools.get(playerId);
        Map<ManaColor, Integer> exileSpellOnly = sourceZone == Zone.EXILE
                ? pool.promoteExiledSpellOnlyMana() : Map.of();
        Map<ManaColor, Integer> exactExileOnly = sourceZone == Zone.EXILE
                ? pool.promoteExiledCardOnlyMana(exileCardId) : Map.of();
        ...
        try {
            ...
            } else if (castingPermissionService.hasSnowManaAsAnyColorPermission(gameData, playerId, exileCardId) && card.getManaCost() != null) {
        ...
        boolean preparedSpellCopy = gameData.playerBattlefields.values().stream()
                .flatMap(List::stream)
                .anyMatch(permanent -> exileCardId.equals(permanent.getPreparedSpellCardId()));
        stackEntry.setSourceZone(sourceZone);
        ...
        stackEntry.setControlledModifiedCreatureAsCast(controlsModifiedCreatureAsCast(gameData, playerId));
        boolean controlledFaerie = gameData.playerBattlefields.getOrDefault(playerId, List.of()).stream()
                .anyMatch(permanent -> gameQueryService.effectiveCreatureSubtypes(gameData, permanent)
        ...
        String sourceDescription = fromOutsideGame ? " from outside the game."
                : sourceZone == Zone.BATTLEFIELD ? " from the battlefield." : " from exile.";
    ...
    private void commitExileCast(GameData gameData, UUID playerId, UUID exileCardId,
        ...
        if (directExilePlayPermission && !copy
                && !gameData.hasExilePlayPermissionRemaining(exileCardId)) {
            throw new IllegalStateException("Exile play limit has been reached");
        }
        if (collectionCounterPermission
                && !castingPermissionService.consumeCollectionCounterPermission(
                        gameData, playerId, exileCardId)) {
            throw new IllegalStateException("Collection-counter exile cast permission is no longer available");
        }
        if (sourceFreeCast
    ...
    public void playCardFromLibraryTop(GameData gameData, Player player, Integer xValue, UUID targetId,
        ...
        if (castingPermissionService.getForbiddenCardNames(gameData, playerId).contains(card.getName())) {
            throw new IllegalStateException("Card is not playable");
        }
    ...
    public void castCardFromLibraryWhileSearching(GameData gameData, Player player, Card card) {
        castCardFromLibraryWhileSearching(gameData, player, player.getId(), card, null);
    }
    ...
    public void castCardFromLibraryWhileSearching(GameData gameData, Player player,
        ...
        castCardFromLibraryWhileSearching(gameData, player, libraryOwnerId, card, null);
    }

    /** Casts a card from the bottom of its owner's library for a supplied alternate cost. */
    public void castCardFromLibraryForAlternateCostWhileSearching(GameData gameData, Player player,
                                                                   Card card, String alternateCost) {
        castCardFromLibraryWhileSearching(gameData, player, player.getId(), card, alternateCost);
    }

    private void castCardFromLibraryWhileSearching(GameData gameData, Player player,
                                                   UUID libraryOwnerId, Card card, String alternateCost) {
        if (gameData.status != GameStatus.RUNNING) {
            throw new IllegalStateException("Game is not running");
        }
        ...
        if (alternateCost == null) {
            ...
            payAlternateManaCostFromNonHandZone(gameData, playerId, card, alternateCost, Zone.LIBRARY);
        }
        ...
        stackEntry.setSourceZone(Zone.LIBRARY);
        stackEntry.setAlternateCost(alternateCost != null);
        stampCastDuringMainPhase(gameData, stackEntry, playerId);
        gameData.stack.add(stackEntry);
        ...
        gameLogService.append(gameData, GameLog.textCardText(
                player.getUsername() + " casts ", castCard,
                alternateCost == null
                        ? " from their library while searching."
                        : " from their library for its alternate cost (" + alternateCost + ")."));
    ...
    public void payAlternateManaCostFromNonHandZone(GameData gameData, UUID playerId, Card card,
                                                     String manaCost, Zone sourceZone) {
        ManaPool pool = gameData.playerManaPools.get(playerId);
        int before = pool.getTotalAllMana();
        ManaCost cost = castingCostService.applyColoredManaCostReductions(
                gameData, playerId, card, new ManaCost(manaCost));
        int additionalCost = castingCostService.getCastCostModifier(
                gameData, playerId, card, 0, sourceZone);
    ...
    private void payLifeForLibraryTopAlternative(GameData gameData, Player player, Card card) {
        ...
        if (amount <= 0) return;
        UUID playerId = player.getId();
        lifeSupport.applyLifePayment(gameData, playerId, amount,
    ...
    private <T> T withMulticoloredSpellMana(GameData gameData, UUID playerId, Card card,
                                             Supplier<T> action) {
        ManaPool pool = gameData.playerManaPools.get(playerId);
        ManaPool.ExactlyThreeColorSpellManaState exactlyThreeColorState =
                gameQueryService.getEffectiveCardColors(gameData, card).size() == 3
                        ? pool.promoteExactlyThreeColorSpellOnlyMana() : null;
        ...
        try {
            ...
            if (exactlyThreeColorState != null) {
                pool.restorePromotedExactlyThreeColorSpellOnlyMana(exactlyThreeColorState);
            }
            if (state != null) {
    ...
    private void recordTreasureManaSpent(GameData gameData, Card card, int before, int after) {
        gameData.addSpellCastTreasureManaSpent(card.getId(), Math.max(0, before - after));
    }
    ...
    private SpellManaPayment computeSpellManaPayment(GameData gameData, UUID playerId, Card card, int effectiveXValue,
        ...
        boolean previousUnionKickedPermission =
                pool.isKickedOrInstantSorceryOnlyManaUsableForKickedSpell();
        boolean previousUnionInstantPermission =
                pool.isKickedOrInstantSorceryOnlyManaUsableForInstantSorcery();
        pool.setInstantSorceryOrClassLevelManaUsableForInstantSorcery(card.hasType(CardType.INSTANT) || card.hasType(CardType.SORCERY));
        if (kicked) {
            pool.setKickedOrInstantSorceryOnlyManaUsableForInstantSorcery(false);
            pool.setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(true);
        ...
        ManaPool.RoomSpellsOrUnlocksManaState roomSpellsOrUnlocksMana =
                card.hasType(CardType.ENCHANTMENT)
                        && card.getSubtypes().contains(CardSubtype.ROOM)
        ...
        try {
            ...
            pool.setInstantSorceryOrClassLevelManaUsableForInstantSorcery(previousClassPermission);
            pool.setKickedOrInstantSorceryOnlyManaUsableForKickedSpell(previousUnionKickedPermission);
            ...
            if (roomSpellsOrUnlocksMana != null) {
                pool.restorePromotedRoomSpellsOrUnlocksMana(roomSpellsOrUnlocksMana);
            }
            if (enchantmentOrRoomUnlockOrTurnFaceUpMana != null) {
    ...
    private SpellManaPayment computeSpellManaPaymentInternal(GameData gameData, UUID playerId, Card card, int effectiveXValue,
        ...
        if (sourceZone == Zone.EXILE) {
            ...
            if (exilePlayCostModifier != null
                    ...
                    && exilePlayCostModifier.replacementManaCost() != null) {
                baseMana = exilePlayCostModifier.replacementManaCost();
        ...
        int before = pool.getTotalAllMana();
        int additionalCost = castingCostService.getCastCostModifier(
                gameData, playerId, card, effectiveXValue, sourceZone, kicked, collectEvidenceCostPaid)
    ...
    private Map<UUID, Integer> validateCounterAssignments(
            GameData gameData, Card card, UUID playerId, int xValue,
        DistributeCountersAmongTargetsEffect effect, Map<UUID, Integer> damageAssignments) {
        ...
        for (Map.Entry<UUID, Integer> assignment : assignments.entrySet()) {
            ...
            if (effect.targetRestriction() != null
                    && !predicateEvaluationService.matchesPermanentPredicate(
                    target, effect.targetRestriction(), assignmentContext)) {
                throw new IllegalStateException("Target is not a legal target");
            }
    ...
    private DealDividedDamageEffect findChosenDividedDamageEffectForCast(
            GameData gameData, UUID playerId, List<CardEffect> effects, int xValue, boolean kicked) {
        for (CardEffect e : effects) {
            if (e instanceof DealDividedDamageEffect d
                    && d.mode() == DivisionMode.CHOSEN && !d.etbAssignments()) {
                return d;
            ...
            if (e instanceof ConditionalEffect conditional) {
                DealDividedDamageEffect nested = findChosenDividedDamageEffectForCast(
                        gameData, playerId, List.of(conditional.wrapped()), xValue, kicked);
                if (nested != null) {
                    return nested;
                }
            }
            if (e instanceof ConditionalReplacementEffect conditional) {
                CardEffect selected = conditionEvaluationService.isMet(
                        gameData, conditional.condition(),
                        com.github.laxika.magicalvibes.service.effect.ConditionContext
                                .forCasting(playerId, kicked, false).withXValue(xValue))
                        ? conditional.upgradedEffect() : conditional.baseEffect();
                if (selected != null) {
                    DealDividedDamageEffect nested = findChosenDividedDamageEffectForCast(
                            gameData, playerId, List.of(selected), xValue, kicked);
                    if (nested != null) {
                        return nested;
                    }
                }
    ...
    private void validateDividedDamageAssignments(GameData gameData, Card card, UUID playerId,
        ...
        if (card.hasXScaledTargets()
                && (assignments.size() < card.getEffectiveMinTargets(resolvedXValue)
                || assignments.size() > card.getEffectiveMaxTargets(resolvedXValue))) {
            throw new IllegalStateException("Incorrect number of targets for X");
        }
    ...
    private void validateDividedDamageAmountsAndTypes(GameData gameData, Card card, UUID playerId,
        ...
        if (dividedEffect.totalDamage() instanceof Fixed fixedTotal) {
            ...
            if (!dividedEffect.canTargetPlayers()) {
                for (UUID assignedTargetId : assignments.keySet()) {
                    Permanent target = gameQueryService.findPermanentById(gameData, assignedTargetId);
                    if (target == null || !isValidDividedDamagePermanentTarget(gameData, target, dividedEffect)) {
                        throw new IllegalStateException("All targets must be creatures");
                    }
                    if (dividedEffect.targetRestriction() != null
                            && !predicateEvaluationService.matchesPermanentPredicate(
            ...
            if (dividedEffect.maxTargets() > 0 && assignments.size() > dividedEffect.maxTargets()) {
                throw new IllegalStateException("Too many targets");
            }
            for (Map.Entry<UUID, Integer> assignment : assignments.entrySet()) {
                UUID assignedTargetId = assignment.getKey();
    ...
    private boolean isValidDividedDamagePermanentTarget(GameData gameData, Permanent target,
                                                        DealDividedDamageEffect dividedEffect) {
        return gameQueryService.isCreature(gameData, target)
                || (dividedEffect.canTargetPlaneswalkers()
                || dividedEffect.canTargetPlayers()) && gameQueryService.isPlaneswalker(gameData, target)
                || dividedEffect.canTargetPlayers() && gameQueryService.isBattle(gameData, target);
    }
    ...
    private SpliceEffect findMatchingSpliceEffect(Card spliceCard, Card hostSpell) {
        return spliceCard.getEffects(EffectSlot.STATIC).stream()
                ...
                .filter(s -> s.appliesTo(hostSpell))
                .findFirst()
                .orElse(null);
    }
    ...
    private int computeKickerManaPayment(GameData gameData, Player player, Card card, KickerEffect kickerEffect,
        ...
        if (kickerEffect.hasManaCost()) {
            ...
            try {
                ...
                } else if (pool.getKickedOnlyManaTotal() > 0) {
    ...
    /** Animal Attendant: each tagged mana spent on a non-Human creature spell grants one additional counter. */
    private void applyNonHumanAdditionalCounterGrantingMana(GameData gameData, UUID playerId, Card card,
                                                            int nonHumanAdditionalCounterGrantingBefore) {
        if (!card.hasType(CardType.CREATURE)
                || gameQueryService.getCardSubtypes(card, gameData, playerId).contains(CardSubtype.HUMAN)) {
            return;
        }
    ...
    private void validateGraveyardCastAdditionalCosts(GameData gameData, UUID playerId, Card card,
        ...
        validateGraveyardDiscardCosts(gameData, playerId, graveyardCast.additionalCosts(),
    ...
    private void validateGraveyardCastAdditionalCosts(GameData gameData, Player player,
        ...
        validateGraveyardDiscardCosts(gameData, player.getId(), additionalCosts,
                discardHandCardIndex, discardHandCardIndices);
        ...
        for (CastingCost cost : additionalCosts) {
            if (cost instanceof LifeCastingCost lifeCost) {
                ...
                // The discard selection was validated as a group above.
            } else if (cost instanceof ReturnPermanentsCost returnCost) {
                validateMorphAdditionalCost(gameData, player, returnCost,
                        sacrificePermanentId == null ? List.of() : List.of(sacrificePermanentId));
    ...
    private void validateGraveyardDiscardCosts(GameData gameData, UUID playerId,
                                               List<CastingCost> additionalCosts,
                                               Integer discardHandCardIndex,
                                               List<Integer> discardHandCardIndices) {
        List<Integer> selectedIndices = selectedGraveyardDiscardIndices(
                discardHandCardIndex, discardHandCardIndices);
        int requiredCount = additionalCosts.stream()
                .filter(DiscardCardCastingCost.class::isInstance)
                .mapToInt(cost -> ((DiscardCardCastingCost) cost).count())
                .sum();
        if (requiredCount == 0) {
            return;
        }
        if (selectedIndices.size() != requiredCount) {
            throw new IllegalStateException("Must discard exactly " + requiredCount
                    + " cards to cast from the graveyard");
        }
        if (selectedIndices.stream().distinct().count() != selectedIndices.size()) {
            throw new IllegalStateException("Duplicate cards cannot be discarded to cast from the graveyard");
        }
        List<Card> hand = gameData.playerHands.getOrDefault(playerId, List.of());
        int selectionIndex = 0;
        for (CastingCost cost : additionalCosts) {
            if (!(cost instanceof DiscardCardCastingCost discardCost)) {
                continue;
            }
            for (int i = 0; i < discardCost.count(); i++) {
                int handIndex = selectedIndices.get(selectionIndex++);
                if (handIndex < 0 || handIndex >= hand.size()) {
                    throw new IllegalStateException("Invalid card chosen to discard for the graveyard cast");
                }
                Card selected = hand.get(handIndex);
                if (discardCost.predicate() != null
                        && !predicateEvaluationService.matchesCardPredicate(
                        selected, discardCost.predicate(), selected.getId())) {
                    String label = discardCost.label() == null ? "a matching card" : discardCost.label();
                    throw new IllegalStateException("Each discarded card must be " + label);
                }
            }
        }
    }

    private List<Integer> selectedGraveyardDiscardIndices(Integer discardHandCardIndex,
                                                          List<Integer> discardHandCardIndices) {
        List<Integer> selectedIndices = new ArrayList<>();
        if (discardHandCardIndex != null) {
            selectedIndices.add(discardHandCardIndex);
        }
        if (discardHandCardIndices != null) {
            selectedIndices.addAll(discardHandCardIndices);
        }
    ...
    private void payGraveyardCastAdditionalCosts(GameData gameData, Player player, Card card,
        ...
        payGraveyardDiscardCosts(gameData, player, card, graveyardCast.additionalCosts(),
                discardHandCardIndex, discardHandCardIndices);
        if (exileCost != null) {
            List<Card> graveyard = gameData.playerGraveyards.get(playerId);
            List<Integer> sortedDescending = exileGraveyardCardIndices.stream()
    ...
    private void payGraveyardCastAdditionalCosts(GameData gameData, Player player, Card card,
        ...
        for (CastingCost cost : additionalCosts) {
            if (cost instanceof LifeCastingCost lifeCost) {
                ...
                // Discard costs are paid together below so pre-removal hand indices remain stable.
            } else if (cost instanceof ReturnPermanentsCost returnCost) {
                ...
                if (permanentRemovalService.sacrificePermanentToGraveyard(gameData, permanent)) {
                        gameLogService.append(gameData, GameLog.builder()
                                ...
                                .build());
                        triggerCollectionService.checkAllyPermanentSacrificedTriggers(
                                gameData, playerId, permanent.getCard(), card);
                    }
                }
            } else {
                throw new IllegalStateException("Cannot pay this graveyard cast cost");
        ...
        payGraveyardDiscardCosts(gameData, player, card, additionalCosts,
                discardHandCardIndex, discardHandCardIndices);
    }

    private void payGraveyardDiscardCosts(GameData gameData, Player player, Card card,
                                           List<CastingCost> additionalCosts,
                                           Integer discardHandCardIndex,
                                           List<Integer> discardHandCardIndices) {
        List<Integer> selectedIndices = selectedGraveyardDiscardIndices(
        ...
        List<Integer> descendingIndices = selectedIndices.stream()
                .sorted(java.util.Comparator.reverseOrder()).toList();
    ...
    private int payFlashbackOrGraveyardCastCost(GameData gameData, Player player, Card card,
        ...
        if (usesNormalManaCost) {
            ...
            int paymentAdditionalCost = additionalCost - delveReduction;
            withMulticoloredSpellMana(gameData, playerId, card, () -> {
                if (card.isRequiresNoMana()) {
                    int manaXValue = cost.hasX() ? effectiveXValue : 0;
                    ManaPool noManaPool = new ManaPool();
                    if (!cost.canPayWithConvoke(noManaPool, paymentAdditionalCost + manaXValue,
                            convokeContributions)) {
                        throw new IllegalStateException("This spell must be cast using convoke and delve");
                    }
                    cost.payWithConvoke(noManaPool, paymentAdditionalCost + manaXValue, convokeContributions);
                } else if (anyManaType) {
                    if (!cost.canPayAsGeneric(pool, effectiveXValue, paymentAdditionalCost)) {
                        throw new IllegalStateException("Not enough mana to pay casting cost");
                    }
                    cost.payAsGeneric(pool, effectiveXValue, paymentAdditionalCost);
                    if (gameData.hasPendingAnyManaTypeForNextSpellThisTurn(playerId)) {
                        gameData.markSpellPaidUsingPendingAnyManaType(card.getId());
                    }
                } else if (cardHasFlashback) {
                    int manaXValue = cost.hasX() ? effectiveXValue : 0;
                    if (!cost.canPayFlashbackFromGraveyard(pool, manaXValue + paymentAdditionalCost)) {
            ...
            manaSpent = before - pool.getTotalAllMana();
            if (card.isRequiresNoMana()) {
                manaSpent = convokeContributions.size();
            }
        } else {
    ...
    private UUID validateSneakCast(GameData gameData, UUID playerId, AlternateHandCast altCast,
                                   List<UUID> alternateCostPermanentIds) {
        if (gameData.currentStep != TurnStep.DECLARE_BLOCKERS
                || !playerId.equals(gameData.activePlayerId)) {
            throw new IllegalStateException("Sneak may only be used during declare blockers on your turn");
        }

        int returnIndex = altCast.getCost(SacrificePermanentsCost.class)
                .map(SacrificePermanentsCost::count).orElse(0)
                + altCast.getCost(TapUntappedPermanentsCost.class)
                .map(TapUntappedPermanentsCost::count).orElse(0);
        if (alternateCostPermanentIds.size() <= returnIndex) {
            throw new IllegalStateException("Sneak requires an unblocked attacker to return");
        }
        ...
        Permanent attacker = gameData.playerBattlefields.getOrDefault(playerId, List.of()).stream()
                ...
                .findFirst()
                .orElse(null);
        if (attacker == null || !predicateEvaluationService.matchesPermanentPredicate(
                gameData, attacker, new PermanentIsUnblockedAttackingPredicate())) {
            throw new IllegalStateException("Sneak requires an unblocked attacker you control");
        }
        if (attacker.getAttackTarget() == null) {
            throw new IllegalStateException("Sneak requires an attacking creature with a legal attack target");
        }
        ...
        try {
            if (altCast.reduceManaBySacrificedManaCost()) {
                if (manaCostOpt.get().treasureManaOnly()) {
                    pool.payWithTreasureManaAfterReduction(cost, sacrificedManaCost);
                } else {
                ...
                int faceDownCostReduction = card.getMorphCost() != null
                        ? castingCostService.getCastCostModifierForFaceDownSpell(gameData, playerId, card) : 0;
                int costArgument = alternateCostXArgument(cost, xValue, emergeReduction) + faceDownCostReduction + alternateCostModifier;
                if (manaCostOpt.get().treasureManaOnly()) {
                    pool.payWithTreasureMana(cost, costArgument);
                } else {
                    cost.pay(pool, costArgument);
                }
            }
        } finally {
            if (restrictedMana != null) {
    ...
    public void finishSpellCast(GameData gameData, UUID playerId, Player player, List<Card> hand, Card card, boolean castFromHand) {
        ...
        if (castEntry != null && castEntry.isCastFaceDown()) {
            ...
            castCharacteristics = card.createRuntimeCopyWithFace(faceDown);
            castCharacteristics.freeze();
        } else if (castEntry != null && card.getBackFaceCard() != null
        ...
        boolean commandCast = commanderId != null;
        if (commandCast) {
        ...
        if (castEntry != null) {
            ...
            castEntry.setControlledModifiedCreatureAsCast(controlsModifiedCreatureAsCast(gameData, playerId));
            boolean controlledFaerie = gameData.playerBattlefields.getOrDefault(playerId, List.of()).stream()
                    .anyMatch(permanent -> gameQueryService.effectiveCreatureSubtypes(gameData, permanent)
                            .contains(CardSubtype.FAERIE));
    ...
    public void playCardWithCasualty(GameData gameData, Player player, int cardIndex, Integer xValue,
        ...
        try {
            playCardInternal(gameData, player, cardIndex, xValue, targetId, damageAssignments, targetIds,
                    convokeCreatureIds, fromGraveyard, sacrificePermanentId, phyrexianLifeCount,
                    alternateCostSacrificePermanentIds, exileGraveyardCardIndex, exileGraveyardCardIndices, kicked,
                    discardHandCardIndex, discardHandCardIndices, false, List.of(), null,
                    List.of(), imposedSacrificePermanentIds, additionalCostSacrificePermanentIds,
                    repeatedAdditionalCosts, buyback, false, List.of(), sharedColorDiscardHandCardIndex,
                    beholdPermanentId, beholdHandCardIndex, beholdPermanentIds, beholdHandCardIndices,
                    beholdChosenSubtype, null, false, null, false, false, null, casualtyCreatureIds);
        } catch (IllegalArgumentException | IllegalStateException e) {
