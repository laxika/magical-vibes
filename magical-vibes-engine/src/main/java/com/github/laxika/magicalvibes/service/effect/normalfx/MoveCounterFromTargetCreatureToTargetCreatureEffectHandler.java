...
public class MoveCounterFromTargetCreatureToTargetCreatureEffectHandler implements NormalEffectHandlerBean {
    ...
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        ...
        if (targets == null || targets.size() < 2) {
            gameBroadcastService.logAndBroadcast(gameData, entry.getCard().getName() + "'s ability fizzles (invalid targets).");
            return;
        }
        ...
        if (source == null || destination == null) {
            gameBroadcastService.logAndBroadcast(gameData, entry.getCard().getName() + "'s ability fizzles (target no longer on the battlefield).");
            return;
        }
        ...
        gameBroadcastService.logAndBroadcast(gameData,
                "A counter is moved from " + source.getCard().getName() + " onto " + destination.getCard().getName() + ".");
