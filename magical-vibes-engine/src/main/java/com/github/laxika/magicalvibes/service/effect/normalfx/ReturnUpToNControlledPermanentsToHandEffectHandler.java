...
/** Resolves a controller's choice of up to N matching permanents to return. */
@Component
@RequiredArgsConstructor
public class ReturnUpToNControlledPermanentsToHandEffectHandler implements NormalEffectHandlerBean {
    ...
    public Class<? extends CardEffect> handledEffect() {
        return ReturnUpToNControlledPermanentsToHandEffect.class;
    }

    @Override
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        var e = (ReturnUpToNControlledPermanentsToHandEffect) effect;
        List<UUID> validIds = matchingControlledPermanentIds(gameData, entry, e);
        int maxCount = Math.min(Math.max(0, e.maxCount()), validIds.size());
        if (maxCount == 0) {
            return;
        }

        playerInputService.beginMultiPermanentChoice(gameData, entry.getControllerId(), validIds, maxCount,
                new MultiPermanentChoiceContext.ReturnUpToNControlledPermanentsToHand(e),
                "Choose up to " + maxCount + " " + plural(e.noun(), maxCount)
                        + " you control to return.");
    }
    ...
    public void completeChoice(GameData gameData, List<UUID> permanentIds,
                               ...
                               StackEntry entry) {
        ReturnUpToNControlledPermanentsToHandEffect effect = context.effect();
    ...
    private List<UUID> matchingControlledPermanentIds(GameData gameData, StackEntry entry,
                                                       ReturnUpToNControlledPermanentsToHandEffect effect) {
