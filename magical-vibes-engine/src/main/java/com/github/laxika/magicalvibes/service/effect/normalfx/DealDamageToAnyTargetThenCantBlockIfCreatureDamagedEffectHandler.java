...
public class DealDamageToAnyTargetThenCantBlockIfCreatureDamagedEffectHandler implements NormalEffectHandlerBean {

    private final DamageSupport damageSupport;
    private final GameQueryService gameQueryService;
    private final GameOutcomeService gameOutcomeService;
    private final AmountEvaluationService amountEvaluationService;
    private final CantBlockThisTurnEffectHandler cantBlockThisTurnEffectHandler;

    @Override
    public Class<? extends CardEffect> handledEffect() {
        return DealDamageToAnyTargetThenCantBlockIfCreatureDamagedEffect.class;
    }

    @Override
    public void resolve(GameData gameData, StackEntry entry, CardEffect effect) {
        var damageEffect = (DealDamageToAnyTargetThenCantBlockIfCreatureDamagedEffect) effect;
        ...
        for (Map.Entry<UUID, Integer> dealtDamage : gameData.damageDealtToPermanentsThisTurn.entrySet()) {
            ...
            if (damagedPermanent != null && gameQueryService.isCreature(gameData, damagedPermanent)) {
                cantBlockThisTurnEffectHandler.resolveForTarget(gameData, damagedPermanent);
            }
        }
