...
public interface InteractionHandler<T extends PendingInteraction> {
    ...
    /**
     * The legacy {@link AwaitingInput} value set alongside the active interaction so
     * enum-based awaiting checks (and existing tests) keep working until the enum teardown.
     */
    AwaitingInput legacyInputType();
