...
import com.github.laxika.magicalvibes.model.effect.VoteForDenialOrDuplicationEffect;
...
/** Resolves Split Decision's denial-or-duplication vote in turn order. */
@Component
@RequiredArgsConstructor
public class VoteForDenialOrDuplicationEffectHandler implements NormalEffectHandlerBean {

    private final InteractionHandlerRegistry interactionHandlerRegistry;

    @Override
    public Class<? extends CardEffect> handledEffect() {
        return VoteForDenialOrDuplicationEffect.class;
    }
    ...
    public void completeVote(GameData gameData, String choice,
                             ChoiceContext.VoteForDenialOrDuplicationChoice context) {
        if (!ChoiceContext.VoteForDenialOrDuplicationChoice.OPTIONS.contains(choice)) {
            throw new IllegalArgumentException("Invalid Split Decision vote: " + choice);
        }
    ...
    private void beginNextVote(GameData gameData, List<UUID> remainingPlayerIds,
        ...
        if (remaining.isEmpty()) {
            ...
            if (pendingEntry == null) {
                throw new IllegalStateException("Split Decision resolution is not resumable");
            }

            CardEffect result = votes.getOrDefault(ChoiceContext.VoteForDenialOrDuplicationChoice.DENIAL, 0)
                    > votes.getOrDefault(ChoiceContext.VoteForDenialOrDuplicationChoice.DUPLICATION, 0)
        ...
        interactionHandlerRegistry.begin(gameData, new PendingInteraction.ColorChoice(
                ...
                new ChoiceContext.VoteForDenialOrDuplicationChoice(
                        effectControllerId, remaining, votes, sourceName),
                ChoiceContext.VoteForDenialOrDuplicationChoice.OPTIONS,
                sourceName + " — vote for denial or duplication."));
    }
