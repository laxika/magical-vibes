    ...
    public boolean isMet(GameData gameData, Condition condition, ConditionContext ctx) {
        return switch (condition) {
            ...
            case SourceCounterThreshold c -> {
                Permanent source = sourcePermanent(gameData, ctx);
                yield source != null && (c.counterType() == CounterType.ANY
                        ? source.getCounters().values().stream().anyMatch(count -> count >= c.threshold())
                        : source.getCounterCount(c.counterType()) >= c.threshold());
            }
