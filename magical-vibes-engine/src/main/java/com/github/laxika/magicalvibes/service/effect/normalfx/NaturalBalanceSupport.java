...
/**
 * Drives Natural Balance's two-stage resolution: first the APNAP queue of per-player "you may
 * search your library for up to X basic land cards and put them onto the battlefield" picks, then
 * the forced land sacrifices held on the same {@link LibrarySearchFollowUp.NaturalBalanceQueue}.
 * The queue is advanced from the effect handler and re-entered by the library-search input handler
 * after every pick resolves.
 */
@Component
@RequiredArgsConstructor
public class NaturalBalanceSupport {
    ...
    public boolean advance(GameData gameData, LibrarySearchFollowUp followUp) {
        LibrarySearchFollowUp.NaturalBalanceQueue queue = followUp.naturalBalance();
        if (queue == null) {
            return false;
        }
        ...
        while (!remaining.isEmpty()) {
            ...
            if (startSearch(gameData, pick, followUp.withNaturalBalance(queue.withRemainingPicks(remaining)))) {
                return true;
            }
        }
     ...
     * to sacrifice.
     */
    private boolean beginSacrifices(GameData gameData, LibrarySearchFollowUp.NaturalBalanceQueue queue) {
