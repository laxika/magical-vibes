...
public class GameActionAvailabilityService {
    ...
    private boolean isCardPlayableForFace(GameData gameData, UUID playerId, Card card, ManaPool pool,
        ...
        ExileNCardsFromGraveyardOrPayManaCost exileOrPayCost = card.getEffects(EffectSlot.SPELL).stream()
                .filter(ExileNCardsFromGraveyardOrPayManaCost.class::isInstance)
                .map(ExileNCardsFromGraveyardOrPayManaCost.class::cast)
                .findFirst().orElse(null);
        if (exileOrPayCost != null) {
            List<Card> graveyard = gameData.playerGraveyards.getOrDefault(playerId, List.of());
            long matchingCount = graveyard.stream()
                    .filter(c -> (exileOrPayCost.requiredType() == null || c.hasType(exileOrPayCost.requiredType()))
                            && (exileOrPayCost.predicate() == null
                            || predicateEvaluationService.matchesCardPredicate(c, exileOrPayCost.predicate(), null)))
                    .count();
            if (matchingCount < exileOrPayCost.count()
                    && !castingCostService.canPayExileNCardsOrPayManaOption(
                    gameData, playerId, card, exileOrPayCost, pool)) {
                return false;
            }
        }
    ...
    private boolean isPlayableAsSpell(GameData gameData, UUID playerId, Card card, ManaPool pool,
        ...
        if (card.isRequiresNoMana()) {
            boolean hasConvoke = card.hasKeyword(Keyword.CONVOKE)
                    || hasSpellCastingAbilityGrant(gameData, playerId, card, Keyword.CONVOKE, Zone.HAND);
            if (!hasConvoke) {
                return false;
            }
            List<ManaColor> contributions = availableConvokeContributions(
                    gameData, ctx.battlefield(), extraConvokeMana);
            return cost != null
                    && !contributions.isEmpty()
                    && cost.canPayWithConvoke(new ManaPool(), effectiveAdditionalCost, contributions);
        }
    ...
    private List<ManaColor> availableConvokeContributions(GameData gameData, List<Permanent> battlefield,
                                                           int maxSources) {
        ...
        for (Permanent permanent : battlefield) {
            ...
            ManaColor contribution = colors == null || colors.isEmpty()
                    ? null
                    : colors.stream()
            ...
            if (maxSources > 0 && contributions.size() >= maxSources) {
                break;
            }
        }
        return contributions;
    ...
    public List<Integer> getPlayableFlashbackIndices(GameData gameData, UUID playerId) {
        ...
        for (int i = 0; i < graveyard.size(); i++) {
            ...
            boolean canPayMana = card.isRequiresNoMana()
                    ...
                    - maximumDelveReductionForGraveyardCast(gameData, playerId, card, cost, additionalCost),
                    availableConvokeContributions(gameData,
                            gameData.playerBattlefields.getOrDefault(playerId, List.of()), 0))
                    : isDisturb
            ...
            if (!card.isRequiresNoMana()
                    && !cardHasFlashback && !isDisturb && !isHarmonize && graveyardAlternateManaCost == null) {
