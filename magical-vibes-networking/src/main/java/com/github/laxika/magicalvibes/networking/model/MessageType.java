package com.github.laxika.magicalvibes.networking.model;

public enum MessageType {
    LOGIN,
    CREATE_GAME,
    JOIN_GAME,
    LOGIN_SUCCESS,
    LOGIN_FAILURE,
    TIMEOUT,
    GAME_JOINED,
    OPPONENT_JOINED,
    NEW_GAME,
    GAME_UPDATED,
    PASS_PRIORITY,
    PRIORITY_UPDATED,
    STEP_ADVANCED,
    TURN_CHANGED,
    GAME_LOG_ENTRY,
    KEEP_HAND,
    TAKE_MULLIGAN,
    HAND_DRAWN,
    MULLIGAN_RESOLVED,
    GAME_STARTED,
    SELECT_CARDS_TO_BOTTOM,
    BOTTOM_CARDS,
    DECK_SIZES_UPDATED,
    HAND_SIZES_UPDATED,
    PLAYABLE_CARDS_UPDATED,
    PLAY_CARD,
    BATTLEFIELD_UPDATED,
    TAP_PERMANENT,
    SACRIFICE_PERMANENT,
    ACTIVATE_ABILITY,
    ACTIVATE_GRAVEYARD_ABILITY,
    MANA_UPDATED,
    SET_AUTO_STOPS,
    AUTO_STOPS_UPDATED,
    DECLARE_ATTACKERS,
    DECLARE_BLOCKERS,
    AVAILABLE_ATTACKERS,
    AVAILABLE_BLOCKERS,
    LIFE_UPDATED,
    GAME_OVER,
    CHOOSE_CARD_FROM_HAND,
    CARD_CHOSEN,
    CHOOSE_PERMANENT,
    PERMANENT_CHOSEN,
    CHOOSE_CARD_FROM_GRAVEYARD,
    GRAVEYARD_CARD_CHOSEN,
    STACK_UPDATED,
    GRAVEYARD_UPDATED,
    CHOOSE_COLOR,
    COLOR_CHOSEN,
    MAY_ABILITY_CHOICE,
    MAY_ABILITY_CHOSEN,
    CHOOSE_MULTIPLE_PERMANENTS,
    MULTIPLE_PERMANENTS_CHOSEN,
    CHOOSE_MULTIPLE_CARDS_FROM_GRAVEYARDS,
    MULTIPLE_GRAVEYARD_CARDS_CHOSEN,
    REORDER_LIBRARY_CARDS,
    LIBRARY_CARDS_REORDERED,
    CHOOSE_CARD_FROM_LIBRARY,
    LIBRARY_CARD_CHOSEN,
    REVEAL_HAND,
    CHOOSE_FROM_REVEALED_HAND,
    CHOOSE_HAND_TOP_BOTTOM,
    HAND_TOP_BOTTOM_CHOSEN,
    GAME_STATE,
    ERROR,
    CREATE_DRAFT,
    DRAFT_JOINED,
    DRAFT_PACK_UPDATE,
    DRAFT_PICK,
    DECK_BUILDING_STATE,
    SUBMIT_DECK,
    TOURNAMENT_UPDATE,
    TOURNAMENT_GAME_READY,
    DRAFT_FINISHED,
    COMBAT_DAMAGE_ASSIGNMENT,
    COMBAT_DAMAGE_ASSIGNED,
    REQUEST_CARD_LIST,
    CARD_LIST_RESPONSE,
    VALID_TARGETS_REQUEST,
    VALID_TARGETS_RESPONSE,
    PAY_SEARCH_TAX,
    X_VALUE_CHOICE,
    X_VALUE_CHOSEN,
    SCRY,
    SCRY_COMPLETED,
    SURRENDER,
    LEAVE_GAME,
    LOBBY_GAMES_RESPONSE
}
