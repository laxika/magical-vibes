...
public class StitcherGeralf extends Card {
    ...
    public StitcherGeralf() {
        addActivatedAbility(new ActivatedAbility(
                true,
                "{2}{U}",
                List.of(new MillEachPlayerExileUpToTwoCreaturesCreateTokenEffect(
                        3,
                        new CreateTokenEffect("Zombie", 0, 0, CardColor.BLUE,
                                List.of(CardSubtype.ZOMBIE), Set.of(), Set.of()))),
                "{2}{U}, {T}: Each player mills three cards. Exile up to two creature cards put into graveyards this way. "
                        + "Create an X/X blue Zombie creature token, where X is the total power of the cards exiled this way."
        ));
    }
}
