...
public class SisayWeatherlightCaptain extends Card {
    ...
    public SisayWeatherlightCaptain() {
        var otherLegendaryPermanents = new PermanentAllOfPredicate(List.of(
                new PermanentHasSupertypePredicate(CardSupertype.LEGENDARY),
                new PermanentNotPredicate(new PermanentIsSourcePermanentPredicate())
        ));
        var colorsAmongOtherLegendaryPermanents = new ColorsAmongControlledPermanents(otherLegendaryPermanents);
        addEffect(EffectSlot.STATIC, new DynamicStaticBoostEffect(
                colorsAmongOtherLegendaryPermanents,
                colorsAmongOtherLegendaryPermanents,
                GrantScope.SELF
        ));

        var legendaryPermanentWithLowerManaValue = new CardAllOfPredicate(List.of(
        ...
        addActivatedAbility(new ActivatedAbility(
                false,
                "{W}{U}{B}{R}{G}",
                List.of(new SearchLibraryEffect(legendaryPermanentWithLowerManaValue,
                        LibrarySearchDestination.BATTLEFIELD,
                        new ManaValueBound(new SourcePower(), false, -1))),
