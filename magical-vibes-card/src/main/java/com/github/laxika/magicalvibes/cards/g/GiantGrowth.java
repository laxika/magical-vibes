package com.github.laxika.magicalvibes.cards.g;

import com.github.laxika.magicalvibes.cards.CardRegistration;
import com.github.laxika.magicalvibes.model.Card;
import com.github.laxika.magicalvibes.model.EffectSlot;
import com.github.laxika.magicalvibes.model.effect.BoostTargetCreatureEffect;

@CardRegistration(set = "10E", collectorNumber = "266")
@CardRegistration(set = "M10", collectorNumber = "184")
@CardRegistration(set = "M14", collectorNumber = "174")
@CardRegistration(set = "M11", collectorNumber = "178")
@CardRegistration(set = "9ED", collectorNumber = "243")
@CardRegistration(set = "8ED", collectorNumber = "254")
@CardRegistration(set = "7ED", collectorNumber = "248")
@CardRegistration(set = "6ED", collectorNumber = "233")
@CardRegistration(set = "5ED", collectorNumber = "299")
@CardRegistration(set = "4ED", collectorNumber = "248")
@CardRegistration(set = "ICE", collectorNumber = "246")
@CardRegistration(set = "RTR", collectorNumber = "125")
@CardRegistration(set = "ATH", collectorNumber = "55")
@CardRegistration(set = "FDN", collectorNumber = "223")
@CardRegistration(set = "BRO", collectorNumber = "183")
@CardRegistration(set = "BRB", collectorNumber = "29")
@CardRegistration(set = "BTD", collectorNumber = "57")
@CardRegistration(set = "SUM", collectorNumber = "199")
@CardRegistration(set = "DKM", collectorNumber = "27")
@CardRegistration(set = "WAR", collectorNumber = "162")
@CardRegistration(set = "MSH", collectorNumber = "167")
@CardRegistration(set = "DD1", collectorNumber = "21")
@CardRegistration(set = "3ED", collectorNumber = "199")
@CardRegistration(set = "DDD", collectorNumber = "14")
@CardRegistration(set = "DPA", collectorNumber = "66")
@CardRegistration(set = "ME2", collectorNumber = "167")
@CardRegistration(set = "ME3", collectorNumber = "121")
@CardRegistration(set = "ME4", collectorNumber = "156")
@CardRegistration(set = "EVG", collectorNumber = "21")
@CardRegistration(set = "GVL", collectorNumber = "14")
@CardRegistration(set = "A25", collectorNumber = "172")
@CardRegistration(set = "E02", collectorNumber = "32")
@CardRegistration(set = "SLD", collectorNumber = "717")
@CardRegistration(set = "MB1", collectorNumber = "200")
@CardRegistration(set = "SOA", collectorNumber = "52")
@CardRegistration(set = "2ED", collectorNumber = "198")
public class GiantGrowth extends Card {

    public GiantGrowth() {
        addEffect(EffectSlot.SPELL, new BoostTargetCreatureEffect(3, 3));
    }
}
