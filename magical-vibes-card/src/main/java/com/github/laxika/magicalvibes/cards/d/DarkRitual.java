package com.github.laxika.magicalvibes.cards.d;

import com.github.laxika.magicalvibes.cards.CardRegistration;
import com.github.laxika.magicalvibes.model.Card;
import com.github.laxika.magicalvibes.model.EffectSlot;
import com.github.laxika.magicalvibes.model.ManaColor;
import com.github.laxika.magicalvibes.model.effect.AwardManaEffect;

@CardRegistration(set = "5ED", collectorNumber = "153")
@CardRegistration(set = "4ED", collectorNumber = "129")
@CardRegistration(set = "ICE", collectorNumber = "120")
@CardRegistration(set = "MIR", collectorNumber = "116")
@CardRegistration(set = "TMP", collectorNumber = "118")
@CardRegistration(set = "TPR", collectorNumber = "93")
@CardRegistration(set = "ITP", collectorNumber = "19")
@CardRegistration(set = "RQS", collectorNumber = "18")
@CardRegistration(set = "USG", collectorNumber = "127")
@CardRegistration(set = "MMQ", collectorNumber = "129")
@CardRegistration(set = "BRB", collectorNumber = "18")
@CardRegistration(set = "BTD", collectorNumber = "21")
@CardRegistration(set = "SUM", collectorNumber = "99")
@CardRegistration(set = "DKM", collectorNumber = "4")
@CardRegistration(set = "CST", collectorNumber = "24")
@CardRegistration(set = "3ED", collectorNumber = "99")
@CardRegistration(set = "2ED", collectorNumber = "99")
@CardRegistration(set = "DDE", collectorNumber = "18")
@CardRegistration(set = "DDC", collectorNumber = "45")
@CardRegistration(set = "HOP", collectorNumber = "24")
@CardRegistration(set = "VMA", collectorNumber = "113")
@CardRegistration(set = "DVD", collectorNumber = "45")
@CardRegistration(set = "V13", collectorNumber = "1")
@CardRegistration(set = "MP2", collectorNumber = "21")
@CardRegistration(set = "A25", collectorNumber = "82")
@CardRegistration(set = "SLD", collectorNumber = "1170")
@CardRegistration(set = "SLD", collectorNumber = "2001")
@CardRegistration(set = "SLD", collectorNumber = "2020")
@CardRegistration(set = "MB1", collectorNumber = "82")
@CardRegistration(set = "STA", collectorNumber = "26")
@CardRegistration(set = "TMC", collectorNumber = "131")
@CardRegistration(set = "FCA", collectorNumber = "8")
@CardRegistration(set = "SLZ", collectorNumber = "38")
@CardRegistration(set = "SLZ", collectorNumber = "159")
@CardRegistration(set = "SLZ", collectorNumber = "280")
@CardRegistration(set = "ME4", collectorNumber = "74")
public class DarkRitual extends Card {

    public DarkRitual() {
        addEffect(EffectSlot.SPELL, new AwardManaEffect(ManaColor.BLACK, 3));
    }
}
