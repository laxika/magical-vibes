package com.github.laxika.magicalvibes.cards.g;

import com.github.laxika.magicalvibes.model.Card;
import com.github.laxika.magicalvibes.model.CardType;
import com.github.laxika.magicalvibes.model.EffectSlot;
import com.github.laxika.magicalvibes.model.GraveyardChoiceDestination;
import com.github.laxika.magicalvibes.model.effect.MayEffect;
import com.github.laxika.magicalvibes.model.effect.ReturnCardFromGraveyardEffect;
import com.github.laxika.magicalvibes.model.filter.CardTypePredicate;
import com.github.laxika.magicalvibes.cards.CardRegistration;

@CardRegistration(set = "10E", collectorNumber = "146")
@CardRegistration(set = "M10", collectorNumber = "97")
@CardRegistration(set = "M11", collectorNumber = "98")
@CardRegistration(set = "M12", collectorNumber = "99")
@CardRegistration(set = "9ED", collectorNumber = "136")
@CardRegistration(set = "POR", collectorNumber = "95")
@CardRegistration(set = "8ED", collectorNumber = "138")
@CardRegistration(set = "7ED", collectorNumber = "139")
@CardRegistration(set = "6ED", collectorNumber = "134")
@CardRegistration(set = "AKH", collectorNumber = "93")
@CardRegistration(set = "TMP", collectorNumber = "137")
@CardRegistration(set = "M15", collectorNumber = "99")
@CardRegistration(set = "M19", collectorNumber = "98")
@CardRegistration(set = "ODY", collectorNumber = "140")
@CardRegistration(set = "M20", collectorNumber = "103")
@CardRegistration(set = "TPR", collectorNumber = "106")
@CardRegistration(set = "S99", collectorNumber = "78")
public class Gravedigger extends Card {

    public Gravedigger() {
        addEffect(EffectSlot.ON_ENTER_BATTLEFIELD, new MayEffect(ReturnCardFromGraveyardEffect.builder().destination(GraveyardChoiceDestination.HAND).filter(new CardTypePredicate(CardType.CREATURE)).build(), "Return a creature card from your graveyard to your hand?"));
    }
}
