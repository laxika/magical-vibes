...
| `DoubleControllerDamageEffect` | `()` | double all damage dealt by sources controlled by this permanent's controller (static). Unlike DoubleDamageEffect (global), only affects the controller's sources. Unlike DoubleControllerSpellDamageEffect (instants/sorceries only), affects all sources including combat damage, abilities, and spells. Used by Angrath's Marauders |
