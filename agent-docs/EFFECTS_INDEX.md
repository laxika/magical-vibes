...
| `PutCounterOnTargetPermanentEffect` | `(CounterType counterType)` | put a counter of the specified type on target permanent. Supports all CounterType values (CHARGE, LORE, PLUS_ONE_PLUS_ONE, MINUS_ONE_MINUS_ONE, etc.). For LORE counters on Sagas, also triggers the appropriate chapter ability (MTG Rule 714.3b). Use with target filters to restrict valid targets (e.g. ControlledPermanentPredicateTargetFilter + PermanentHasSubtypePredicate(SAGA) for "target Saga you control") |
