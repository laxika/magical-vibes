...
Effects returning `true`: `BoostSelfEffect`, `DoubleSelfPowerToughnessEffect`, `TapSelfEffect`, `UntapSelfEffect`, `AnimateSelfEffect`, `AnimateSelfByChargeCountersEffect`, `AnimateSelfWithStatsEffect`, `AnimateLandEffect`, `PutChargeCounterOnSelfEffect`, `PutSlimeCounterAndCreateOozeTokenEffect`.
...
Effects that target both players and permanents (any-target): DealDamageToAnyTargetEffect, DealDamageToAnyTargetEqualToChargeCountersOnSourceEffect, DealDamageToAnyTargetAndGainLifeEffect, DealOrderedDamageToAnyTargetsEffect, DealXDamageToAnyTargetEffect, DealXDamageToAnyTargetAndGainXLifeEffect, DealXDamageToEachTargetEffect.
...
| `SubtypeConditionalEffect` | `(CardSubtype subtype, CardEffect wrapped)` | Wraps any trigger with "triggering creature has subtype" condition. Works with ON_ALLY_CREATURE_ENTERS_BATTLEFIELD (checked in BattlefieldEntryService), ON_ANY_CREATURE_DIES and ON_ALLY_CREATURE_DIES (checked in TriggerCollectionService), and ON_EQUIPPED_CREATURE_DIES (checked in TriggerCollectionService). Creates stack entry with sourcePermanentId. Used by Champion of the Parish (HUMAN, PutCountersOnSourceEffect(1,1,1)), Village Cannibals (HUMAN, PutCountersOnSourceEffect(1,1,1)), and Avacyn's Collar (HUMAN, CreateTokenEffect for Spirit token) |
...
| `ControlsAnotherSubtypeConditionalEffect` | `(CardSubtype subtype, CardEffect wrapped)` or `(CardSubtype subtype, boolean nontokenOnly, CardEffect wrapped)` or `(Set<CardSubtype> subtypes, boolean nontokenOnly, CardEffect wrapped)` | Intervening-if wrapper: "if you control another [subtype]". Checked at trigger time and resolution time per CR 603.4. The condition is met when the controller has at least one other permanent with any of the specified subtypes besides the source. When `nontokenOnly` is true, only nontoken permanents satisfy the condition. The `Set<CardSubtype>` constructor supports OR-logic across multiple subtypes (e.g. "another Merfolk or an Island"). Used by Ghitu Journeymage (WIZARD), Fathom Fleet Captain (PIRATE, nontokenOnly=true), Kumena's Speaker (MERFOLK+ISLAND) |
...
| `ControllerGraveyardCardThresholdConditionalEffect` | `(int threshold, CardPredicate filter, CardEffect wrapped)` | Wraps any effect with "as long as there are N or more [filtered] cards in your graveyard" condition. For static effects: wraps StaticBoostEffect or GrantKeywordEffect, applied only while controller's graveyard contains >= threshold non-token cards matching filter (selfOnly handler). Used by Ghitu Lavarunner with threshold=2, instant/sorcery filter, wrapping StaticBoostEffect(+1/+0, HASTE) |
| `ControllerTurnConditionalEffect` | `(CardEffect wrapped)` | Wraps any effect with "during your turn" condition. The wrapped effect only applies when the source permanent's controller is the active player. Supports GrantKeywordEffect and StaticBoostEffect as wrapped effects. Used by Jousting Lance for conditional first strike |
| `ControllerLifeThresholdConditionalEffect` | `(int lifeThreshold, CardEffect wrapped)` | Wraps any effect with "as long as you have N or more life" condition. Works in both static effects (wraps StaticBoostEffect or GrantKeywordEffect, applied only while controller's life total >= threshold) and effect resolution (intervening-if in activated abilities, checked at resolution time in EffectResolutionService). Used by Serra Ascendant with threshold 30 (static), Chalice of Life with threshold 30 wrapping TransformSelfEffect (activated ability) |
| `ControllerLifeAtOrBelowThresholdConditionalEffect` | `(int lifeThreshold, CardEffect wrapped)` | Wraps any effect with "if you have N or less life" intervening-if condition. Checked at trigger time (StepTriggerService) and resolution time (EffectResolutionService). Controller's life total must be <= threshold. Used by Arguel's Blood Fast with threshold 5 wrapping MayEffect(TransformSelfEffect()) |
| `ControlsPermanentConditionalEffect` | `(PermanentPredicate filter, CardEffect wrapped)` | Wraps any effect with "if you control a [permanent matching predicate]" condition. Checked at resolution time. More general than ControlsSubtypeConditionalEffect — works with any PermanentPredicate (e.g. PermanentIsArtifactPredicate for card type checks). Used by Temporal Machinations with PermanentIsArtifactPredicate wrapping DrawCardEffect(1) |
| `ControlsPermanentCountConditionalEffect` | `(int minCount, PermanentPredicate filter, CardEffect wrapped)` | Wraps any effect with "if you control N or more [permanents matching predicate]" intervening-if condition. Checked at trigger time (StepTriggerService CONTROLLER_END_STEP_TRIGGERED) and resolution time (EffectResolutionService). Used by Growing Rites of Itlimoc with minCount=4, PermanentIsCreaturePredicate, wrapping TransformSelfEffect() |
| `ControlsSubtypeConditionalEffect` | `(CardSubtype subtype, CardEffect wrapped)` | Wraps any effect with "as long as you control a [subtype]" / "if you control a [subtype]" condition. For static effects: wraps GrantKeywordEffect, StaticBoostEffect, ProtectionFromColorsEffect, or GrantEffectEffect, applied only while controller has at least one permanent with the specified subtype (selfOnly handler). For triggered effects (e.g. ON_ATTACK): intervening-if checked at trigger time and resolution time per CR 603.4. Used by Angelic Overseer with HUMAN, Karplusan Hound with CHANDRA, Teferi's Sentinel with TEFERI, Jace's Sentinel with JACE |
| `ControlsSubtypeReplacementEffect` | `(CardSubtype subtype, CardEffect baseEffect, CardEffect upgradedEffect)` | Replacement wrapper: picks between baseEffect and upgradedEffect based on whether the controller controls a permanent with the specified subtype. Used for planeswalker-signature spells like "If you control a [subtype] planeswalker, do X instead." Condition checked at resolution time. Used by Huatli's Spurring with HUATLI |
...
| `AnyPlayerControlsColorConditionalEffect` | `(CardColor color, CardEffect wrapped)` | Wraps any effect with "as long as any player controls a [color] permanent" condition. For static effects: wraps GrantKeywordEffect, StaticBoostEffect, or ProtectionFromColorsEffect, applied only while at least one permanent of the specified color exists on any player's battlefield (selfOnly handler). Used by Knight of Grace with BLACK |
...
| `AttacksAloneConditionalEffect` | `(CardEffect wrapped)` | Trigger-condition wrapper: "whenever this creature attacks alone". Filtered at trigger collection time in CombatAttackService (only fires if creature is the sole attacker, per CR 506.5) and re-checked at resolution time. Place in ON_ATTACK slot. Used by Grunn, the Lonely King wrapping DoubleSelfPowerToughnessEffect |
| `MinimumAttackersConditionalEffect` | `(int minimumAttackers, CardEffect wrapped)` | Trigger-condition wrapper: "whenever you attack with N or more creatures". Filtered at trigger collection time in CombatAttackService (only fires if attacker count >= minimumAttackers) and re-checked at resolution time using xValue. Place in GRAVEYARD_ON_ALLY_CREATURES_ATTACK slot. Used by Warcry Phoenix with minimumAttackers=3 wrapping MayPayManaEffect |
| `HasNontokenSubtypeAttackerConditionalEffect` | `(CardSubtype requiredSubtype, CardEffect wrapped)` | Trigger-condition wrapper: "whenever one or more nontoken [subtype] you control attack". Filtered at trigger collection time in CombatAttackService (only fires if at least one nontoken permanent with the required subtype is attacking) and re-checked at resolution time. Place in ON_ALLY_CREATURES_ATTACK slot. Used by Mavren Fein, Dusk Apostle with VAMPIRE wrapping CreateTokenEffect |
| `SacrificedPermanentSubtypeConditionalEffect` | `(CardSubtype requiredSubtype, CardEffect wrapped)` | Trigger-condition wrapper for ON_ALLY_PERMANENT_SACRIFICED: "whenever you sacrifice a [subtype]". Checked at trigger collection time in MiscTriggerCollectorService — only fires if the sacrificed permanent had the specified subtype. Used by Captain Lannery Storm with TREASURE wrapping BoostSelfEffect(1, 0) |
| `BlockedByMinCreaturesConditionalEffect` | `(int minBlockers, CardEffect wrapped)` | Wraps any effect with "as long as N or more creatures are blocking this" condition. For static effects: wraps StaticBoostEffect or GrantKeywordEffect, applied only while source is blocked by >= minBlockers creatures (selfOnly handler). Used by Rampaging Cyclops with minBlockers=2, StaticBoostEffect(-2, 0, SELF) |
...
| `RaidReplacementEffect` | `(CardEffect baseEffect, CardEffect raidEffect)` | Picks between base and upgraded effect at resolution based on raid. Resolves `raidEffect` if the controller attacked with a creature this turn, otherwise `baseEffect`. Targeting delegates to both inner effects (union). No new handler needed — unwrapped in `EffectResolutionService`. Used by Firecannon Blast |
...
| `DealDamageToTargetCreatureEffect` | `(int damage)` or `(int damage, boolean unpreventable)` | deal N damage to target creature. When `unpreventable=true`, bypasses all damage prevention (shields, protection, global prevention). Supports multi-target: when used with `target(min, max)` or multi-target activated ability constructor, deals damage to each creature in `targetIds` |
...
| `DealDamageToTargetPlayerEffect` | `(int damage)` | deal N damage to target player. Also usable on ON_ANY_OTHER_CREATURE_ENTERS_BATTLEFIELD slot where targetId is set to the entering creature's controller (e.g. Rampaging Ferocidon) |
...
| `SacrificePermanentThenEffect` | `(PermanentPredicate filter, CardEffect thenEffect, String permanentDescription)` | "Sacrifice a [permanent matching filter]. If you do, [thenEffect]." Controller must sacrifice a permanent matching the predicate; if successful, the wrapped effect executes. If no matching permanent exists, nothing happens. Uses `PermanentChoiceContext.SacrificePermanentThen` for interactive choice |
...
| `TargetPlayerSacrificesPermanentsEffect` | `(int count, PermanentPredicate filter)` | targeted player (from stack entry's targetId) sacrifices N permanents matching filter. Use with `canTargetPlayer()`. If fewer matching permanents than count, all are sacrificed. Use PermanentTruePredicate for any permanent. Used by Storm Fleet Arsonist |
...
| `SacrificePermanentCost` | `(PermanentPredicate filter, String description)` | sacrifice a permanent matching filter as cost. Works for both spell costs and activated abilities (via `MultiplePermanentSacrificeCostHandler`). Uses `GameQueryService.matchesPermanentPredicate()` for validation. E.g. `new SacrificePermanentCost(new PermanentAnyOfPredicate(List.of(new PermanentIsArtifactPredicate(), new PermanentIsCreaturePredicate())), "Sacrifice an artifact or creature")` |
...
| `DiscardCardAndUntapSelfEffect` | `()` | discard a card, then untap the source permanent. Commonly wrapped in MayEffect for "you may discard a card. If you do, untap [source]." Uses `pendingUntapAfterDiscardPermanentId` on GameData to untap after the interactive discard completes. Used by Elaborate Firecannon |
...
| `LookAtTopCardsMayRevealByPredicatePutIntoHandRestOnBottomEffect` | `(int count, CardPredicate predicate)` or `(int count, CardPredicate predicate, boolean anyNumber)` | look at top N, may reveal card matching predicate to hand, rest on bottom; anyNumber=true allows multi-select. Use CardTypePredicate, CardAnyOfPredicate, CardIsHistoricPredicate, etc. |
...
| `LookAtTopCardsChooseNToHandRestToGraveyardEffect` | `(int count, int toHandCount)` | look at top N cards, choose toHandCount to hand, rest to graveyard. Used by Forbidden Alchemy (4, 1), Dark Bargain (3, 2) |
...
| `ExileTargetPermanentAndReturnImmediatelyEffect` | `()` or `(CardSubtype bonusSubtype, CardEffect bonusEffect)` | exile target permanent, immediately return it to the battlefield under its owner's control (Cloudshift-style instant flicker). No-arg constructor for plain flicker. When `bonusSubtype` is set, checks if the exiled permanent had that subtype and applies the bonus effect (e.g. `DrawCardEffect(1)` for Siren's Ruse). Provider: `ExileResolutionService` |
...
| Effect | Constructor | Intent |
|--------|-------------|--------|
| `CreateTokenEffect` | `(String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, Set<Keyword> keywords, Set<CardType> additionalTypes)` or `(int amount, ...)` or multi-color: `(int amount, String tokenName, int power, int toughness, CardColor color, Set<CardColor> colors, List<CardSubtype> subtypes)` or `(String tokenName, int power, int toughness, CardColor color, Set<CardColor> colors, List<CardSubtype> subtypes)` or tapped-and-attacking: `(int amount, String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, boolean tappedAndAttacking)` or tapped-attacking-with-keywords-exile: `(int amount, String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, Set<Keyword> keywords, boolean tappedAndAttacking, boolean exileAtEndOfCombat)` or enters-tapped: `(int amount, String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, Set<Keyword> keywords, Set<CardType> additionalTypes, boolean tapped)` or with-token-effects: `(int amount, String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, Set<Keyword> keywords, Set<CardType> additionalTypes, Map<EffectSlot, CardEffect> tokenEffects)` or factory: `CreateTokenEffect.ofArtifactToken(int amount, String tokenName, List<CardSubtype> subtypes, List<ActivatedAbility> abilities)` or static factories for common tokens: `CreateTokenEffect.whiteSpirit(int amount)` (1/1 white Spirit with flying), `CreateTokenEffect.blackZombie(int amount)` (2/2 black Zombie), `CreateTokenEffect.whiteSoldier(int amount)` (1/1 white Soldier), `CreateTokenEffect.ofTreasureToken(int amount)` (Treasure artifact) or canonical (all params): `(CardType primaryType, int amount, String tokenName, int power, int toughness, CardColor color, Set<CardColor> colors, List<CardSubtype> subtypes, Set<Keyword> keywords, Set<CardType> additionalTypes, boolean tappedAndAttacking, boolean tapped, Map<EffectSlot, CardEffect> tokenEffects, List<ActivatedAbility> tokenAbilities, boolean exileAtEndOfCombat, boolean exileAtEndStep, boolean legendary)` | create N tokens. `primaryType` determines the token's card type (CREATURE for creature tokens, ARTIFACT for artifact tokens like Treasure/Clue/Food); all convenience constructors default to CREATURE. `tokenAbilities` adds activated abilities to the token (e.g. Treasure's tap-sacrifice-for-mana). For non-creature tokens, power/toughness are ignored and `handleCreatureEnteredBattlefield` is not called. `ofArtifactToken` factory creates colorless artifact tokens with abilities. `color` is primary display color. `colors` (Set&lt;CardColor&gt;, nullable) is full color identity for multi-color tokens. Multi-color constructors default keywords/additionalTypes to empty sets. When `tappedAndAttacking=true`, tokens enter the battlefield tapped and attacking (CR 508.8) — used for "whenever ~ attacks, create tokens tapped and attacking" abilities (e.g. Hero of Bladehold). When `tapped=true`, tokens enter the battlefield tapped but not attacking — used for cards like Army of the Damned. When `exileAtEndOfCombat=true`, tokens are exiled at end of combat step — used for temporary attack tokens (e.g. Geist of Saint Traft). When `tokenEffects` is non-empty, each entry's effect is registered on the token card at the given slot, giving the token its own triggered abilities (e.g. Mitotic Slime's cascading death triggers). When `legendary=true`, the token has the LEGENDARY supertype (subject to legend rule) — used for named legendary tokens (e.g. Verix Bladewing creating Karox Bladewing) |
...
| `CreateTokenPerAttachmentOnSourceEffect` | `(String tokenName, int power, int toughness, CardColor color, List<CardSubtype> subtypes, Set<Keyword> keywords, Set<CardType> additionalTypes, boolean countAuras, boolean countEquipment, boolean exileAtEndStep)` | create creature tokens equal to the number of matching attachments (Auras and/or Equipment based on flags) on the source permanent. If `exileAtEndStep` is true, tokens are exiled at the beginning of the next end step. Generalizes CreateTokenPerEquipmentOnSourceEffect. Used by Valduk, Keeper of the Flame |
...
| `AnyPlayerControlsPermanentConditionalEffect` | `(PermanentPredicate filter, CardEffect wrapped)` | conditional wrapper: "as long as any player controls a permanent matching [filter]". Wraps StaticBoostEffect, GrantKeywordEffect, or ProtectionFromColorsEffect (selfOnly handler). Used by Knight of Malice with `PermanentColorInPredicate(Set.of(WHITE))` wrapping `StaticBoostEffect(1, 0, SELF)` |
...
| `PutPlusOnePlusOneCounterOnEachOwnCreatureEffect` | `()` or `(int count)` | put one or more +1/+1 counters on each creature you control. No-arg defaults to 1 counter |
...
| `PutPlusOnePlusOneCounterOnFirstTargetEffect` | `(int count)` | unconditionally put N +1/+1 counters on the first target in a multi-target spell (e.g. Savage Stomp). Use when the counter placement is not conditional on the target's properties |
...
| `EnterWithPlusOnePlusOneCountersIfRaidEffect` | `(int count)` | enters battlefield with N +1/+1 counters if you attacked this turn (raid replacement effect). E.g. Rigging Runner |
...
| `CastFromHandConditionalEffect` | `(CardEffect wrapped)` | conditional wrapper that resolves its inner effect only when the permanent was cast from hand (MTG Rule 603.4 intervening-if). Place on ON_ENTER_BATTLEFIELD slot. Use for "When this enters, if you cast it from your hand, [effect]" patterns. E.g. Wakening Sun's Avatar uses `CastFromHandConditionalEffect(new DestroyAllPermanentsEffect(...))` |
| `KickedConditionalEffect` | `(CardEffect wrapped)` | conditional wrapper that resolves its inner effect only when the spell was kicked (MTG Rule 702.32c). Place on SPELL slot alongside other spell effects. Use when kicked *adds* an extra effect. E.g. Blink of an Eye uses `KickedConditionalEffect(new DrawCardEffect())` |
| `NotKickedConditionalEffect` | `(CardEffect wrapped)` | conditional wrapper that resolves its inner effect only when the permanent was NOT kicked. Intervening-if checked at trigger time (StepTriggerService) and resolution time (EffectResolutionService). Place on END_STEP_TRIGGERED slot. Use for "if wasn't kicked, sacrifice it" patterns. E.g. Skizzik uses `NotKickedConditionalEffect(new SacrificeSelfEffect())` |
| `KickerReplacementEffect` | `(CardEffect baseEffect, CardEffect kickedEffect)` | replacement wrapper that picks between base and kicked effects. At resolution, if kicked resolves `kickedEffect`; otherwise resolves `baseEffect`. Use when kicked mode *replaces* the base mode ("instead" in oracle text). E.g. Fight with Fire uses `KickerReplacementEffect(DealDamageToTargetCreatureEffect(5), DealDividedDamageAmongAnyTargetsEffect(10))` |
...
| `EnterWithPlusOnePlusOneCountersPerCreatureDeathsThisTurnEffect` | `()` | ETB replacement effect: enters with a +1/+1 counter for each creature that died this turn (across all players, including tokens). Handled in `BattlefieldEntryService.putPermanentOntoBattlefield()`. Used by Bloodcrazed Paladin |
...
| `PutChargeCounterOnTargetPermanentEffect` | `()` | **LEGACY** — use `PutCounterOnTargetPermanentEffect(CounterType.CHARGE)` instead. Kept for backward compatibility; handler delegates to the generic effect |
| `PutCounterOnTargetPermanentEffect` | `(CounterType counterType)` or `(CounterType counterType, int count, PermanentPredicate predicate)` | put counter(s) of the specified type on a permanent. Single-arg form: targets a permanent (canTargetPermanent=true), puts 1 counter. Three-arg form with non-null predicate: resolution-time choice from controller's battlefield matching predicate (non-targeting, canTargetPermanent=false), auto-selects if one match, prompts multi-choice if multiple, does nothing if none. Supports all CounterType values including LOYALTY. For LORE counters on Sagas, also triggers chapter abilities (MTG Rule 714.3b). Used by Settle the Score (LOYALTY, 2, PermanentIsPlaneswalkerPredicate) |
...
| `PutXPlusOnePlusOneCountersOnTargetPermanentEffect` | `()` | put X +1/+1 counters on target permanent, where X comes from the spell/ability's X value (targets permanent). Used by Waker of the Wilds |
...
| Effect | Constructor | Intent |
|--------|-------------|--------|
| `GrantKeywordEffect` | `(Keyword keyword, GrantScope scope)` or `(Keyword keyword, GrantScope scope, PermanentPredicate filter)` or `(Set<Keyword> keywords, GrantScope scope)` or `(Set<Keyword> keywords, GrantScope scope, PermanentPredicate filter)` | grant one or more keywords. Scope: `SELF`, `TARGET`, `ENCHANTED_CREATURE`, `EQUIPPED_CREATURE`, `OWN_TAPPED_CREATURES`, `OWN_CREATURES`, `ALL_CREATURES`, `ENCHANTED_PLAYER_CREATURES`. Optional predicate filter for conditional grants. Use `Set<Keyword>` overload when granting multiple keywords at once (e.g. flying + vigilance + lifelink) |
...
| `GrantKeywordToTargetIfSubtypeEffect` | `(Keyword keyword, CardSubtype subtype)` | grant keyword to targeted permanent until end of turn, but only if it has the specified subtype (e.g. VAMPIRE). Used by Vampire's Zeal |
...
| `TapAllAttackingCreaturesEffect` | `()` | tap all attacking creatures across all battlefields; no targeting, no parameters. Used by fog variants (e.g. Clinging Mists) |
| `SkipNextUntapAllAttackingCreaturesEffect` | `()` | all attacking creatures across all battlefields don't untap during their controller's next untap step; increments `skipUntapCount` on each attacking creature. Pair with `TapAllAttackingCreaturesEffect` for "tap all attackers, they don't untap" (e.g. Clinging Mists fateful hour) |
...
| `AwardManaEqualToSourcePowerEffect` | `(ManaColor color)` | add mana of the specified color equal to the source permanent's effective power. Implements `ManaProducingEffect` so the ability is treated as a mana ability. Produces no mana if power is 0 or less. Tracks creature mana when source is a creature. Used by Marwyn, the Nurturer |
...
| `CopySpellEffect` | `()` or `(StackEntryPredicate spellFilter)` | copy target spell. No-arg form for SPELL slot (filter on Card's SpellTarget). Parameterized form for ON_ENTER_BATTLEFIELD slot — carries the filter to restrict which spells can be targeted (e.g. Naru Meha uses `StackEntryAllOfPredicate(StackEntryTypeInPredicate(INSTANT, SORCERY), StackEntryControlledByPredicate())`) |
...
| `ReduceOwnCastCostIfTargetingControlledSubtypeEffect` | `(CardSubtype subtype, int amount)` | reduce this spell's cast cost by N if it targets a permanent with the given subtype that the caster controls (static, Savage Stomp). Playability optimistically checks if controller controls the subtype; actual reduction validated against chosen first target in SpellCastingService |
