...
| `PutCounterOnSelfThenTransformIfThresholdEffect` | `(CounterType counterType, int threshold)` or `(CounterType counterType, int threshold, boolean optional)` | put a counter of the specified type on this permanent, then if the count reaches the threshold, remove all counters of that type and transform. When `optional` is true, the player is prompted ("you may") before removing counters and transforming. Used by Ludevic's Test Subject (HATCHLING, 5), Primal Amulet (CHARGE, 4, true) |
