...
| `canTargetSpell()` | CounterSpellEffect, CounterSpellIfControllerPoisonedEffect, CounterUnlessPaysEffect, CopySpellEffect, ChangeTargetOfTargetSpellWithSingleTargetEffect, ChangeTargetOfTargetSpellToSourceEffect |
...
| `RemoveCountersFromTargetAndBoostSelfEffect` | `()` | remove up to X counters of any type from target permanent (X from xValue), then boost source +1/+0 per counter removed until end of turn. Removes in order: +1/+1, charge, loyalty, -1/-1, awakening. Used by Hex Parasite |
...
| `EntersTappedUnlessControlLandSubtypeEffect` | `(List<CardSubtype> requiredSubtypes)` | enters tapped unless you control a permanent with one of the required land subtypes (check lands, static) |
