...
| `LookAtTopXCardsPermanentsToBattlefieldRestToGraveyardEffect` | `(CardPredicate alwaysEligiblePredicate, CardPredicate mvCappedEligiblePredicate)` | look at top X cards (X from spell's xValue), choose any number of eligible cards to put onto battlefield, rest to graveyard. Cards matching alwaysEligiblePredicate are eligible regardless of MV; cards matching mvCappedEligiblePredicate are eligible only if MV ≤ X. Either predicate may be null. Used by Kamahl's Druidic Vow: `(CardTypePredicate(LAND), CardAllOfPredicate(CardSupertypePredicate(LEGENDARY), CardIsPermanentPredicate()))` |
