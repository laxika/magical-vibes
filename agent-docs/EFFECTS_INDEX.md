...
| `LoseAllAbilitiesUntilEndOfTurnEffect` | `()` | one-shot: target permanent loses all abilities until end of turn. Sets `Permanent.losesAllAbilitiesUntilEndOfTurn` flag (cleared by `resetModifiers()`). Keywords, activated abilities, and triggered abilities are suppressed. Used by Merfolk Trickster. Handler: `KeywordGrantResolutionService` |
