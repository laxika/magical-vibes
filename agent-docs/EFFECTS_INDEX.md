...
| `ControlsAnotherSubtypeConditionalEffect` | `(CardSubtype subtype, CardEffect wrapped)` | Intervening-if wrapper: "if you control another [subtype]". Checked at trigger time and resolution time per CR 603.4. The condition is met when the controller has at least one other permanent with the specified subtype besides the source. Used by Ghitu Journeymage with WIZARD |
