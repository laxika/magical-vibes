...
| `ControlsAnotherSubtypeConditionalEffect` | `(CardSubtype subtype, CardEffect wrapped)` or `(CardSubtype subtype, boolean nontokenOnly, CardEffect wrapped)` | Intervening-if wrapper: "if you control another [subtype]". Checked at trigger time and resolution time per CR 603.4. The condition is met when the controller has at least one other permanent with the specified subtype besides the source. When `nontokenOnly` is true, only nontoken permanents satisfy the condition (e.g. "another nontoken Pirate"). Used by Ghitu Journeymage (WIZARD), Fathom Fleet Captain (PIRATE, nontokenOnly=true) |
