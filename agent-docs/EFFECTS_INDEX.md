...
| `DealDamageToTargetOpponentOrPlaneswalkerEffect` | `(int damage)` | deal N damage to target opponent or planeswalker. canTargetPlayer + canTargetPermanent. Validator enforces opponent-only for players, planeswalker-only for permanents. Used by Burning Sun's Avatar |
