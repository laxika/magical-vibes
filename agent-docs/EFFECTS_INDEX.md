...
| `MorbidConditionalEffect` | `(CardEffect wrapped)` | Wraps any effect with morbid condition (a creature died this turn). For ETB triggers: checked at trigger time (intervening-if) and resolution time, delegates targeting to wrapped effect. Used by Festerhide Boar with PutCountersOnSourceEffect |
