...
| `ExileNCardsFromGraveyardCost` | `(int count, CardType requiredType)` | exile exactly N cards of the specified type from your graveyard as an additional cost to cast a spell (null requiredType = any). Add to SPELL slot and pass `exileGraveyardCardIndices` (List<Integer>) via `PlayCardRequest`. `GameBroadcastService` checks graveyard has enough matching cards for castability. Handled by `SpellCastingService`. E.g. Skaab Goliath (2, CREATURE) |
