...
| Metalcraft damage + can't block | `c/ConcussiveBolt.java` | SPELL DealDamageToPlayersEffect(4, DamageRecipient.TARGET_PLAYER) + ConditionalEffect(new Metalcraft(), TargetPlayerCreaturesCantBlockThisTurnEffect) — damage always, metalcraft adds mass can't-block on target player's creatures |
