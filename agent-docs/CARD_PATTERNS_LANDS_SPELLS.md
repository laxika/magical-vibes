...
| Pump + debuff (two distinct targets) | `l/LeechingBite.java` | BoostFirstTargetCreatureEffect + BoostSecondTargetCreatureEffect, multi-target with creature filters. Oracle text says "Another target creature", so cross-group distinct (the default) applies — no `setAllowSharedTargets` |
