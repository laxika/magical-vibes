...
| Protection choice + fateful hour "instead" | `f/FaithsShield.java` | target(ControlledPermanentPredicateTargetFilter(PermanentTrue)) with two **mutually exclusive** conditionals: ControllerLifeThresholdConditionalEffect(6, GrantProtectionChoiceUntilEndOfTurnEffect) [≥6 life → target only] and ControllerLifeAtOrBelowThresholdConditionalEffect(5, GrantProtectionChoiceToControllerAndPermanentsUntilEndOfTurnEffect) [≤5 life → you + all your permanents]. Only one fires → single color prompt. Spell always requires the target (fizzles if it becomes illegal). Pattern for any "fateful hour — instead" spell |
...
| Static self-boost per controlled subtype | `e/EarthServant.java` | STATIC BoostSelfPerControlledPermanentEffect(0, 1, PermanentHasSubtypePredicate(MOUNTAIN)) — +0/+1 for each Mountain you control |
| Static conditional boost (another multi-subtype) | `k/KumenasSpeaker.java` | STATIC ControlsAnotherPermanentConditionalEffect(PermanentHasAnySubtypePredicate(Set.of(MERFOLK, ISLAND)), StaticBoostEffect(1, 1, SELF)) — +1/+1 as long as you control another Merfolk or Island |
