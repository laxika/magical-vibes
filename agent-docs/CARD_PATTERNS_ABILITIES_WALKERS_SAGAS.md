...
| Sacrifice self + prevent target's combat damage | `r/ResistanceFighter.java` | SacrificeSelfCost + PreventAllDamageByTargetCreatureEffect(true) — no-mana ability, single creature target; `combatOnly=true` prevents only combat damage |
