...
| `CardHasCyclingPredicate` | `()` | a card with cycling — detected via a hand-activated ability whose description begins with `"Cycling"` (the engine's cycling convention, see `AbilityActivationService`). Use as the filter of `ReturnTargetCardsFromGraveyardToHandEffect` for "cards with cycling" (Sacred Excavation) |
