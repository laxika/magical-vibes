...
| `PermanentSharesCreatureTypeWithEnchantedCreaturePredicate` | `()` | permanents sharing at least one creature type with the creature the **source Aura** enchants; Changeling counts as every creature type. Requires live game data and source identity; never matches while unattached. Alpha Status |
| `PermanentSharesCreatureTypeWithSourcePermanentPredicate` | `()` | permanents sharing at least one effective creature type with the permanent carrying the source ability; Changeling and layered/transient creature types are respected. Useful for source-relative enter triggers such as Plane-Merge Elf's Kinfall |
...
| `PermanentIsAuraAttachedToAttackingCreatureControlledByOpponentOfSourceControllerPredicate` | `()` | an Aura permanent currently attached to an attacking creature controlled by an opponent of the source controller — needs game data + `sourceControllerId` |
...
| `PermanentAttackedThisTurnPredicate` | `()` | creatures that were declared as attackers at some point this turn, including creatures later removed from combat. Unlike `PermanentIsAttackingPredicate`, this reads the turn-persistent attack marker. Total War |
...
| `PermanentBlockedThisTurnPredicate` | `()` | creatures that blocked another creature this turn (blocker direction only). Reads `GameData.combatOpponentIdsBlockedByThisTurn`, so it still matches after combat ends; AND with `PermanentIsCreaturePredicate` for "target creature that blocked this turn" (Sizzling Barrage) |
...
| `PermanentPowerToughnessTotalAtLeastPredicate` | `(int minTotal)` | permanents whose effective power plus toughness is at least N |
| `PermanentPowerToughnessTotalAtMostPredicate` | `(int maxTotal)` | permanents whose effective power plus toughness is at most N |
...
| `PermanentToughnessLessThanBasicLandTypesAmongControlledLandsPredicate` | `()` | permanents with toughness < the number of distinct basic land types among lands controlled by the source's controller (Zar Ojanen, Scion of Efrava) |
...
| `PermanentToughnessGreaterThanPowerPredicate` | `()` | permanents whose effective toughness is greater than their effective power (e.g. Bedrock Tortoise) |
...
| `PermanentManaValueAtMostControllerGraveyardCountPredicate` | `()` | permanents with mana value <= the number of non-token cards in that permanent's controller's graveyard | `gameData` |
...
| `PermanentMaxManaValueColorsSpentToCastPredicate` | `()` | permanents whose mana value is at most the number of distinct colors of mana spent to cast the evaluating spell; permissive before payment and exact during resolution | `gameData` + `sourceCardId` |
...
| `PermanentPowerGreaterThanBasePowerPredicate` | `()` | permanents whose effective power is greater than their current base power; power modifiers and counters count, while base-power-setting effects define the comparison baseline | `gameData` |
| `PermanentBasePowerEqualsPredicate` | `(int basePower)` | permanents whose current base power equals the supplied value; base-power-setting effects define the comparison baseline | `gameData` |
| `PermanentPowerLessThanSourcePowerPredicate` | `()` | creatures with power strictly < source permanent's effective power (Champion of Lambholt: "Creatures with power less than this creature's power can't block creatures you control"). During cast-time ETB validation, when the source is not yet on the battlefield, falls back to the source card's base power; resolution-time checks can use `FilterContext.sourcePermanentSnapshot` | `gameData` + `sourceCardId` |
...
| `PermanentHasSourceChosenColorPredicate` | `()` | permanents carrying a color chosen by the **source** as it entered the battlefield (`Permanent.chosenColor`); matches nothing when the source is gone or made no choice. Supported in the context-aware static-filter path as well as ordinary predicate evaluation | `gameData` + `sourceCardId` |
| `PermanentHasChosenSpellColorPredicate` | `()` | permanents carrying the color chosen by the currently resolving spell or ability (`GameData.chosenSpellColor`) | `gameData` |
...
| `PermanentCastForWarpCostPredicate` | `()` | permanents whose spell was cast for its Warp cost | — |
...
| `PermanentControllerControlsPermanentCountAtMostPredicate` | `(int maxCount, PermanentPredicate countFilter)` | permanents whose OWN controller controls at most `maxCount` permanents matching `countFilter` (Sheltering Prayers — basic lands whose controller controls three or fewer lands). Supported in `PredicateEvaluationService.matchesStaticFilter`, so it works as a dynamic static filter | `gameData` |
...
| `StackEntryManaSpentLessThanManaValuePredicate` | `()` | spells whose total mana spent to cast them is less than their mana value, including chosen X; usable as a `SpellCastTriggerEffect.castSpellTargetCondition` |
| `StackEntryHasXInManaCostPredicate` | `()` | spells whose mana cost contains `{X}`, regardless of the chosen X. Used by Frontline Medic |
| `StackEntryManaValueEqualsSourcePowerPredicate` | `()` | spells whose mana value (including chosen X) equals the evaluating source permanent's effective power. Source-dependent: evaluated by `TargetLegalityService` with the source permanent |
| `StackEntryManaValueAtMostSourcePowerPredicate` | `()` | spells whose mana value (including chosen X) is at most the evaluating source permanent's effective power. Source-dependent: evaluated by `TargetLegalityService` with the source permanent |
| `StackEntryManaValuePowerOrToughnessEqualsSourceChosenNumberPredicate` | `()` | spells whose mana value (including chosen X), printed power, or printed toughness equals the number chosen on the evaluating source permanent. Source-dependent: evaluated by `TargetLegalityService` with the source permanent |
| `StackEntryManaValueParityMatchesSourceChosenParityPredicate` | `()` | spells whose mana value (including chosen X) is odd or even according to the quality chosen on the evaluating source permanent. Source-dependent: evaluated by `TargetLegalityService` with the source permanent |
...
| `StackEntryManaValueGreaterThanControllerExperienceCountersPredicate` | `()` | spells whose mana value (including chosen X) is greater than the evaluating controller's experience-counter total. Used by Mizzix of the Izmagnus's spell-cast trigger; evaluated by `TargetLegalityService.matchesStackEntryPredicate` |
...
| `StackEntryManaValueAtMostControllerGraveyardCountPredicate` | `()` | spells whose mana value <= the number of non-token cards in the spell controller's graveyard | `gameData` |
...
| `PlayerDamagedBySourceCombatThisTurnPredicate` | `()` | players dealt **combat damage** by the ability's own source permanent this turn (evaluated against `GameData.combatDamageToPlayersThisTurn`, keyed by source permanent id). Source-relative and used by Hope of Ghirapur |
...
| `CardHasKickerPredicate` | `()` | a card with a kicker or multikicker ability, detected from `KickerEffect` or a multikicker `RepeatableAdditionalManaCost`; use for filters such as “card with a kicker ability” |
...
| `CardHasColorManaSymbolPredicate` | `(ManaColor)` | cards whose mana cost contains a mana symbol containing the given color; hybrid and Phyrexian symbols count once, while generic and `{X}` symbols do not |
...
| `CardDoesNotShareNameWithControlledRoomPredicate` | `()` | a card that shares no name with any unlocked door of a Room controlled by the perspective player; requires the `GameData` overload of `matchesCardPredicate` and the evaluating player's id (Central Elevator, `DSK`) |
...
| `CardToughnessAtMostPredicate` | `(int maxToughness)` | a card whose printed toughness is <= `maxToughness`; cards without toughness (non-creatures) never match. Compose with `CardTypePredicate(CREATURE)` via `CardAllOfPredicate` for library searches like "a creature card with toughness 2 or less" (Recruiter of the Guard) |
...
| `CardNameStartsWithPredicate` | `(String prefix)` | a card whose name starts with `prefix`; combine with `CardTypePredicate` when a named card type is required, such as a Tamiyo planeswalker |
...
| `CardHasDisturbPredicate` | `()` | a card that has a disturb casting option; combine with `CardHasFlashbackPredicate` via `CardAnyOfPredicate` for "a card with flashback or disturb" (Phantom Carriage) |
| `CardHasNoAbilitiesPredicate` | `()` | a card with no rules text and no keyword abilities; compose with `CardTypePredicate(CREATURE)` for "creature card with no abilities" (Fang-Druid Summoner) |
| `CardHasXInManaCostPredicate` | `()` | a card whose mana cost contains `{X}`, regardless of the chosen X; use for "a land card or a card with `{X}` in its mana cost" library filters (Paradox Surveyor) |
