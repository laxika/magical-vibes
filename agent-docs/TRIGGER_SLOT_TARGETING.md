...
| `ON_DEALT_DAMAGE` (permanent-targeting may) | `DamageTriggerCollectorService.handleDealtDamageMayEffect` routes `MayEffect`s whose `targetSpec()` admits permanents (High Priest of Penance's "you may destroy target nonland permanent") through `SpellTargetTriggerAnyTarget` with the card's own `TargetFilter` — CR 603.3d: the target is chosen as the trigger goes on the stack, the "may" is answered on resolution. Non-targeting mays and every other effect keep the plain entry from `handleDealtDamageDefault` (damaged permanent as `targetId`, damage amount on `eventValue`) | Spell target |
...
| `ON_SELF_OR_ALLY_CREATURE_ENTERS_BATTLEFIELD` / `ON_ALLY_CREATURE_ENTERS_BATTLEFIELD` / `ON_ALLY_CREATURES_ENTERS_BATTLEFIELD` / `ON_OPPONENT_CREATURE_ENTERS_BATTLEFIELD` / `ON_OPPONENT_LAND_ENTERS_BATTLEFIELD` / `ON_ALLY_NONTOKEN_ARTIFACT_ENTERS_BATTLEFIELD` (permanent-targeting effects only) | `EnterTriggerCollectorService.handleEnterDefault` → `EntersTriggerTarget` (queued when the effect's `targetSpec()` includes permanents, e.g. Reaper King's "destroy target permanent"). Player-targeting effects still push straight to the stack with the pre-set `defaultTargetPlayerId`. The plural ally-creature slot is a single trigger for a simultaneous batch of one or more other creatures. | Enters (reuses `TriggerTargetCollector.Options.ATTACK` for the target list — permanents honouring the card's `PermanentPredicateTargetFilter` / `ControlledPermanentPredicateTargetFilter`; true "any target" effects are narrowed by evaluating `TargetPredicates.anyTarget()`) |
...
| `ON_ALLY_CREATURES_LEAVE_BATTLEFIELD_WITHOUT_DYING` (targeting effects only) | `PermanentRemovalService` → `TriggerCollectionService.checkAllyCreatureLeavesBattlefieldWithoutDyingTriggers` / batch variant → `SelfTriggeredAbilityTarget`; non-targeting effects push directly. Fires once per simultaneous non-graveyard leave event and uses last-known watchers when the source leaves in the same event. | Non-dying creature leave (reuses `TriggerTargetCollector.Options.END_STEP`) |
| `ON_SELF_OR_ALLY_CREATURES_LEAVE_BATTLEFIELD_WITHOUT_DYING` (targeting effects only) | `PermanentRemovalService` → `TriggerCollectionService.checkSelfOrAllyCreatureLeavesBattlefieldWithoutDyingTriggers` / batch variant → `SelfTriggeredAbilityTarget`; non-targeting effects push directly. Fires once when the source itself or one or more creatures its controller controls leave without dying. | Non-dying creature leave (reuses `TriggerTargetCollector.Options.END_STEP`) |
...
`ON_YOU_PUT_PLUS_ONE_PLUS_ONE_COUNTERS_ON_CREATURE` (Stocking the Pantry; fires once when the
controller puts one or more +1/+1 counters on a creature they control),
