...
| `ON_COMBAT_DAMAGE_TO_PLAYER` (graveyard-targeting) | `CombatDamageService` routes effects implementing `GraveyardCardChoosingEffect` (Skullsnatcher's `ExileCardsFromGraveyardEffect`) to `GraveyardTargetingService.handleCombatDamageGraveyardChoiceTargeting` — the same up-to-N multi-select, narrowed to the **damaged** player's graveyard ("from that player's graveyard") | Combat damage |
