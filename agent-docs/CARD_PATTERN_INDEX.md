...
| Counter + bounce | `l/LostInTheMist.java` | CounterSpellEffect + ReturnTargetPermanentToHandEffect — targets both a spell and a permanent. Uses `targetId` (spell, Zone.STACK) + `targetPermanentIds` (permanent). Multi-zone fizzle: only fizzles if ALL targets become illegal |
