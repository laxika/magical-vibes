...
| ETB tap opponent creature + untap lock while controlled | `d/DungeonGeists.java` | ON_ENTER_BATTLEFIELD TapTargetPermanentEffect + PreventTargetUntapWhileSourceOnBattlefieldEffect, target filter PermanentAllOfPredicate(IsCreature, Not(ControlledBySourceController)). Tapped creature stays locked until Dungeon Geists leaves the battlefield (same effect pair as Time of Ice's saga chapters). Contrast Watertrap Weaver which uses SkipNextUntapOnTargetEffect for a one-turn lock |
