...
| `ON_CONTROLLER_PLAYS_LAND` | Controller plays a land (land-play special action only — not every land ETB). Fired from every path that increments `landsPlayedThisTurn` via `TriggerCollectionService.checkControllerPlaysLandTriggers`. Pair with `ON_CONTROLLER_CASTS_SPELL` for "When you play a card" (Juju Bubble) |
