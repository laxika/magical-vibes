...
For an ability on a permanent that says "Unattach an Equipment from [this permanent]" and uses
that Equipment's mana value as X, use `new UnattachEquipmentFromSourceCost()`. It searches all
battlefields for an Equipment attached to the source, detaches the chosen Equipment, and records
its mana value as the activated ability's X value.
...
`GivePoisonCountersEffect` with recipient `CONTROLLER` is also supported as a mana-ability rider (Noxious Bayou). New rider effects belong in a `ManaAbilityEffectHandler` under `service/effect/manafx` so the class-keyed mana-ability registry handles them without adding engine-level type dispatch.

`MillEffect` with recipient `CONTROLLER` is also supported as a reflexive mana-ability rider (Shaun & Rebecca, Agents). The mana-ability classifier treats this controller-only mill form as an inline rider; targeted and opponent-recipient mill effects remain stack abilities.
...
| `WaterbendCost` | `(int amount)` | Waterbend cost: each untapped artifact or creature tapped while paying the waterbend cost pays for one generic mana. The activation and SPELL-slot paths tap only as many qualifying permanents as needed after using available mana, then pay the remaining generic mana normally. |
...
| `ExileNCardsFromGraveyardCost` | `(int count, CardType requiredType)` or `(int count, CardType requiredType, CardPredicate predicate)` | "Exile N matching cards from your graveyard: ..." (null type/predicate = unrestricted). Battlefield and graveyard-activated abilities prompt the player when more than one legal combination exists; the sole legal combination is paid automatically. Graveyard-activated abilities exclude the source card, so a self-return ability such as Salvage Titan needs N *other* matching cards. The AI chooses the lowest-mana-value legal cards. |
...
| `PayMulticoloredSourceManaCost` | `()` | Ability-only playtest `{Z}` cost: spends one mana tagged as produced by a source capable of producing at least two colors; use with the ordinary generic mana cost for cards such as Experiment Five (MB1 #73) |
...
| `COMMAND_ZONE_UPKEEP_TRIGGERED` | Controller's upkeep while the source card is in their command zone; use an intervening `SourceCardInCommandZone` condition when leaving the command zone before resolution should stop the effect |
...
| `EACH_DRAW_TRIGGERED` | At the beginning of any player's draw step (draw step only). For "each **opponent's** draw step", use an effect implementing `OpponentDrawStepOnlyEffect` with `opponentDrawStepOnly()` true — `StepTriggerService` then skips the trigger on the controller's own draw step (Malignant Growth, Well of Ideas) |
...
| `ON_ENCHANTED_PLAYER_DRAWS` | Whenever the player enchanted by this Aura draws a card (all draws: draw step, spells, abilities). The Aura controller is the triggered ability controller, and the attachment identifies the one enchanted player. |
...
| `EXILE_ON_ALLY_CREATURE_ATTACKS_UNBLOCKED` | Fires once per unblocked creature the controller controls from a face-up card in the controller's exile zone. Supports `TriggeringCardConditionalEffect` to filter the attacker; the stack entry carries the attacking creature and its attack destination, while an optional source permanent ID remains available for source-linked effects. Checked in `CombatBlockService`. Used by Senu, Keen-Eyed Protector |
...
| `GRAVEYARD_ON_ANY_CREATURE_ENTERS_BATTLEFIELD` | Like ON_ANY_OTHER_CREATURE_ENTERS_BATTLEFIELD but fires from every player's graveyard whenever any creature enters. A `TriggeringCardConditionalEffect(CardMinManaValuePredicate(...), inner)` can gate on the entering creature's mana value. Scanned in `TriggerCollectionService.checkAllyCreatureEntersTriggers`. Used by Dragon Scales ("when a creature with mana value 6 or greater enters") |
| `GRAVEYARD_ON_ANY_LAND_PUT_INTO_GRAVEYARD_FROM_ANYWHERE` | Fires from every graveyard whenever any non-token land card enters any graveyard from any zone. `MayPayManaEffect` can pay to return the source card to its owner's hand; checked in `GraveyardService.addCardToGraveyard`. Used by Centaur Vinecrasher. |
...
| `ON_CREATURE_ATTACKS_YOU_DIRECTLY` | Whenever a creature attacks you. Fires once per attacking creature only when it attacks the player directly, not a planeswalker they control. The attacker is stored as the non-targeting `targetId`. Used by Barbed Foliage |
...
| `ON_ANY_CREATURE_ATTACKS` | Whenever ANY creature attacks (any controller, any defender). Fires once per attacking creature, on every permanent with this slot across all battlefields. The attacking creature is set as the non-targeting `targetId`, so a plain `DealDamageToTargetCreatureEffect` hits "it"; the attack target is also captured as `attackedTargetId` for effects referring to the defending player. Checked in `CombatAttackService.declareAttackers`. Supports `TriggeringPermanentConditionalEffect` to restrict which attackers trigger it (Windreader Sphinx — flying attackers only). Used by Caltrops |
...
| `ON_CONTROLLER_PERMANENT_RETURNED_TO_HAND` | Whenever a permanent is returned from the battlefield to this permanent's controller's hand. Fires once per matching permanent, including the watching permanent itself, before removal. Fired from `PermanentRemovalService.removePermanentToHand` via `TriggerCollectionService.checkControllerPermanentReturnedToHandTriggers`. Used by Azorius Aethermage |
...
| `ON_COMBAT_DAMAGE_TO_CREATURE` | This creature deals combat damage to a creature. Fires once per damaged creature; the damaged creature is baked as non-targeting `targetId` so effects like `DestroyTargetPermanentEffect` / `PutCounterOnTargetPermanentEffect` act on "that creature"; granted, temporary, and persistent abilities are captured before combat deaths, so they still trigger when the granting permanent dies in the same damage event |
...
| `ON_ALLY_TOKEN_ENTERS_BATTLEFIELD` | One or more tokens enter battlefield under your control. Fires once per token-creation batch; wrap the effect in `TriggeringCardConditionalEffect` to restrict the trigger to matching token cards. |
...
| `ON_PERMANENT_ENTERS_FROM_EXILE` | A permanent enters the battlefield directly from exile under the source controller's control, checked via `enteredFromExile`. Queues a non-targeting stack entry for the source controller (`TriggerCollectionService.checkPermanentEntersFromExileTriggers`). Used by Fire Lord Zuko |
...
| `ON_ALLY_PERMANENT_ENTERS_BATTLEFIELD` | "Whenever another permanent you control enters" — fires for any permanent type under the source controller's control, excluding the source's own entry. The entering permanent is passed through the normal enters-trigger context; player or permanent targets are chosen as the trigger is put on the stack when needed. `TriggerCollectionService.checkAllyPermanentEntersTriggers`. |
...
| `ON_ANOTHER_ALLY_CREATURE_BECOMES_TARGET_OF_SPELL_OR_ABILITY` | Global monitor: another creature you control becomes target of any spell/ability. The targeted creature is stored as non-targeting `targetId`; the watcher itself is excluded. Used by Monk Gyatso |
...
| `ON_CONTROLLER_PLAYS_ANOTHER_LAND` | The controller plays a land other than the source itself. Uses the land-play path, not general landfall (City of Traitors). |
