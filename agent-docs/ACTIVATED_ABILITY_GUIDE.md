...
| `CraftMaterialCost` | `()`, `(CardSubtype)`, or `(minimumCount, requiredType, nonlandOnly, requireActivatedAbility)` | Craft's alternative material. The default exiles another artifact permanent you control or an artifact card from your graveyard; pass `null` for `requiredType` to allow any other permanents/cards. The subtype form requires a matching permanent or card. Use `CraftMaterialCost.nonlandsWithActivatedAbilities(N)` for N+ nonlands with activated abilities. Pair with `ExileSelfCost` and a return-from-exile transformed effect. |
