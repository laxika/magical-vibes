...
For a numeric graveyard-departure gate, use `CardsLeftGraveyardAtLeastThisTurn(minimum)`; it counts every card in a batched departure event, not just whether the graveyard changed.
...
| `TapCreaturesForManaCost` | `()` | optional ability-side mana payment: choose any number of untapped creatures you control and reduce the activation's mana cost by one per chosen creature. Send those payment choices in the activation request's `targetIds`; they are not ability targets (Heirloom Epic). |
...
| `ON_CONTROLLER_GIVES_GIFT` | Controller gives an opponent a promised Gift; dispatched when the marked opponent-token creation resolves, not when Gift is merely promised |
...
| `ON_ALLY_CREATURE_DEALS_DAMAGE_TO_PLANESWALKER` | A creature you control deals damage to a planeswalker. Fires once per source and planeswalker damage event; the target planeswalker ID is baked into the non-targeting trigger entry — Hooded Blightfang |
...
| `ON_ALLY_CREATURES_LEAVE_BATTLEFIELD_WITHOUT_DYING` | One or more other creatures you control leave the battlefield without going to a graveyard. Fires once per simultaneous non-dying leave event, including when the watching permanent leaves in that event. Fired by `PermanentRemovalService` through its non-dying leave batch pipeline. Used by Dour Port-Mage (`DrawCardEffect(1)`) |
| `ON_SELF_OR_ALLY_CREATURES_LEAVE_BATTLEFIELD_WITHOUT_DYING` | This creature or one or more creatures you control leave the battlefield without going to a graveyard. Fires once per simultaneous non-dying leave event, including when the watching permanent itself is the only creature leaving. Fired by `PermanentRemovalService` through its non-dying leave batch pipeline. Used by Three Tree Scribe (`PutCounterOnTargetPermanentEffect(PLUS_ONE_PLUS_ONE, 1)`) |
