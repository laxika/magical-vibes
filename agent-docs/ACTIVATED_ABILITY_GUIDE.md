...
| `RemoveCounterFromControlledPermanentCost` | `()` | Removes one counter of any kind from a permanent you control. As an activated-ability cost, one candidate pays automatically and several prompt a permanent choice; as a `ForcedCostOrElseEffect` cost, no counters means the penalty resolves without a prompt. On a permanent with several counter kinds the first kind present is removed. Chisei, Heart of Oceans; Power Conduit |
