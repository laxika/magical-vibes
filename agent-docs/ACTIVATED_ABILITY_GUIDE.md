...
`Card.addExileActivatedAbility()` and are activated by card id, not an exile list index, because the
exile zone is unified and can contain cards owned by different players.

- The ability is exposed via `Card.getExileActivatedAbilities()` and `CardView.exileActivatedAbilities`
- Activation is routed through `AbilityActivationService.activateExileAbility()` and the
  `ACTIVATE_EXILE_ABILITY` message
