...
- Prowl ("Prowl {cost}" — an alternate hand cost gated on combat damage):
  ...
  - A static "[filter] permanent spells you cast have evoke {cost}" grant uses `GrantEvokeToSpellsEffect`; the engine synthesizes the alternate cast and adds the standard evoke sacrifice ETB to the runtime card.
...
| `ConditionalEffect(new SelfDealtDamageToCreatureThisTurn(), wrapped)` | - | yes | yes (end step) | source dealt actual damage to another creature this turn, including a creature that later left the battlefield; combat and noncombat damage are tracked separately from player damage |
