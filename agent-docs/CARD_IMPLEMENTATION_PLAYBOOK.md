...
- Distinct targets (CR 114.10a): when oracle text says "another target" or "different target", call `setRequireDistinctTargets(true)` on the Card. This enforces cross-group uniqueness validation — without it, the engine only checks within-group uniqueness per CR 114.6c. Example: `BloodFeud` ("Target creature fights another target creature").
