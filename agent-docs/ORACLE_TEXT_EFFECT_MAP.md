...
| "[body], then clash with an opponent. If you win, repeat this process." | `addEffect(SPELL, new RepeatWhileWinningClashEffect(List.of(bodyEffects...)))` | SPELL | Clash-*source* loop (LRW). Runs the body first, then clashes; repeats the whole sequence while the controller keeps winning `performClash`, stopping on the first loss. Contrast `ClashEffect` (clash first, reward only on win). E.g. Hoarder's Greed: `RepeatWhileWinningClashEffect(List.of(new LoseLifeEffect(2), new DrawCardEffect(2)))` |
...
| "put a +1/+1 counter on CARDNAME. If this is the third time this ability has resolved this turn, remove all +1/+1 counters and it deals that much damage to each creature and each player" | `AshlingThePilgrimEffect()` | ACTIVATED_ABILITY | Card-specific; counts resolutions per source permanent this turn, fires only on the exact third. Ashling the Pilgrim |
