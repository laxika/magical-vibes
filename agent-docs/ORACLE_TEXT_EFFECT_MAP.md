...
| "Increment (Whenever you cast a spell, if the mana you spent is greater than this creature's power or toughness, put a +1/+1 counter on it)" | `IncrementTriggerEffect()` | `ON_CONTROLLER_CASTS_SPELL` | Increment keyword (SOS). Place directly on the slot; collector snapshots mana spent and re-checks on resolution (e.g. Ambitious Augmenter) |
