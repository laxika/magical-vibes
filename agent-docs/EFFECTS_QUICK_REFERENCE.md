...
- `MatchingCreaturesCantAttackOrBlockEffect(PermanentPredicate matcher, String description)` — global STATIC: while the source is on the battlefield, any creature matching `matcher` (either player's) can't attack AND can't block (Light of Day: "Black creatures can't attack or block", `matcher = PermanentColorInPredicate(BLACK)`). Attack side in `CombatAttackService.isCantAttackDueToGlobalRestriction`, block side in `GameQueryService.canBlock`
